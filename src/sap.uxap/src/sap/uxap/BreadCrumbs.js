...
sap.ui.define(
    [
      "sap/m/Link", "sap/m/Select", "sap/ui/core/Control",
      "sap/ui/core/ResizeHandler", "sap/ui/core/delegate/ItemNavigation",
      "sap/ui/core/Item", "sap/ui/core/Icon", "sap/ui/Device", "./library",
      "sap/ui/core/InvisibleText", "sap/ui/util/openWindow",
      "./BreadCrumbsRenderer", "sap/ui/thirdparty/jquery"
    ],
    function(Link, Select, Control, ResizeHandler, ItemNavigation, Item, Icon,
             Device, library, InvisibleText, openWindow, BreadCrumbsRenderer,
             jQuery) {
      "use strict";
       ...
       * <h3>Overview</h3>
       *
       * The <code>BreadCrumbs</code> control allows the users to quickly
       * navigate to a previous location on the path that got them to the
       * current location by choosing the displayed navigation steps.
       *
       * It has two main modes of operation:
       * <ul>
       * <li>A trail of links followed by separators, when there's enough space
       * for the control to fit on one line.</li> <li>A dropdown list with the
       * links, when the trail of links wouldn't fit on one line.</li>
       * </ul>
      ...
      var BreadCrumbs = Control.extend(
          "sap.uxap.BreadCrumbs", /** @lends sap.uxap.BreadCrumbs.prototype */ {
            metadata : {
              ...
              properties : {
                ...
                /**
                 * Sets the visibility of the current/last element in the
                 * BreadCrumbs path.
                 */
                showCurrentLocation :
              ...
              aggregations : {
                ...
                /**
                 * An icon that is used as a separator after each link in the
                 * standard mode.
                 */
                _tubeIcon : {
                ...
                /**
                 *
                 * A select control which is used to display the BreadCrumbs
                 * content on smaller mobile devices or when there's not enough
                 * space for the control to fit on one line.
                 */
                _overflowSelect : {
                  type : "sap.m.Select",
                  multiple : false,
      ...
      BreadCrumbs.prototype.onAfterRendering =
          function() { this._handleInitialModeSelection(); };

      /**
       * Handles the initial mode selection between overflowSelect and normal
       * mode
       *
       * @private
       * @returns {object} this
      ...
      BreadCrumbs.prototype._getOverflowSelect = function() {
        var oOverflowSelect, aSelectItems;
      ...
      BreadCrumbs.prototype._overflowSelectChangeHandler = function(oEvent) {
        var oSelectedKey = oEvent.getParameter("selectedItem").getKey(),
            oControl = sap.ui.getCore().byId(oSelectedKey), sLinkHref,
            sLinkTarget;
      ...
      /**
       * Handles the decision making on whether or not the control should go
       * into overflow mode
       *
      ...
      BreadCrumbs.prototype._shouldOverflow = function() {
        ...
        bShouldOverflow =
            $breadcrumbs.outerHeight() > this._iContainerMaxHeight;
      ...
      BreadCrumbs.prototype._getAriaLabelledBy = function() {
        if (!this._oAriaLabelledBy) {
          BreadCrumbs.prototype._oAriaLabelledBy =
              new InvisibleText({
                text : sap.ui.getCore()
                           .getLibraryResourceBundle("sap.uxap")
                           .getText("BREADCRUMB_TRAIL_LABEL")
              }).toStatic();
        }
      ...
      BreadCrumbs.prototype._configureKeyboardHandling = function() {
        ...
        // fix the tab indexes so the first link to be 0 and read correctly by
        // the screen reader
       ...
       * @private
       */
      BreadCrumbs.prototype.onsappageup = function(
          oEvent) { this._handlePageKeys(oEvent, false); };
       ...
       * @private
       */
      BreadCrumbs.prototype.onsappagedown = function(
          oEvent) { this._handlePageKeys(oEvent, true); };

      BreadCrumbs.prototype._handlePageKeys = function(oEvent, bMovingDown) {
        var iNextIndex, aBreadCrumbs = this._getItemsToNavigate(),
                        iEventTargetIndex = 0,
        ...
        if (bMovingDown) {
          iNextIndex =
              iEventTargetIndex + BreadCrumbs.PAGEUP_AND_PAGEDOWN_JUMP_SIZE;
        } else {
          iNextIndex =
              iEventTargetIndex - BreadCrumbs.PAGEUP_AND_PAGEDOWN_JUMP_SIZE;
        }

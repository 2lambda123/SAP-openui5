...
sap.ui.define(
    [
      "sap/ui/core/Renderer", "sap/ui/core/library", "sap/ui/core/Core",
      "sap/ui/Device", "sap/base/Log", "./library", "./ListItemBaseRenderer"
    ],
    function(Renderer, coreLibrary, Core, Device, Log, library,
      ...
      ColumnListItemRenderer.getAriaRole = function(oLI) { return ""; };
       ...
       *            oRenderManager the RenderManager that can be used for
      ...
      ColumnListItemRenderer.renderLIContentWrapper = function(rm, oLI) {
        ...
        var aColumns = oTable.getColumns(true), aCells = oLI.getCells();

        // remove cloned headers
        oLI._destroyClonedHeaders();

        aColumns.forEach(function(oColumn, i) {
          var oHeader, bRenderCell = true,
          ...
          if (oColumn) {
            ...
            aStyleClass && aStyleClass.forEach(function(
                               sClassName) { rm.class(sClassName); });
            ...
            if (!oTable.hasPopin() && oColumn.getMergeDuplicates()) {
              var sFuncWithParam = oColumn.getMergeFunctionName(),
                  ...
                  sFuncParam = aFuncWithParam[1], sFuncName = aFuncWithParam[0];

              if (typeof oCell[sFuncName] != "function") {
                Log.warning("mergeFunctionName property is defined on " +
                            oColumn + " but this is not function of " + oCell);
                ...
                if (lastColumnValue === cellValue) {
                  // it is not necessary to render the cell content but screen
                  // readers need the content to announce it
            ...
            oColumn.getVAlign() != "Inherit" &&
                rm.style("vertical-align", oColumn.getVAlign().toLowerCase());
      ...
      ColumnListItemRenderer.applyAriaLabelledBy = function(oHeader, oCell) {
        /* add the header as an aria-labelled by association for the cells if it
         * does not already exists */
        /* only set the header text to the aria-labelledby association if the
         * header is a textual control and visible */
        if (oHeader && oHeader.getText && oHeader.getVisible() &&
      ...
      ColumnListItemRenderer.renderPopin = function(rm, oLI, oTable) {
        ...
        rm.attr("colspan", oTable.shouldRenderDummyColumn()
                               ? oTable.getColSpan() + 1
                               : oTable.getColSpan());

        var sPopinLayout = oTable.getPopinLayout();
        // overwrite sPopinLayout=Block to avoid additional margin-top in IE and
        // Edge
        if (Device.browser.msie ||
            (Device.browser.edge && Device.browser.version < 16)) {
          sPopinLayout = PopinLayout.Block;
        }

        rm.attr("aria-labelledby",
                this.getAriaAnnouncement(null, "TABLE_POPIN_ROLE_DESCRIPTION"));
        rm.openEnd();
        ...
        var aCells = oLI.getCells(), aColumns = oTable.getColumns(true);
        ...
        aColumns.forEach(function(oColumn) {
          ...
          if (oHeader && sPopinDisplay != PopinDisplay.WithoutHeader) {
            ...
            rm.text(Core.getLibraryResourceBundle("sap.m").getText(

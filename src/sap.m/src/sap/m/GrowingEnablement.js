...
sap.ui.define(
    [
      'sap/ui/base/Object', 'sap/ui/core/format/NumberFormat', 'sap/m/library',
      'sap/ui/model/ChangeReason', 'sap/ui/base/ManagedObjectMetadata',
      'sap/ui/core/HTML', 'sap/m/CustomListItem', "sap/base/security/encodeXML"
    ],
    function(BaseObject, NumberFormat, library, ChangeReason,
             ManagedObjectMetadata, HTML, CustomListItem, encodeXML) {
      "use strict";
      ...
      /**
       * Creates a GrowingEnablement delegate that can be attached to ListBase
       * Controls requiring capabilities for growing
       *
       * @extends sap.ui.base.Object
       * @alias sap.m.GrowingEnablement
       * @experimental Since 1.16. This class is experimental and provides only
       * limited functionality. Also the API might be changed in future.
       *
       * @param {sap.m.ListBase} oControl the ListBase control of which this
       *     Growing is the delegate
       *
       * @constructor
       * @protected
       */
      var GrowingEnablement = BaseObject.extend(
          "sap.m.GrowingEnablement",
          /** @lends sap.m.GrowingEnablement.prototype */ {
             ...
             * This function must be called by the control which uses this
            ...
            onAfterRendering : function() {
              ...
              if (oControl.getGrowingScrollToLoad()) {
                ...
                if (oScrollDelegate) {
                  ...
                  oScrollDelegate.setGrowingList(
                      this.onScrollToLoad.bind(this),
                      oControl.getGrowingDirection(),
                      this._updateTrigger.bind(this, false));
                }
            ...
            setTriggerText : function(
                sText) { this._oControl.$("triggerText").text(sText); },

            // reset paging on rebind
            reset : function() {
              this._iLimit = 0;

              // if factory function is used we do not activate the replace
              // option of the extended change detection
              var oBindingInfo = this._oControl.getBindingInfo("items");
              this._oControl.oExtendedChangeDetectionConfig =
                  (!oBindingInfo || !oBindingInfo.template) ? null
                                                            : {replace : true};
            },
            ...
            onScrollToLoad : function() {
              ...
              if (this._bLoading || !oTriggerButton ||
                  oTriggerButton.style.display != "none") {
                return;
              }

              if (this._oControl.getGrowingDirection() ==
                  ListGrowingDirection.Upwards) {
            ...
            requestNewPage : function() {
              ...
              if (oBinding && !oBinding.isLengthFinal() ||
                  this._iLimit < this._oControl.getMaxItemsCount()) {
                // The GrowingEnablement has its own busy indicator. Do not show
                // the busy indicator, if existing, of the parent control.
                if (this._oControl.getMetadata().hasProperty(
                        "enableBusyIndicator")) {
                  this._bParentEnableBusyIndicator =
                      this._oControl.getEnableBusyIndicator();
            ...
            _onAfterPageLoaded : function(sChangeReason) {
              ...
              // After the data has been loaded, restore the busy indicator
              // handling of the parent control.
              if (this._oControl.setEnableBusyIndicator) {
                this._oControl.setEnableBusyIndicator(
                    this._bParentEnableBusyIndicator);
              }
            },
            ...
            _getTrigger : function() {
              ...
              sTriggerText =
                  sTriggerText ||
                  sap.ui.getCore().getLibraryResourceBundle("sap.m").getText(
                      "LOAD_MORE_DATA");
              ...
              // The growing button is changed to span tag as h1 tag was
                    ...
                    content : new HTML({
                      content :
                          '<div class="sapMGrowingListTrigger">' +
                              ...
                              '<span class="sapMSLITitle" id="' + sTriggerID +
                              'Text">' + encodeXML(sTriggerText) + '</span>' +
                              '</div>' +
                      ...
                      .attachPress(this.requestNewPage, this)
                      .addDelegate({
                        ...
                        onAfterRendering : function(oEvent) {
                          ...
                          // aria-selected is added as the CustomListItem
                          // type="Active" aria-selected should be removed as it
                          // is not allowed with role="button"
            ...
            _getListItemInfo : function() {
              return ("[ " + this._iRenderedDataItems + " / " +
                      NumberFormat.getFloatInstance().format(
                          this._oControl.getMaxItemsCount()) +
                      " ]");
            },
            ...
            // determines if the scroll container of the list has enough
            // scrollable area to hide the growing button
            _getHasScrollbars : function() {
              ...
              // after growing-button gets hidden scroll container should still
              // be scrollable
              return this._oScrollDelegate.getMaxScrollTop() >
                     this._oControl.getDomRef("triggerList").clientHeight;
            },
            ...
            addListItem : function(oContext, oBindingInfo,
                                   bSuppressInvalidate) {
              var oControl = this._oControl, oBinding = oBindingInfo.binding,
              ...
              if (oBinding.isGrouped()) {
                ...
                var aItems = oControl.getItems(true),
                    ...
                    oGroupInfo =
                        oBinding.getGroup(oItem.getBindingContext(sModelName));

                if (oLastItem && oLastItem.isGroupHeader()) {
                  oControl.removeAggregation("items", oLastItem, true);
                  this._fnAppendGroupItem = this.appendGroupItem.bind(
                      this, oGroupInfo, oLastItem, bSuppressInvalidate);
                  oLastItem = aItems[aItems.length - 1];
                }
                ...
                if (!oLastItem ||
                    oGroupInfo.key !==
                        oBinding
                            .getGroup(oLastItem.getBindingContext(sModelName))
                            .key) {
                  var oGroupHeader =
                      (oBindingInfo.groupHeaderFactory)
                          ? oBindingInfo.groupHeaderFactory(oGroupInfo)
                          : null;
                  if (oControl.getGrowingDirection() ==
                      ListGrowingDirection.Upwards) {
                    this.applyPendingGroupItem();
                    this._fnAppendGroupItem = this.appendGroupItem.bind(
                        this, oGroupInfo, oGroupHeader, bSuppressInvalidate);
                  } else {
                    this.appendGroupItem(oGroupInfo, oGroupHeader,
                                         bSuppressInvalidate);
                  }
                }
              }
            ...
            appendGroupItem : function(oGroupInfo, oGroupHeader,
                                       bSuppressInvalidate) {
              oGroupHeader = this._oControl.addItemGroup(
                  oGroupInfo, oGroupHeader, bSuppressInvalidate);
              if (bSuppressInvalidate) {
            ...
            // render all the collected items in the chunk and flush them into
            // the DOM vInsert whether to append (true) or replace (falsy) or to
            // insert at a certain position (int)
            applyChunk : function(vInsert, oDomRef) {
              ...
              // prevent DOM updates when there are no visible columns in the
              // table and items are being inserted (see BCP: 2080250160)
              if (this._oControl.isA("sap.m.Table")) {
                var bHasVisibleColumn =
                    this._oControl.getColumns().some(function(
            ...
            addListItems : function(aContexts, oBindingInfo,
                                    bSuppressInvalidate) {
              for (var i = 0; i < aContexts.length; i++) {
                this.addListItem(aContexts[i], oBindingInfo,
                                 bSuppressInvalidate);
              }
            },

            // destroy all the items and create from scratch
            rebuildListItems : function(aContexts, oBindingInfo,
                                        bSuppressInvalidate) {
              this.destroyListItems(bSuppressInvalidate);
              this.addListItems(aContexts, oBindingInfo, bSuppressInvalidate);
              if (bSuppressInvalidate) {
                var bHasFocus =
                    this._oContainerDomRef.contains(document.activeElement);
            ...
            refreshItems : function(sChangeReason) {
              ...
              // set iItemCount to initial value if not set or no items at the
              // control yet
              if (!this._iLimit || this.shouldReset(sChangeReason) ||
                  !this._oControl.getItems(true).length) {
                this._iLimit = this._oControl.getGrowingThreshold();
              }
            ...
            updateItems : function(sChangeReason) {
              ...
              // set limit to initial value if not set yet or no items at the
              // control yet
              if (!this._iLimit || this.shouldReset(sChangeReason) ||
                  !aItems.length) {
              ...
              // fire growing started event if data was requested this is a
              // followup call of updateItems
              if (this._bDataRequested) {
                this._bDataRequested = false;
              } else {
                this._onBeforePageLoaded(sChangeReason);
              ...
              // if getContexts did cause a request to be sent, set the internal
              // flag so growing started event is not fired again
              if (aContexts.dataRequested) {
                this._bDataRequested = true;

                // a partial response may already be contained, so only return
                // here without updating the list when diff is empty
                if (aContexts.diff && !aContexts.diff.length) {
                  return;
                }
              }

              // cache dom ref for internal functions not to lookup again and
              // again
              this._oContainerDomRef = oControl.getItemsContainerDomRef();

              // aContexts.diff ==> undefined : New data we should build from
              // scratch aContexts.diff ==> [] : There is no diff, means data
              // did not changed at all aContexts.diff ==> [{index: 0, type:
              // "delete"}, {index: 1, type: "insert"},...] : Run the diff logic
              var aDiff = aContexts.diff, bFromScratch = false, vInsertIndex;
              ...
              if (!aContexts.length) {
              ...
              } else if (oBinding.isGrouped() ||
                         oControl.checkGrowingFromScratch()) {

                if (this._sGroupingPath != this._getGroupingPath(oBinding)) {
                  // grouping is changed so we need to rebuild the list for the
                  // group headers
                  bFromScratch = true;
                } else {
                  // append items if possible
                  for (var i = 0; i < aDiff.length; i++) {
                    var oDiff = aDiff[i], oContext = aContexts[oDiff.index];
                ...
                if (this._sGroupingPath) {
                  // if it was already grouped then we need to remove group
                  // headers first
                ...
                for (var i = 0; i < aDiff.length; i++) {
                  var oDiff = aDiff[i], iDiffIndex = oDiff.index,
                  ...
                  if (oDiff.type == "delete") {
                    ...
                    if (vInsertIndex == -1) {
                      // the subsequent of items needs to be inserted at this
                      // position
                      vInsertIndex = iDiffIndex;
                    } else if (iLastInsertIndex > -1 &&
                               iDiffIndex != iLastInsertIndex + 1) {
                      // this item is not simply appended to the last one but
                      // has been inserted
                      this.applyChunk(vInsertIndex);
                      vInsertIndex = iDiffIndex;
                    }
              ...
              if (bFromScratch) {
                ...
                // set the binding context of items inserting/deleting entries
                // shifts the index of all following items
            ...
            _updateTriggerDelayed : function(bLoading) {
              if (this._oControl.getGrowingScrollToLoad()) {
                this._iTriggerTimer &&
                    window.cancelAnimationFrame(this._iTriggerTimer);
                this._iTriggerTimer = window.requestAnimationFrame(
                    this._updateTrigger.bind(this, bLoading));
              } else {
            ...
            _updateTrigger : function(bLoading) {
              var oTrigger = this._oTrigger, oControl = this._oControl,
                  bVisibleItems =
                      oControl && oControl.getVisibleItems().length > 0;

              // If there are no visible columns or items then also hide the
              // trigger.
              if (!oTrigger || !oControl || !bVisibleItems ||
                  !oControl.shouldRenderItems() || !oControl.getDomRef()) {
                return;
              }
              ...
              oTrigger.$().toggleClass("sapMGrowingListBusyIndicatorVisible",
                                       bLoading);
              ...
              if (bLoading) {
                ...
                // put the focus to the newly added item if growing button is
                // pressed
                if (oTriggerDomRef &&
                    oTriggerDomRef.contains(document.activeElement)) {
                ...
                if (!iItemsLength || !this._iLimit ||
                  ...
                  if (bLengthFinal) {
                    oControl.$("triggerInfo")
                        .css("display", "block")
                        .text(this._getListItemInfo());
                  }

                  oTrigger.$().removeClass(
                  ...
                  if (oControl.isA("sap.m.Table") && !oControl.hasPopin() &&
                      oControl.shouldRenderDummyColumn()) {
                    var sCalWidth =
                        Array.from(oControl.getDomRef("tblHeader").childNodes)
                            .slice(0, -1)
                            .map(function(oDomRef) {
                              var sWidth =
                                  oDomRef.getAttribute("data-sap-width");
                              if (!sWidth || !sWidth.includes("%")) {
                                return oDomRef.getBoundingClientRect().width +
                                       "px";
                              } else {
                                return sWidth;
                              }
                    ...
                    oTriggerDomRef.style.width =
                        "calc(" + sCalWidth + " + 1px)";
                    oTriggerDomRef.classList.add("sapMGrowingListDummyColumn");
                  }
                }

                // store the last item count to be able to focus to the newly
                // added item when the growing button is pressed
                this._iLastItemsCount = this._oControl.getItems(true).length;

                // at the beginning we should scroll to last item
                if (bHasScrollToLoad && this._oScrollPosition === undefined &&
                    oControl.getGrowingDirection() ==
                        ListGrowingDirection.Upwards) {
                  this._oScrollPosition = {left : 0, top : 0};
                }
                ...
                if (iItemsLength > 0 && this._oScrollPosition) {
                  ...
                  oScrollDelegate.scrollTo(oScrollPosition.left,
                                           oScrollDelegate.getScrollHeight() -
                                               oScrollPosition.top);
      ...
      GrowingEnablement.createItem = function(oContext, oBindingInfo,
                                              sIdSuffix) {
        var oItem = oBindingInfo.factory(
            ManagedObjectMetadata.uid(sIdSuffix ? sIdSuffix : "clone"),
            oContext);

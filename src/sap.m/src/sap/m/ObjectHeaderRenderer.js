...
sap.ui.define(
    [
      'sap/ui/core/Control', 'sap/ui/core/library', 'sap/m/library',
      'sap/ui/Device', "sap/base/Log", 'sap/m/Link', 'sap/m/Text',
      "sap/ui/thirdparty/jquery", "sap/ui/util/defaultLinkTypes"
    ],
    function(Control, coreLibrary, library, Device, Log, Link, Text, jQuery,
       ...
       * @private
       */
      ObjectHeaderRenderer._cleanupNotRenderedChildControls = function(oRM,
                                                                       oOH) {
       ...
       * @private
       */
      ObjectHeaderRenderer._renderIntro = function(oRM, oOH, sIntroClass,
                                                   sIntroActiveClass) {
       ...
       * @private
       */
      ObjectHeaderRenderer._renderAttribute = function(oRM, oOH, oAttr,
                                                       bFullWidth) {
        oRM.openStart("div");
      ...
      ObjectHeaderRenderer._renderRow = function(oRM, oOH, oLeft, aRight) {
        ...
        if (!ObjectHeaderRenderer._isEmptyObject(
                oLeft)) { // if the object with the attributes is not empty then
                          // render the attributes
          this._renderAttribute(oRM, oOH, oLeft,
                                ObjectHeaderRenderer._isEmptyArray(aRight));
          ...
          if (aRight[0] instanceof
              sap.m.ProgressIndicator) { // check if the first element in the
        ...
        if (!ObjectHeaderRenderer._isEmptyArray(
                aRight)) { // check do we have statuses, icons or progress
      ...
      ObjectHeaderRenderer._renderAttributesAndStatuses = function(oRM, oOH) {
        ...
        if (!oOH.getResponsive()) {
          for (var iCount = 0; iCount < iNoOfRows; iCount++) {
            this._renderRow(oRM, oOH, aVisibleAttribs[iCount],
      ...
      ObjectHeaderRenderer.render = function(oRM, oOH) {
        ...
        oRM.accessibilityState({
          role : "region",
          labelledby : {value : oOH.getId() + "-titleText-inner", append : true}
        });
      ...
      ObjectHeaderRenderer._renderChildControl = function(oRM, oOH, oControl) {
        ...
        if (!oOH.getResponsive() &&
            oOH.__controlsToBeRendered) { // if control is rendered remove it
                                          // from the array
       ...
       * @private
       **/
      ObjectHeaderRenderer._renderResponsiveTitleBlock = function(oRM,
                                                                  oControl) {
        // Title container displayed to the left of the number and number units
        // container.
        oRM.openStart("div",
      ...
      ObjectHeaderRenderer._renderResponsiveRow = function(oRM, oOH) {
        ...
        var aVisibleAttrAndStat =
                aVisAtribsStatuses[0].concat(aVisAtribsStatuses[1]),
        ...
        if ((Device.system.tablet && !Device.system.desktop) ||
          ...
          if (!oOH.getFullScreenOptimized() ||
              (Device.orientation.portrait &&
               oOH.getFullScreenOptimized())) { // full screen portrait or
                                                // master detail
            iRenderCols = 2;                    // render two columns
            sClassColCount = 'sapMOHRTwoCols';
          } else {
            if (oOH.getFullScreenOptimized() &&
                (Device.orientation.landscape ||
                 (Device.system.desktop &&
                  oOH._isMediaSize("Tablet")))) { // full screen landscape
        ...
        this._renderResponsiveStatesColumn(oRM, oOH, iRenderCols,
                                           aVisibleAttrAndStat,
                                           iCountVisibleAttr, sClassColCount);
      };
      ...
      ObjectHeaderRenderer._renderResponsiveStatesColumn = function(
          oRM, oOH, iRenderCols, aVisibleAttrAndStat, iCountVisibleAttr,
          sClassColCount) {
        var iCountInCols = Math.floor(
            aVisibleAttrAndStat.length /
            iRenderCols); // number of attributes and states in each column
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveAttribute = function(oRM, oOH,
                                                                 oAttr) {
        oRM.openStart("div");
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveStatus = function(oRM, oOH,
                                                              oStatus) {
        oRM.openStart("div");
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveTitleAndArrow = function(oRM, oOH,
                                                                     nCutLen) {
      ...
      ObjectHeaderRenderer._rerenderResponsiveStates = function(oRM, oOH) {
        ...
        if (Device.orientation
                .portrait) { // full screen portrait or master detail
          iRenderCols = 2;   // render two columns
          sClassColCount = 'sapMOHRTwoCols';
        } else {
          if (iCountAttrAndStat >= 1 && iCountAttrAndStat <= 2) {

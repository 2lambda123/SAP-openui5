...
sap.ui.define(
    ...
    function(
       ...
       * @namespace
       */
      var ObjectHeaderRenderer = {apiVersion : 2};

      /**
       * Check if the object exists. In case object has _isEmpty() method then
       * this method is called. If there is no such method then object is not
       * empty.
       *
      ...
      ObjectHeaderRenderer._isEmptyObject = function(oObject) {
        ...
        if ((!oObject._isEmpty || !oObject._isEmpty()) &&
            (!oObject.getVisible || oObject.getVisible())) {
          return false;
        }
        return true;
      };
      ...
      /**
       * Array is considered empty if it is null or undefined or has no controls
       * or all the controls are empty.
       *
      ...
      ObjectHeaderRenderer._isEmptyRow = function(oLeft, aRight) {
        return (ObjectHeaderRenderer._isEmptyObject(oLeft) &&
                ObjectHeaderRenderer._isEmptyArray(aRight));
      };
       ...
       *
       * @param {sap.ui.core.Control[]} aObjects array of controls to be
       *     renderer
       *
       ...
       * @private
       */
      ObjectHeaderRenderer._computeChildControlsToBeRendered = function(
          oOH,
      ) {
      ...
      /**
       * Delete all controls that were empty and were not rendered inside Object
       * Header.
       *
       * @param {sap.ui.core.RenderManager} oRM the RenderManager that can be
       *     used for writing to the render output buffer
       ...
       * @private
       */
      ObjectHeaderRenderer._cleanupNotRenderedChildControls = function(
          oRM,
          oOH,
      ) {
        for (var id in oOH.__controlsToBeRendered) {
          oRM.cleanupControlWithoutRendering(
              oOH.__controlsToBeRendered[id],
          );
        }
       ...
       * @private
       */
      ObjectHeaderRenderer._getMarkers = function(
          oOH) { return oOH._getVisibleMarkers(); };

      /**
       * Render intro as sap.m.Text or sap.m.Link depending if it's active or
       * not. used in both ObjectHeader and ObjectHeaderResponsive
       *
       * @param {sap.ui.core.RenderManager} oRM the RenderManager that can be
       *     used for writing to the render output buffer
       ...
       * @param {string} sIntroActiveClass the css class of the intro container
       *     if the intro is active
       * @private
       */
      ObjectHeaderRenderer._renderIntro = function(
          oRM,
          oOH,
          sIntroClass,
          sIntroActiveClass,
       ...
       * @private
       */
      ObjectHeaderRenderer._renderAttribute = function(
          oRM,
          oOH,
          oAttr,
          bFullWidth,
      ...
      /**
       * Validate the statuses control list to only display sap.m.ObjectStatus
       * and sap.m.ProgressIndicator and returns only the visible once that
       * should be rendered
       *
        ...
        if (oOH.getStatuses()) {
          ...
          for (var i = 0; i < aStatuses.length; i++) {
            if (!aStatuses[i].getVisible || aStatuses[i].getVisible()) {
              if ((aStatuses[i] instanceof sap.m.ObjectStatus &&
                   !aStatuses[i]._isEmpty()) ||
                  aStatuses[i] instanceof sap.m.ProgressIndicator) {
                aVisibleStatuses.push([ aStatuses[i] ]);
              } else {
                Log.warning(
                    'Only sap.m.ObjectStatus or sap.m.ProgressIndicator are allowed in "sap.m.ObjectHeader.statuses" aggregation.' +
                        " Current object is " +
                        aStatuses[i].constructor.getMetadata().getName() +
                        ' with id "' + aStatuses[i].getId() + '"',
                );
              }
            }
          }
      ...
      /**
       * Returns only the visible statuses and attributes that should be
       * rendered
       *
       * @param {sap.m.ObjectHeader} oOH an object representation of the control
       *     that should be rendered
       * @returns {array} The visible statuses and attributes
       * @private
       */
      ObjectHeaderRenderer._getVisibleAttribsAndStatuses = function(oOH) {
        var aResult = [], aAttribs = oOH.getAttributes(), aVisibleAttribs = [];
      ...
      ObjectHeaderRenderer._renderRow = function(oRM, oOH, oLeft, aRight) {
        ...
        if (!ObjectHeaderRenderer._isEmptyObject(oLeft)) {
          // if the object with the attributes is not empty then
        ...
        } else if (ObjectHeaderRenderer._isEmptyObject(oLeft) &&
                   !ObjectHeaderRenderer._isEmptyArray(aRight)) {
          // if there are no attributes at all and the array containing statuses
          // and progress indicators isn't empty
          if (aRight[0] instanceof sap.m.ProgressIndicator) {
            // check if the first element in the
            // array is progress indicator, and if
            // it's so then place an empty
            // "attribute" div before the progress
            // indicator
            oRM.openStart("div");
            oRM.class("sapMOHAttr");
            oRM.openEnd();
        ...
        if (!ObjectHeaderRenderer._isEmptyArray(aRight)) {
          // check do we have statuses, icons or progress
          // indicators and render them accordingly
          oRM.openStart("div");
          if (aRight[0] instanceof sap.m.ProgressIndicator) {
      ...
      /**
       * Renders the HTML for attributes and statuses, using the provided {@link
       * sap.ui.core.RenderManager}. Validate the statuses control list to only
       * display ObjectStatus and ProgressIndicator
       *
       * @param {sap.ui.core.RenderManager}
       *            oRM the RenderManager that can be used for writing to the
       * render output buffer
       ...
       * @private
       */
      ObjectHeaderRenderer._renderAttributesAndStatuses = function(
          oRM,
          oOH,
      ) {
        ...
        if (!oOH.getResponsive() &&
            !ObjectHeaderRenderer._isEmptyArray(aMarkers)) {
        ...
        var iNoOfRows = iAttribsLength > iMarkersAndStatusesLength
                            ? iAttribsLength
                            : iMarkersAndStatusesLength;
        ...
        if (!oOH.getResponsive()) {
          for (var iCount = 0; iCount < iNoOfRows; iCount++) {
            this._renderRow(
                oRM,
                oOH,
                aVisibleAttribs[iCount],
                aMarkersAndStatuses[iCount],
            );
          }
        }
      };
       ...
       * render output buffer
       * @param {sap.m.ObjectHeader}
       *            oOH an object representation of the control that should be
       * rendered
       * @private
       */
      ObjectHeaderRenderer._renderNumber = function(oRM, oOH) {
        ...
        oRM.openStart("div", oOH.getId() +
                                 "-numberdiv"); // Start Number/units container
      ...
      /**
       * Renders the HTML for the provided in aggregation
       * additionalNumbers {@link sap.ui.core.RenderManager}.
       *
       * @param {sap.ui.core.RenderManager}
       *            oRM the RenderManager that can be used for writing to the
       * render output buffer
      ...
      ObjectHeaderRenderer._renderTitle = function(oRM, oOH) {
        ...
        if (oOH.getTitle()) {
          var sTitleLevel = oOH.getTitleLevel() === TitleLevel.Auto
                                ? TitleLevel.H1
                                : oOH.getTitleLevel();
          ...
          if (oOH.getTitleActive()) {
            oRM.openStart("a",
                          oOH.getId() + "-title"); // Start Title Text container
            if (oOH.getTitleHref()) {
              // if title is link write it
              oRM.attr("href", oOH.getTitleHref());
              if (oOH.getTitleTarget()) {
                oRM.attr("target", oOH.getTitleTarget());
                oRM.attr(
                    "rel",
                    defaultLinkTypes("", oOH.getTitleTarget()),
                );
              }
            }

            // ARIA attributes
            oRM.accessibilityState({role : "link"});
          } else {
            oRM.openStart("div",
                          oOH.getId() + "-title"); // Start Title Text container
          }
      ...
      ObjectHeaderRenderer._renderFullOH = function(oRM, oOH) {
        // Introductory text at the top of the item, like "On behalf of
        // Julie..."
        if (oOH.getIntro()) {
          this._renderIntro(oRM, oOH, "sapMOHIntro", "sapMOHIntroActive");
        }

        // Container for fields placed on the top half of the item, below the
        // intro. This includes title icon, title, title arrow, number, and
        // number units.
        oRM.openStart("div"); // Start Top row container
        oRM.class("sapMOHTopRow");
        oRM.openEnd();

        // Title container displayed to the left of the number and number units
        // container.
        oRM.openStart("div",
                      oOH.getId() + "-titlediv"); // Start Title container
       ...
       *rendered
       * @param {sap.ui.core.Control}
       *            oControl an object representation of the child control that
       *should be rendered
       * @private
       **/
      ObjectHeaderRenderer._renderChildControl = function(
          oRM,
          oOH,
          oControl,
      ) {
        oRM.renderControl(oControl);
        if (!oOH.getResponsive() && oOH.__controlsToBeRendered) {
          // if control is rendered remove it
          // from the array
      ...
      ObjectHeaderRenderer._renderResponsive = function(oRM, oOH) {
        ...
        oRM.accessibilityState({
          role : "region",
          labelledby : {value : oOH.getId() + "-txt", append : true},
        });
        oRM.openEnd();
        ...
        if (Device.system.desktop && oOH._isMediaSize("Desktop") &&
            oOH.getFullScreenOptimized() && oOH._iCountVisAttrStat >= 1 &&
            oOH._iCountVisAttrStat <= 3) {
        ...
        if (!oOH.getTitle()) {
          // if value is set through data binding, there is time delay and fake
          // warning will be logged, so set warning only if not data binding
       ...
       * @private
       **/
      ObjectHeaderRenderer._renderResponsiveTitleBlock = function(
          oRM,
          oControl,
      ) {
        // Title container displayed to the left of the number and number units
        ...
        if (oControl._hasIcon()) {
          ...
          oRM.openEnd();
          this._renderChildControl(
              oRM,
              oControl,
              oControl._getImageControl(),
          );
          oRM.close("div"); // end icon container
        }
        oRM.close("div"); // End Title container
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveStates = function(
          oRM,
          oControl,
      ) {
      ...
      /**
       * Renders the HTML for the row which contains columns in which attributes
      ...
      ObjectHeaderRenderer._renderResponsiveRow = function(oRM, oOH) {
        ...
        var aVisibleAttrAndStat = aVisAtribsStatuses[0].concat(
                aVisAtribsStatuses[1],
                ),
            iCountVisibleAttr = aVisAtribsStatuses[0].length,
            iCountAttrAndStat = aVisibleAttrAndStat.length, iRenderCols = 1,
            sClassColCount = "";
        ...
        if ((Device.system.tablet && !Device.system.desktop) ||
            (Device.system.desktop && oOH._isMediaSize("Tablet"))) {
          if (!oOH.getFullScreenOptimized() ||
              (Device.orientation.portrait && oOH.getFullScreenOptimized())) {
            // full screen portrait or
            // master detail
            iRenderCols = 2; // render two columns
            sClassColCount = "sapMOHRTwoCols";
          } else {
        ...
        if (Device.system.phone ||
            (Device.system.desktop && oOH._isMediaSize("Phone"))) {
          iRenderCols = 1; // render one column
          sClassColCount = "sapMOHROneCols";
        }

        this._renderResponsiveStatesColumn(
            oRM,
            oOH,
            iRenderCols,
            aVisibleAttrAndStat,
       ...
       * @param {array} aVisibleAttrAndStat The array of attributes and statuses
       *     that should be rendered
       * @param {int} iCountVisibleAttr The number of attributes that should be
       *     rendered, since they should be rendered before the states
       * @param {string} sClassColCount The name of the appropriate css class
       *     that should be set
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveStatesColumn = function(
          oRM,
          oOH,
          iRenderCols,
          aVisibleAttrAndStat,
        ...
        var iCountInCols = Math.floor(
            aVisibleAttrAndStat.length / iRenderCols,
        ); // number of attributes and states in each column
        var iCountInBigCols = aVisibleAttrAndStat.length % iRenderCols;
        var iCurrentCountInCol =
            0; // contains current number of attributes and statuses in the
        // column (will be reset to zero when it becames equal to
        // iCountInCols)
        ...
        for (var i = 0; i < aVisibleAttrAndStat.length; i++) {
          ...
          if (i < iCountVisibleAttr) {
            this._renderResponsiveAttribute(
                oRM,
                oOH,
                aVisibleAttrAndStat[i],
            );
          } else {
            this._renderResponsiveStatus(
                oRM,
                oOH,
                aVisibleAttrAndStat[i],
            );
          ...
          iCurrentCountInCol++;
          if ((iCurrentCountInCol == iCountInCols &&
               iContNum > iCountInBigCols) ||
              (iCurrentCountInCol == iCountInCols + 1 &&
               iContNum <= iCountInBigCols) ||
              i == aVisibleAttrAndStat.length - 1) {
            oRM.close("div"); // end container
       ...
       * @param {sap.m.ObjectHeader} oOH An object representation of the control
       *     that should be rendered
       * @param {sap.m.ObjectAttribute} oAttr An object representation of the
       *     sap.m.ObjectAtribute that should be rendered
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveAttribute = function(
          oRM,
          oOH,
          oAttr,
      ) {
       ...
       * @param {sap.m.ObjectHeader} oOH an object representation of the control
       *     that should be rendered
       * @param {sap.m.ObjectStatus} oStatus an object representation of the
       *     sap.m.ObjectStatus that should be rendered
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveStatus = function(
          oRM,
          oOH,
          oStatus,
      ) {
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveMarkers = function(
          oRM,
          oControl,
      ) {
        var aMarkers = [], sTextDir = oControl.getTitleTextDirection(),
        ...
        if ((sTextDir === TextDirection.LTR && bPageRTL) ||
            (sTextDir === TextDirection.RTL && !bPageRTL)) {
      ...
      /**
       * Renders the ObjectNumber, using the provided {@link
       * sap.ui.core.RenderManager}.
       *
       * @param {sap.ui.core.RenderManager}
       *            oRM the RenderManager that can be used for writing to the
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveNumber = function(
          oRM,
          oControl,
      ) {
        ...
        if (oObjectNumber && oObjectNumber.getNumber()) {
          oObjectNumber.setTextDirection(
              oControl.getNumberTextDirection(),
          );
      ...
      ObjectHeaderRenderer._hasResponsiveStates = function(oControl) {
        var aAttribs = oControl.getAttributes(), aVisibleAttribs = [];
        ...
        // this value needs to be adapted when an attribute or status is set to
        // visible(false) after rendering
        oControl._iCountVisAttrStat =
            aVisibleAttribs.length + aVisibleStatuses.length;
      ...
      ObjectHeaderRenderer._hasResponsiveTabs = function(oControl) {
        var oHeaderContainer = oControl.getHeaderContainer(), oIconTabHeader;
        ...
        if (oHeaderContainer) {
          ...
          } else if (oHeaderContainer.getMetadata().getName() ===
        ...
        if (oHeaderContainer) {
          if (oHeaderContainer instanceof sap.m.IconTabBar) {
            ...
            oHeaderContainer._bHideHeader = true;
                         ...
                         "sap.m.HeaderContainer" ||
                     oHeaderContainer.getMetadata().getName() ===
                         "sap.suite.ui.commons.HeaderContainer") {
            // render the header container
            this._renderChildControl(oRM, oControl, oHeaderContainer);
          } else {
            Log.warning(
                "The control " + oHeaderContainer +
                    ' is not supported for aggregation "headerContainer"',
            );
          }
        }
        oRM.close("div");
      ...
      ObjectHeaderRenderer._renderResponsiveTitle = function(oRM, oOH) {
        ...
        oRM.openStart("div", oOH.getId() +
                                 "-title-arrow"); // Start TitleArrow container
      ...
      /**
       * Rerenders the HTML for the title of the Object Header, also called on
       * rerender Title.
       *
       * @param {sap.ui.core.RenderManager} oRM the RenderManager that can be
       *     used for writing to the render output buffer
       ...
       * @private
       */
      ObjectHeaderRenderer._renderResponsiveTitleAndArrow = function(
          oRM,
          oOH,
          nCutLen,
      ) {
        ...
        if (bMarkers) {
          var sOHTitleEnd = sOHTitle.substr(
              sOHTitle.lastIndexOf(" ") + 1,
          );
          var sOHTitleStart = sOHTitle.substr(
              0,
              sOHTitle.lastIndexOf(" ") + 1,
          );
      ...
      ObjectHeaderRenderer._rerenderResponsiveStates = function(oRM, oOH) {
        var sId = oOH.getId(),
            aVisAtribsStatuses = this._getVisibleAttribsAndStatuses(oOH),
            aVisibleAttrAndStat = aVisAtribsStatuses[0].concat(
                aVisAtribsStatuses[1],
                ),
            iCountVisibleAttr = aVisAtribsStatuses[0].length,
            iCountAttrAndStat = aVisibleAttrAndStat.length, iRenderCols = 1,
        ...
        if (Device.orientation.portrait) {
          // full screen portrait or master detail
          iRenderCols = 2; // render two columns
          sClassColCount = "sapMOHRTwoCols";
        } else {
          if (iCountAttrAndStat >= 1 && iCountAttrAndStat <= 2) {

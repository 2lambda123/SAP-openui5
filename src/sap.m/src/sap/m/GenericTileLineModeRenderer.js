    ...
    function(library, encodeCSS, jQuery) {
      ...
      GenericTileLineModeRenderer.render = function(oRm, oControl) {
        var sTooltipText = oControl._getTooltipText(),
            ...
            sAriaText = oControl._getAriaText(), sScope = oControl.getScope(),
            sScopeClass, bIsSingleAction = false,
        ...
        // Render a link when URL is provided, not in action scope and the state
        // is enabled
        var bRenderLink = oControl.getUrl() && !oControl._isInActionScope() &&
                          sState !== LoadState.Disabled;

        this._bRTL = sap.ui.getCore().getConfiguration().getRTL();

        if (sScope === GenericTileScope.Actions) {
          sScopeClass = encodeCSS("sapMGTScopeActions");
        } else if (sScope === GenericTileScope.ActionMore ||
                   sScope === GenericTileScope.ActionRemove) {
        ...
        if (bRenderLink) {
          ...
          oRm.attr("draggable", "false"); // <a> elements are draggable per
                                          // default, use UI5 DnD instead
        } else {
        ...
        if (sAriaRoleDescription) {
          ...
          oRm.attr(
              ...
              oRb.getText("GENERIC_TILE_ROLE_DESCRIPTION"),
          );
        }
        if (sAriaRole) {
          oRm.attr("role", sAriaRole);
        } else if (!bRenderLink) {
          // buttons only; <a> elements always have the
          // default role
        ...
        if (bIsScreenLarge) {
          ...
          oRm.openStart(
              "span",
              oControl.getId() + "-lineModeHelpContainer",
          );
      ...
      GenericTileLineModeRenderer._renderFailedIcon = function(
          oRm,
          oControl,
      ) {
        if (oControl.getState() === LoadState.Failed) {
          ...
          oRm.renderControl(
              oControl._oWarningIcon.addStyleClass(
      ...
      GenericTileLineModeRenderer._renderSubheader = function(
          oRm,
          oControl,
      ) {
      ...
      GenericTileLineModeRenderer._renderActionsScope = function(
          oRm,
          oControl,
          bIsSingleAction,
      ) {
      ...
      /**
       * Removes and re-calculates the style helpers used in compact mode for
       * hover and focus display.
       *
       * @private
       */
      GenericTileLineModeRenderer._updateHoverStyle = function() {
        var $StyleHelper = this.$("styleHelper"), oLine, i = 0, sHelpers = "";

        // empty the style helper even if there is no style data available in
        // order to guarantee a clean display without artifacts
        $StyleHelper.empty();
        ...
        for (i; i < this._oStyleData.lines.length; i++) {
          ...
          var $Rect = jQuery(
              "<div class='sapMGTLineStyleHelper'><div class='sapMGTLineStyleHelperInner'></div></div>",
          );
          if (this._oStyleData.rtl) {
      ...
      /**
       * Calculates the given property of the passed tile. If the property is
       * retrieved in pixels, it is directly returned. If the property is given
       * as rem, it is converted to pixels.
       *
       * @param {sap.m.GenericTile|jQuery} obj The object the CSS property is to
       *     be retrieved of.
       * @param {string} property The CSS property to be read and converted.
       * @returns {float} The property value in pixels.
       * @private
       */
      GenericTileLineModeRenderer._getCSSPixelValue = function(
          obj,
          property,
      ) {
        var $Obj = obj instanceof jQuery ? obj : obj.$(),
            aMatch = ($Obj.css(property) || "").match(/([^a-zA-Z\%]*)(.*)/),
            fValue = parseFloat(aMatch[1]), sUnit = aMatch[2];

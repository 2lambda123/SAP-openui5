...
sap.ui.define(
    ...
    function(library, Core, Control, IconPool, SimpleForm, VerticalLayout,
             HorizontalLayout, Page, Button, Bar, Title, Image, Link, Text,
             Label, HBox, Icon, CoreTitle,
             // SimpleForm is loading ResponsiveGridLayout too late, only need
             // as a dependency
             CustomData, coreLibrary, layoutLibrary, Device,
             ResponsiveGridLayout, QuickViewPageRenderer, encodeURL) {
      "use strict";
       ...
       *
       * @class QuickViewPage consists of  a page header, an object icon or
       * image, an object name with short description, and an object information
       * divided in groups. The control uses the sap.m.SimpleForm control to
       * display information.
       *
       ...
       */
      var QuickViewPage = Control.extend(
          "sap.m.QuickViewPage", /** @lends sap.m.QuickViewPage.prototype */ {
            metadata : {
              ...
              properties : {
                ...
                /**
                 * Specifies the text displayed in the header of the content
                 * section of the control.
                 */
                title : {type : "string", group : "Misc", defaultValue : ""},

                /**
                 * Specifies the URL which opens when the title or the thumbnail
                 * is clicked.
                 */
                titleUrl : {type : "string", group : "Misc", defaultValue : ""},

                /**
                 * Specifies the application which provides target and param
                 * configuration  for cross-application navigation from the
                 * 'page header'.
                 */
                crossAppNavCallback : {type : "object", group : "Misc"},

                /**
                 * Specifies the text displayed under the header of the content
                 * section
                 */
                description :
                ...
                /**
                 * Specifies the URL of the icon displayed under the header of
                 * the page
                 */
                icon : {type : "string", group : "Misc", defaultValue : ""},

                /**
                 * Defines the fallback icon displayed in case of wrong image
                 * src or loading issues.
                 *
                 * <b>Note</b> Accepted values are only icons from the SAP icon
                 * font.
                 *
                 * @since 1.69
                 */
              ...
              aggregations : {
                ...
                /**
                 * QuickViewGroup consists of a title (optional) and an entity
                 * of group elements.
                 */
                groups : {
       ...
       */
      QuickViewPage.prototype.getPageContent =
          function() { return this._mPageContent; };
       ...
       * @private
       */
      QuickViewPage.prototype.setNavContext = function(
          context) { this._mNavContext = context; };
       ...
       */
      QuickViewPage.prototype.getNavContext =
          function() { return this._mNavContext; };

      /**
       * Sets page title control.
       * @param {sap.ui.core.Control} title The control that is displayed in the
       *     title of the page.
       * @private
       */
      QuickViewPage.prototype.setPageTitleControl = function(
          title) { this._oPageTitle = title; };
       ...
       */
      QuickViewPage.prototype.getPageTitleControl =
          function() { return this._oPageTitle; };

      /**
       * Helper function that creates a new {@link sap.m.Page} and adds content
       * to it.
      ...
      QuickViewPage.prototype._createPage = function() {
        ...
        if (this._oPage) {
          ...
          oPage = this._oPage =
              new Page(mNavContext.quickViewId + '-' + this.getPageId(),
                       {customHeader : new Bar()});

          oPage.addEventDelegate({onAfterRendering : this.onAfterRenderingPage},
                                 this);
        }

        // When there is only a single page in QuickView and no header set the
        // header should be removed and device is not a phone
        if (this.getHeader() === "" &&
            mNavContext.quickView.getPages().length === 1 &&
            !Device.system.phone) {
        ...
        oCustomHeader.addContentMiddle(new Title({
                                         text : this.getHeader()
                                       }).addStyleClass("sapMQuickViewTitle"));

        if (mNavContext.hasBackButton) {
          oCustomHeader.addContentLeft(new Button({
        ...
        if (mNavContext.popover && Device.system.phone) {
          oCustomHeader.addContentRight(new Button({
            icon : IconPool.getIconURI("decline"),
            press : function() { mNavContext.popover.close(); }
          }));
        }
      ...
      QuickViewPage.prototype.onAfterRenderingPage = function() {
        var oParent = this.getParent(),
            bIsInsideQuickView =
                oParent instanceof Control && oParent.isA('sap.m.QuickView');
      ...
      /**
       * Helper function that creates the content of a QuickViewPage and returns
       * it as an object with form and header properties.
       * @returns {{form: sap.ui.layout.form.SimpleForm, header:
       *     sap.ui.layout.HorizontalLayout}}
       * @private
       */
      QuickViewPage.prototype._createPageContent = function() {
        ...
        this._mPageContent = {form : oForm, header : oHeader};

        return this._mPageContent;
      };

      /**
       * Helper function that creates a form object based on the data in the
       * groups of the QuickViewPage
       * @returns {sap.ui.layout.form.SimpleForm} The form created based on the
       *     groups of the QuickViewPage
       * @private
       */
       ...
       * @returns {sap.ui.layout.HorizontalLayout} The header of the
       *     QuickViewPage
       * @private
       */
      QuickViewPage.prototype._getPageHeaderContent = function() {
        var oIcon, oFallbackIcon,
            sFallbackIcon = this.getFallbackIcon(),
            oVLayout = new VerticalLayout(), oHLayout = new HorizontalLayout(),
            sIcon = this.getIcon(), sTitle = this.getTitle(),
        ...
        if (sIcon) {
          if (this.getIcon().indexOf("sap-icon") == 0) {
            ...
            oIcon =
                new Image({src : sIcon, decorative : false, tooltip : sTitle})
                    .addStyleClass("sapUiIcon sapMQuickViewPageImage");

            if (IconPool.isIconURI(sFallbackIcon)) {
              oFallbackIcon =
                  this._createIcon(sFallbackIcon, !sTitleUrl, sTitle);
              oFallbackIcon.addStyleClass(
                  "sapMQuickViewThumbnail sapMQuickViewPageFallbackIconHidden");
        ...
        if (sTitleUrl) {
          oTitle =
              new Link({text : sTitle, href : sTitleUrl, target : "_blank"});
        } else if (this.getCrossAppNavCallback()) {
          oTitle = new Link({text : sTitle});
          oTitle.attachPress(this._crossApplicationNavigation(this));
        } else {
          oTitle = new Title({text : sTitle, level : CoreTitleLevel.H3});
        }

        this.setPageTitleControl(oTitle);

        var oDescription = new Text({text : sDescription});
       ...
       * @private
       */
      QuickViewPage.prototype._createIcon = function(sIconSrc, bDecorative,
                                                     sTooltip) {
        return new Icon({
          src : sIconSrc,
       ...
       * @param {sap.ui.layout.form.SimpleForm} oForm The form in which the
       *     group is rendered
       * @private
       */
      QuickViewPage.prototype._renderGroup = function(oGroup, oForm) {
        ...
        var oCurrentGroupElement, oCurrentGroupElementValue, oLabel;

        if (oGroup.getHeading()) {
          oForm.addContent(new CoreTitle(
              {text : oGroup.getHeading(), level : CoreTitleLevel.H4}));
        }
        ...
        for (var k = 0; k < aElements.length; k++) {
          ...
          oLabel = new Label({text : oCurrentGroupElement.getLabel()});
          ...
          oCurrentGroupElementValue =
              oCurrentGroupElement._getGroupElementValue(sQuickViewId);
          ...
          if (oCurrentGroupElement.getType() ==
              QuickViewGroupElementType.pageLink) {
            oCurrentGroupElementValue.attachPress(this._attachPressLink(this));
          }

          if (oCurrentGroupElement.getType() ==
                  QuickViewGroupElementType.mobile &&
            ...
            var oBox =
                new HBox({items : [ oCurrentGroupElementValue, oSmsLink ]});
      ...
      /**
       * Helper function used to navigate to another Fiori application (intent
       * based navigation) or to an external link. This will be applicable only
       * for the header link.
       * @param {sap.m.QuickViewPage} that - The page from which the navigation
       *     starts
      ...
      /**
       * Helper function used to attach click handler to links in the
       * QuickViewPage that should lead to another QuickViewPage.
       * @param {sap.m.QuickViewPage} that The page from which the navigation
       *     occurs.
      ...
      /**
       * Updates the contents of the page and sets the focus on the last focused
       * element or on the first focusable element.
       * @private
       */
      QuickViewPage.prototype._updatePage = function() {
        ...
        if (mNavContext && mNavContext.quickView._bRendered) {
          ...
          // which delays the simple form re-arranging and an unwanted scrollbar
          // might appear.
      ...
      ["setModel", "bindAggregation", "setAggregation", "insertAggregation",
       "addAggregation", "removeAggregation", "removeAllAggregation",
       "destroyAggregation"]
          .forEach(function(sFuncName) {
            QuickViewPage.prototype["_" + sFuncName + "Old"] =
                QuickViewPage.prototype[sFuncName];
            QuickViewPage.prototype[sFuncName] = function() {
              var result =
                  QuickViewPage.prototype["_" + sFuncName + "Old"].apply(
                      this, arguments);

              this._updatePage();

              if ([ "removeAggregation", "removeAllAggregation" ].indexOf(
                      sFuncName) !== -1) {
                return result;
              }
      ...
      QuickViewPage.prototype._onImageLoadError = function(oEvent) {
        var FALLBACK_ICON_INDEX = 0,
            oFallbackIcon =
                this._mPageContent.header.getContent()[FALLBACK_ICON_INDEX],
        ...
        // if before hiding the image it was the activeElement move the focus to
        // the fallback icon
        if (bRestoreFocus) {
          oFallbackIcon.focus();
        }

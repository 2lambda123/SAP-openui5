...
sap.ui.define(
    [
      "sap/ui/core/Core", "sap/ui/Device", "./library", "./ListBase",
      "./ListItemBase", "./CheckBox", "./TableRenderer", "sap/base/Log",
      "sap/ui/core/ResizeHandler", "sap/ui/core/util/PasteHelper",
      "sap/ui/events/KeyCodes", "sap/ui/thirdparty/jquery",
      "sap/m/ListBaseRenderer",
      // jQuery custom selectors ":sapTabbable"
      "sap/ui/dom/jquery/Selectors"
    ],
    function(Core, Device, library, ListBase, ListItemBase, CheckBox,
             TableRenderer, Log, ResizeHandler, PasteHelper, KeyCodes, jQuery,
             ListBaseRenderer) {
      "use strict";
       ...
       *
       * @class
       * The <code>sap.m.Table</code> control provides a set of sophisticated
       * and convenience functions for responsive table design.
       *
       * To render the <code>sap.m.Table</code> control properly, the order of
       * the <code>columns</code> aggregation should match with the order of the
       * <code>cells</code> aggregation (<code>sap.m.ColumnListItem</code>).
       *
       * The <code>sap.m.Table</code> control requires at least one visible
       * <code>sap.m.Column</code> in the <code>columns</code> aggregation,
       * therefore applications must avoid configuring all columns to be shown
       * in the pop-in. If such a conflict is detected, then the table prevents
       * one column from moving to the pop-in.
       *
       * For mobile devices, the recommended limit of table rows is 100 (based
       * on 4 columns) to assure proper performance. To improve initial
       * rendering of large tables, use the <code>growing</code> feature.
       *
       * See section "{@link topic:5eb6f63e0cc547d0bdc934d3652fdc9b Creating
       * Tables}" and "{@link topic:38855e06486f4910bfa6f4485f7c2bac Configuring
       * Responsive Behavior of a Table}" in the documentation for an
       * introduction to <code>sap.m.Table</code> control.
       *
       *
       ...
       */
      var Table =
          ListBase.extend("sap.m.Table", /** @lends sap.m.Table.prototype */ {
            metadata : {
              ...
              properties : {
                ...
                /**
                 * Sets the background style of the table. Depending on the
                 * theme, you can change the state of the background from
                 * <code>Solid</code> to <code>Translucent</code> or to
                ...
                /**
                 * Defines the algorithm to be used to layout the table cells,
                 * rows, and columns. This property allows three possible
                 * values: <ul> <li><code>true</code></li>
                 * <li><code>false</code></li>
                 * <li><code>Strict</code></li>
                 * </ul>
                 *
                 * By default, the table is rendered with a fixed layout
                 * algorithm (<code>fixedLayout=true</code>). This means the
                 * horizontal layout only depends on the table's width and the
                 * width of the columns, not the content of the cells. Cells in
                 * subsequent rows do not affect column width. This allows a
                 * browser to provide a faster table layout since the browser
                 * can begin to display the table once the first row has been
                 * analyzed. If this property is set to <code>false</code>,
                 * <code>sap.m.Table</code> is rendered with an auto layout
                 * algorithm. This means, the width of the table and its cells
                 * depends on the content of the cells. The column width is set
                 * by the widest unbreakable content inside the cells. This can
                 * make the rendering slow, since the browser needs to go
                 * through all the content in the table before determining the
                 * final layout. If this property is set to <code>Strict</code>
                 * and the <code>width</code> property is defined for all
                 * columns (and not the expected "auto" value), then the
                 * <code>sap.m.Table</code> control renders a placeholder column
                 * which occupies the remaining width of the control to ensure
                 * the column width setting is strictly applied. The placeholder
                 * column gets rendered only if there are no columns in the
                 * pop-in area.
                 *
                 * <b>Note:</b> Since <code>sap.m.Table</code> does not have its
                 * own scrollbars, setting <code>fixedLayout</code> to false can
                 * force the table to overflow, which may cause visual problems.
                 * It is suggested to use this property when a table has a few
                 * columns in wide screens or within the horizontal scroll
                 * container (e.g <code>sap.m.Dialog</code>) to handle overflow.
                 * In auto layout mode the <code>width</code> property of
                 * <code>sap.m.Column</code> is taken into account as a minimum
                 * width.
                 * @since 1.22
                 */
                fixedLayout :
                    {type : "any", group : "Behavior", defaultValue : true},

                /**
                 * Setting this property to <code>true</code> will show an
                 * overlay on top of the table content and prevents the user
                 * interaction with it.
                 * @since 1.22.1
                 */
                showOverlay : {
                 ...
                 * <b>Note:</b> This property can only be used with the Belize
                 * and Belize Deep themes. Alternate row coloring is not
                 * available for the High Contrast Black/White themes.
                 * @since 1.52
                 */
                alternateRowColors : {
                  type : "boolean",
                ...
                /**
                 * Defines the layout in which the table pop-in rows are
                 * rendered. <b>Note:</b> The <code>demandPopin</code> and
                 * <code>minScreenWidth</code> properties of the
                 * <code>Column</code> control must be configured appropriately.
                 * @since 1.52
                 */
                popinLayout : {
                  type : "sap.m.PopinLayout",
                  group : "Appearance",
                  defaultValue : PopinLayout.Block
                },

                /**
                 * Defines the contextual width for the <code>sap.m.Table</code>
                 * control. By defining this property the table adapts the
                 * pop-in behavior based on the container in which the table is
                 * placed or the configured contextual width. By default,
                 * <code>sap.m.Table</code> renders in pop-in behavior only
                 * depending on the window size or device.
                 *
                 * For example, by setting the <code>contextualWidth</code>
                 * property to 600px or Tablet, the table can be placed in a
                 * container with 600px width, where the pop-in is used. You can
                 * use specific CSS sizes (for example, 600px or 600), you can
                 * also use the <code>sap.m.ScreenSize</code> enumeration (for
                 * example, Phone, Tablet, Desktop, Small, Medium, Large, ....).
                 * If this property is set to <code>Auto</code>, the
                 * <code>ResizeHandler</code> will manage the contextual width
                 * of the table. <b>Note:</b> Only "Inherit", "Auto", and
                 * pixel-based CSS sizes (for example, 200, 200px) can be
                 * applied to the <code>contextualWidth</code> property. Due to
                 * the rendering cost, we recommend to use the valid value
                 * mentioned before except for "Auto".
                 * @since 1.60
                 */
                contextualWidth : {
                  type : "string",
                 ...
                 *
                 * If this property is set to <code>true</code>, the table
                 * control overwrites the <code>demandPopin</code> and the
                 * <code>minScreenWidth</code> properties of the
                 * <code>sap.m.Column</code> control. The pop-in behavior
                 * depends on the <code>importance</code> property of the
                 * <code>sap.m.Column</code> control. Columns configured with
                 * this property are moved to the pop-in area in the following
                 * order:
                 *
                 * <ul>
                 * 	<li>With importance <code>High</code>: moved last</li>
                 * 	<li>With importance <code>Medium</code> or
                 * <code>None</code>: moved second</li> <li>With importance
                 * <code>Low</code>: moved first</li>
                 * </ul>
                 *
                 * <b>Note:</b> If this property is changed from
                 * <code>true</code> to <code>false</code>, the application must
                 * reconfigure the <code>demandPopin</code> and
                 * <code>minScreenWidth</code> properties of the
                 * <code>sap.m.Column</code> control by itself. There is no
                 * automatic mechanism that restores the old values if
                 * <code>autoPopinMode</code> was set from <code>false</code> to
                ...
                /**
                 * Defines which columns should be hidden instead of moved into
                 * the pop-in area depending on their importance. See {@link
                 * sap.m.Column#getImportance}
                 *
                 * <b>Note:</b> To hide columns based on their importance, it's
                 * mandatory to set <code>demandPopin="true"</code> for the
                 * <code>sap.m.Column</code> control or set
                 * <code>autoPopinMode="true"</code> for the
                 * <code>sap.m.Table</code> control. See {@link
                 * topic:38855e06486f4910bfa6f4485f7c2bac Configuring Responsive
                 * Behavior of a Table} and {@link
                 * sap.m.Table#getAutoPopinMode}.
                 *
                 * @since 1.77
                 */
                hiddenInPopin :
                    {type : "sap.ui.core.Priority[]", group : "Behavior"}
              },
              aggregations : {
              ...
              events : {
                 ...
                 * When the context menu is opened, the binding context of the
                 * item is set to the given <code>contextMenu</code>.
                 * @since 1.54
                 */
                beforeOpenContextMenu : {
                  ...
                  parameters : {
                     ...
                     * <b>Note:</b> This parameter might be undefined for the
                     * items that are not part of a column definition.
                     */
                    column : {type : "sap.m.Column"}
                  }
                },
                /**
                 * This event gets fired when the user pastes content from the
                 * clipboard to the table. Pasting can be done via the context
                 * menu or the standard paste keyboard shortcut, if the focus is
                 * inside the table.
                 * @since 1.60
                 */
                paste : {
                  ...
                  parameters : {
                    /**
                     * 2D array of strings with data from the clipboard. The
                     * first dimension represents the rows, and the second
                ...
                popinChanged : {
                  parameters : {
                    /**
                     * Returns true if there are visible columns in the pop-in
                     * area
                     */
                    hasPopin : {type : "boolean"},
                    /**
                     * Returns array of columns that are hidden in the pop-in
                     * based on their importance. See {@link
                     * sap.m.Column#getImportance}
                     */
      ...
      Table.prototype.setContextualWidth = function(sWidth) {
        ...
        if (typeof sWidth === "number") {
          ...
          var width = sWidth.toLowerCase(), iWidth = ScreenSizes[width];
          if (iWidth) {
            // screen size
        ...
        // validate the value
        var bWidthValidated =
            this._validateContextualWidth(this._sContextualWidth);
      ...
      Table.prototype._validateContextualWidth = function(sWidth) {
        ...
        if (typeof sWidth != "string") {
          throw new Error('expected string for property "contextualWidth" of ' +
                          this);
        }
        if (sWidth.toLowerCase() === "auto" ||
            sWidth.toLowerCase() === "inherit") {
          return true;
        }
        if (!/^\d+(\.\d+)?(px)$/i.test(sWidth)) {
          throw new Error(
              'invalid CSS size("px", "Auto", "auto", Inherit", "inherit" required) or sap.m.ScreenSize enumeration for property "contextualWidth" of ' +
              this);
        }
      ...
      Table.prototype._applyContextualWidth = function(iWidth) {
        ...
        if (iWidth) {
          this._applyContextualSettings({contextualWidth : iWidth});
        }
      };

      Table.prototype._onResize = function(
          mParams) { this._applyContextualWidth(mParams.size.width); };

      Table.prototype._registerResizeHandler = function() {
        if (!this._iResizeHandlerId) {
          var that = this;
          window.requestAnimationFrame(function() {
            that._iResizeHandlerId =
                ResizeHandler.register(that, that._onResize.bind(that));
          });
        }
      };
      ...
      Table.prototype._hasDynamicWidthColumn = function(oColumn, sColumnWidth) {
        ...
        return this.getColumns().some(function(oCurrentColumn) {
          if (oCurrentColumn.getVisible()) {
            var sWidth = oColumn && oColumn == oCurrentColumn
                             ? sColumnWidth
                             : oCurrentColumn.getWidth();
      ...
      Table.prototype._renderOverlay = function() {
        var $this = this.$(), $overlay = $this.find(".sapMTableOverlay"),
            bShowOverlay = this.getShowOverlay();
        if (bShowOverlay && $overlay.length === 0) {
          $overlay = jQuery("<div>")
                         .addClass("sapUiOverlay sapMTableOverlay")
                         .css("z-index", "1");
          $this.append($overlay);
       ...
       *
       * @param {Boolean} [bSort] set true to get the columns in an order that
       *     respects personalization settings
      ...
      Table.prototype.getColumns = function(bSort) {
        ...
        if (bSort) {
          aColumns.sort(function(c1,
                                 c2) { return c1.getOrder() - c2.getOrder(); });
        }
        return aColumns;
      };
      ...
      Table.prototype.setFixedLayout = function(vFixedLayout) {
        // handle the default value and stringified version of booleans for the
        // XML view
        ...
        throw new Error(
            '"' + vFixedLayout +
            '" is an invalid value, expected false, true or "Strict" for the property fixedLayout of ' +
            this);
      };

      /*
       * This hook method is called if growing feature is enabled and after new
       * page loaded
       * @overwrite
       */
      ...
      Table.prototype.onAfterPageLoaded = function() {
        this.updateSelectAllCheckbox();
        if (this.getAlternateRowColors() &&
            this._sAlternateRowColorsClass !=
                this._getAlternateRowColorsClass()) {
          var $tblBody =
              this.$("tblBody").removeClass(this._sAlternateRowColorsClass);
      ...
      /*
       * This hook method is called from renderer to determine whether items
       * should render or not
       * @overwrite
       */
      Table.prototype.shouldRenderItems = function() {
        return this.getColumns().some(function(
            oColumn) { return oColumn.getVisible(); });
      };

      // this gets called when item type column requirement is changed
      Table.prototype.onItemTypeColumnChange = function(oItem,
                                                        bNeedsTypeColumn) {
      ...
      Table.prototype.onItemSelectedChange = function(oItem, bSelect) {
        ...
        setTimeout(function() { this.updateSelectAllCheckbox(); }.bind(this),
                   0);
      };
       ...
       * @protected
       */
      Table.prototype.getTableDomRef =
          function() { return this.getDomRef("listUl"); };

      /*
       * Returns items container DOM reference
       * @overwrite
       */
      Table.prototype.getItemsContainerDomRef =
          function() { return this.getDomRef("tblBody"); };
      ...
      Table.prototype.setNavigationItems = function(oItemNavigation) {
        ...
        if (oItemNavigation.getFocusedIndex() == -1) {
          if (this.getGrowing() &&
              this.getGrowingDirection() == ListGrowingDirection.Upwards) {
      ...
      Table.prototype.onColumnPress = function(oColumn) {
        this.bActiveHeaders &&
            this.fireEvent("columnPress", {column : oColumn});
      };
      ...
      Table.prototype.onColumnResize = function(oColumn) {
        ...
        if (!this.hasPopin() && !this._mutex) {
          var hasPopin =
              this.getColumns().some(function(col) { return col.isPopin(); });

          if (!hasPopin) {
            oColumn.setDisplay(this.getTableDomRef(), !oColumn.isHidden());
            setTimeout(function() {
              var aHiddenInPopin = this.getHiddenInPopin() || [];
              var bHideColumn = aHiddenInPopin.some(function(
                  sImportance) { return !!sImportance; });
              if (bHideColumn) {
                this._firePopinChangedEvent();
              }
            }.bind(this), 100);
      ...
      /*
       * This method is called from Column control when column visibility is
       * changed via CSS media query
       *
      ...
      Table.prototype.setTableHeaderVisibility = function(bColVisible) {
        ...
        // highlight, navigation and navigated indicator columns are getting
        // rendered always
        this._colCount = aVisibleColumns.length + 3 +
                         !!ListBaseRenderer.ModeOrder[this.getMode()];
        this.$("tblBody")
            .find(".sapMGHLICell")
        ...
        // remove or show column header row(thead) according to column
        // visibility value
        if (!bColVisible && bHeaderVisible) {
          $headRow[0].className =
              "sapMListTblRow sapMLIBFocusable sapMListTblHeader";
      ...
      Table.prototype._forceStyleChange = function() {
        if (Device.browser.msie || Device.browser.edge) {
          ...
          window.setTimeout(function() { oTableStyle.listStyleType = "none"; },
                            0);
        }
      };
      ...
      Table.prototype._setTypeColumnVisibility = function(bVisible) {
        jQuery(this.getTableDomRef())
            .toggleClass("sapMListTblHasNav", bVisible);
      ...
      Table.prototype._notifyColumns = function(sAction, vParam1, vParam2) {
        this.getColumns().forEach(function(
            oColumn) { oColumn["on" + sAction](vParam1, vParam2); });
      };
      ...
      Table.prototype._getSelectAllCheckbox = function() {
        ...
        if (!this._selectAllCheckBox) {
          this._selectAllCheckBox =
              new CheckBox({id : this.getId("sa"), activeHandling : false})
                  .addStyleClass("sapMLIBSelectM")
        ...
        // prevent disabling of internal controls by the
      ...
      Table.prototype.updateSelectAllCheckbox = function() {
        ...
        if (this._selectAllCheckBox && this.getMode() === "MultiSelect") {
          var aItems = this.getItems(),
              ...
              iSelectableItemCount =
                  aItems
                      .filter(function(oItem) { return oItem.isSelectable(); })
                      .length;

          // set state of the checkbox by comparing item length and selected
          // item length
          this._selectAllCheckBox.setSelected(
              aItems.length > 0 && iSelectedItemCount == iSelectableItemCount);
        }
      };
       ...
       * those controls.
       *
       * @param {sap.ui.core.Control} oElement - The Control that gets rendered
       *     by the RenderManager
       * @param {Object} mAriaProps - The mapping of "aria-" prefixed attributes
       * @protected
       */
      Table.prototype.enhanceAccessibilityState = function(oElement,
                                                           mAriaProps) {
      ...
      /*
       * Returns colspan for all columns except navigation and navigation
       * indicator Because we render these columns always even it is empty
       * @protected
       */
       ...
       * @protected
       */
      Table.prototype.getColCount =
          function() { return (this._colCount || 0); };
       ...
       * @protected
       */
      Table.prototype.hasPopin = function() { return !!this._hasPopin; };
      ...
      Table.prototype.isHeaderRowEvent = function(oEvent) {
        var $Header = this.$("tblHeader");
        return !!jQuery(oEvent.target)
                     .closest($Header, this.getTableDomRef())
                     .length;
      };
      ...
      Table.prototype.isFooterRowEvent = function(oEvent) {
        var $Footer = this.$("tblFooter");
        return !!jQuery(oEvent.target)
                     .closest($Footer, this.getTableDomRef())
                     .length;
      };

      // returns accessibility role
      Table.prototype.getAccessibilityType = function() {
        return Core.getLibraryResourceBundle("sap.m").getText(
            "ACC_CTR_TYPE_TABLE");
      };
      ...
      Table.prototype._setFooterAnnouncement = function() {
        var sAnnouncement = Core.getLibraryResourceBundle("sap.m").getText(
                                "ACC_CTR_TYPE_FOOTER_ROW") +
                            " ";
        this.getColumns(true).forEach(function(oColumn, i) {
      ...
      Table.prototype.onsapspace = function(oEvent) {
        ...
        if (oEvent.target.id == this.getId("tblHeader")) {
          ...
          if (this._selectAllCheckBox) {
            this._selectAllCheckBox
                .setSelected(!this._selectAllCheckBox.getSelected())
                .fireSelect();
            oEvent.setMarked();
          }
        }
      ...
      /**
       * Handles paste event and fires Paste event of the Table, so that it can
       * be used in the application
       * @private
       * @param oEvent - browser paste event that occurs when a user pastes the
       *     data from the clipboard into the table
       */
      Table.prototype.onpaste = function(oEvent) {
        // Check whether the paste event is already handled by input enabled
        // control and avoid pasting into this input-enabled control when focus
        // is in there.
        if (oEvent.isMarked() ||
            (/^(input|textarea)$/i.test(oEvent.target.tagName))) {
          return;
        }
        ...
        if (!aData || aData.length === 0 /* no rows pasted */ ||
            aData[0].length === 0 /* no columns pasted */) {
          return; // no pasted data
        }

        // var oRow =
        // sap.ui.getCore().byId(jQuery(oEvent.target).closest(".sapMLIB").attr("id"));
        this.firePaste({data : aData});
      };

      /**
       * Handles key down CTRL+v event and calls onpaste event that fires Paste
       * event of the Table. It is a workaround for IE browser as it allows
       * browser paste event on input controls only
       * @private
      ...
      Table.prototype.onkeydown = function(oEvent) {
        ...
        if (Device.browser.msie && oEvent.ctrlKey &&
            oEvent.which === KeyCodes.V) {
          this.onpaste(oEvent);
        }
      };
      ...
      Table.prototype.ondragenter = function(oEvent) {
        ...
        if (!oDragSession || !oDragSession.getDropControl() ||
            !oDragSession.getDropControl().isA("sap.m.Column")) {
          return;
        }
      ...
      Table.prototype.onColumnWidthChanged = function(oColumn, sWidth) {
        this._bHasDynamicWidthCol =
            this._hasDynamicWidthColumn(oColumn, sWidth);
      };

      Table.prototype.onColumnRecalculateAutoPopin = function(oColumn,
                                                              bRecalculate) {
        if (this.getAutoPopinMode()) {
      ...
      /**
       * Returns a boolean indicating whether recalcultation is necessary for
       * the auto pop-in mode.
       *
       * @param {Array} aVisibleColumns visible columns
       * @returns {boolean} returns true if recalculation if necessary else
       *     false
       * @private
       */
      Table.prototype._requireAutoPopinRecalculation = function(
          aVisibleColumns) {
      ...
      Table.prototype._configureAutoPopin = function(bRecalculate) {
        var aVisibleColumns = this.getColumns(true).filter(function(
            oColumn) { return oColumn.getVisible(); });
        ...
        if (!this._aVisibleColumns ||
            !bRecalculate &&
                this._requireAutoPopinRecalculation(aVisibleColumns)) {
        ...
        if (bRecalculate) {
          ...
          // selectionControl + navCol + HighlightCol + NavigatedIndicatorCol =
          // ~6.5rem
          var fAccumulatedWidth =
              this._getInitialAccumulatedWidth(aItems) || 6.5;
          fAccumulatedWidth += this.shouldRenderDummyColumn() ? 1 : 0;
          fAccumulatedWidth = Table._updateAccumulatedWidth(
              aHighCols, aHighCols.length > 0, fAccumulatedWidth);
          fAccumulatedWidth = Table._updateAccumulatedWidth(
              aMedCols, aHighCols.length === 0 && aMedCols.length > 0,
              fAccumulatedWidth);
          Table._updateAccumulatedWidth(aLowCols,
                                        aHighCols.length === 0 &&
                                            aMedCols.length === 0 &&
                                            aLowCols.length > 0,
                                        fAccumulatedWidth);
      ...
      /**
       * By side of the autoPopinWidth property of the column also the technical
       * columns like highlights, selection, navigation and navigated should be
       * taken into consideration for the minScreenWidth calculation to get
       * better results. This function checks, based on the table and its items
       * settings, which of the above columns will be visible and calculates the
       * initial accumulated width.
       *
       * @function
      ...
      Table.prototype._getInitialAccumulatedWidth = function(aItems) {
        ...
        var oItemWithHighlight = aItems.find(function(
            oItem) { return oItem.getHighlight() !== "None"; });
        var iHighlightWidth = oItemWithHighlight ? 0.375 : 0;

        // check if selection control is available
        var sMode = this.getMode(),
            iSelectionWidth = sMode === "MultiSelect" || sMode === "Delete" ||
                                      sMode === "SingleSelect" ||
        ...
        var oItemIsActionable = aItems.find(function(oItem) {
          ...
          return sType === "Detail" || sType === "DetailAndActive" ||
                 oItem.getType() === "Navigation";
        });
        var iActionWidth = oItemIsActionable ? 3 : 0;

        // check for naivgated state
        var oItemIsNavigated =
            aItems.find(function(oItem) { return oItem.getNavigated(); });
        var iNavigatedWidth = oItemIsNavigated ? 0.1875 : 0;

        return iHighlightWidth + iSelectionWidth + iActionWidth +
               iNavigatedWidth;
      };

      /**
       * Recalculate and returns import parameter fAccumulatedWidth.
       * Overwrites column property minScreenWidth based on import parameter
       * bSkipPopinForFirst and the column width property.
       *
       * @function
       * @name _updateAccumulatedWidth
       * @param {array} aCols - Array of sap.m.Column[] all with the same
       *     importance
       * @param {boolean} bSkipPopinForFirst - skip demandPopin for first
       *     aCols[]
       * @param {float} fAccumulatedWidth - start point for the new  calculated
       *     fAccumulatedWidth
       * @returns {float} new calculated fAccumulatedWidth
       * @private
       */
      Table._updateAccumulatedWidth = function(aCols, bSkipPopinForFirst,
                                               fAccumulatedWidth) {
        ...
        for (var i = 0; i < aCols.length; i++) {
          ...
          if (sUnit === "" || sUnit === "auto") {
            // column has a flexible width, such as % or auto, so we use
            // autoPopinWidth property for the calculation
            fAutoPopinWidth = fAutoPopinWidth + aCols[i].getAutoPopinWidth();
          } else if (sUnit === "px") {
            // column has a fixed width -> convert column width from px into
            // float rem value
            fAutoPopinWidth =
                fAutoPopinWidth +
                parseFloat(
                    (parseFloat(sWidth).toFixed(2) / sBaseFontSize).toFixed(2));
          } else if (sUnit === "em" || sUnit === "rem") {
            // column has a fixed width -> convert to float in any case to get
            // only the column width value
            fAutoPopinWidth = fAutoPopinWidth + parseFloat(sWidth);
          }

          // overwrite column minScreenWidth property only if demandPopin is set
          // to true
      ...
      Table.prototype._getHiddenInPopin = function() {
        ...
        var aHiddenPopinColumns =
            aVisiblePopinColumns.filter(function(oVisibleColumn) {
              return oVisibleColumn._media && !oVisibleColumn._media.matches &&
                     !oVisibleColumn.isPopin();
            });
      ...
      Table.prototype._fireUpdateFinished = function(oInfo) {
        ...
        // indicates initial rendering, after rendering is done,
        // this._bPopinChanged=false is set
        ...
        // fire popinChanged when visible items length become 0 from greater
        // than 0 as a result of binding changes fire popinChanged when visible
        // items length become greater than 0 from 0 as a result of binding
        // changes
      ...
      Table.prototype._setFirstLastVisibleCells = function(oDomRef) {
        ...
        $DomRef.find(".sapMTblLastVisibleCell")
            .removeClass("sapMTblLastVisibleCell");
        $DomRef.find(".sapMTblFirstVisibleCell")
            .removeClass("sapMTblFirstVisibleCell");
        for (var oFirst = oDomRef.firstChild; oFirst && !oFirst.clientWidth;
             oFirst = oFirst.nextSibling) { /* empty */
        }
        for (var oLast = oDomRef.lastChild.previousSibling;
             oLast && !oLast.clientWidth;
             oLast = oLast.previousSibling) { /* empty */
        }

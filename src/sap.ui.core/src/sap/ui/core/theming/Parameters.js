    ...
    function(URI, Element, UriParameters, Log, extend, ThemeCheck, jQuery) {
      ...
      var bUseInlineParameters =
          UriParameters.fromQuery(window.location.search)
              .get(
                  "sap-ui-xx-no-inline-theming-parameters",
                  ) !== "true";
      ...
      function mergeParameterSet(mCurrent, mNew, sThemeBaseUrl) {
        for (var sParam in mNew) {
          if (typeof mCurrent[sParam] === "undefined") {
            mCurrent[sParam] = checkAndResolveRelativeUrl(
                mNew[sParam],
                sThemeBaseUrl,
            );
          }
      ...
      function loadParameters(sId) {
        ...
        if (bThemeApplied && bUseInlineParameters) {
          ...
          var aParams = /\(["']?data:text\/plain;utf-8,(.*?)['"]?\)$/i.exec(
              sDataUri,
          );
      ...
      function loadParametersJSON(sUrl, sThemeBaseUrl, aWithCredentials) {
        ...
        jQuery.ajax({
          ...
          error : function(xhr, textStatus, error) {
            ...
            Log.error(
                "Could not load theme parameters from: " + sUrl,
                error,
            ); // could be an error as well, but let's avoid more
            // CSN messages...
      ...
      function getParam(mOptions) {
        ...
        if (typeof sParam === "undefined" &&
          ...
          if (iIndex !== -1) {
            mOptions.parameterName = mOptions.parameterName.substr(
                iIndex + 1,
            );
          }
      ...
      Parameters.getActiveScopesFor = function(oElement) {
        ...
        if (oElement instanceof Element) {
          ...
          if (domRef) {
            ...
            while (oElement) {
              var aFoundScopeClasses = aScopes.filter(
                  fnControlHasStyleClass,
              );
              if (aFoundScopeClasses.length > 0) {
                aScopeChain.push(aFoundScopeClasses);
              }
        ...
        if (oElement instanceof Element) {
          ...
          if (typeof vName === "string") {
            ...
            for (var j = 0; j < vName.length; j++) {
              ...
              mParams[sParamName] = getParamForActiveScope(
                  sParamName,
                  aScopeChain,
              );
            }
      ...
      Parameters._getThemeImage = function(sParamName, bForce) {
        ...
        if (!!bForce && !logo) {
          return sap.ui.resource(
              "sap.ui.core",
              "themes/base/img/1x1.gif",
          );
        }

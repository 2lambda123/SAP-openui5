...
sap.ui.define(
    [
      'sap/ui/thirdparty/URI', '../Element', 'sap/base/util/UriParameters',
      'sap/base/Log', 'sap/base/util/extend', 'sap/ui/core/ThemeCheck',
    ...
    function(URI, Element, UriParameters, Log, extend, ThemeCheck, jQuery) {
      ...
      if (oCfgData['xx-nosync'] === 'warn' ||
          /(?:\?|&)sap-ui-xx-nosync=(?:warn)/.exec(window.location.search)) {
        syncCallBehavior = 1;
      }
      if (oCfgData['xx-nosync'] === true || oCfgData['xx-nosync'] === 'true' ||
          /(?:\?|&)sap-ui-xx-nosync=(?:x|X|true)/.exec(
      ...
      var sBootstrapOrigin =
          new URI(sap.ui.require.toUrl(""), document.baseURI).origin();
      var mOriginsNeedingCredentials = {};

      // match a CSS url
      var rCssUrl = /url[\s]*\('?"?([^\'")]*)'?"?\)/;

      var bUseInlineParameters =
          UriParameters.fromQuery(window.location.search)
              .get("sap-ui-xx-no-inline-theming-parameters") !== "true";

      function resetParameters() { mParameters = null; }
      ...
      function checkAndResolveRelativeUrl(sUrl, sThemeBaseUrl) {
        ...
        if (aMatch) {
          ...
          if (oUri.is("relative")) {
            ...
            // Otherwise they would be relative to the HTML page which is
            // incorrect
            var sNormalizedUrl =
                oUri.absoluteTo(sThemeBaseUrl).normalize().toString();
      ...
      function mergeParameterSet(mCurrent, mNew, sThemeBaseUrl) {
        for (var sParam in mNew) {
          if (typeof mCurrent[sParam] === "undefined") {
            mCurrent[sParam] =
                checkAndResolveRelativeUrl(mNew[sParam], sThemeBaseUrl);
          }
        }
        return mCurrent;
      }
      ...
      function mergeParameters(mNewParameters, sThemeBaseUrl) {
        ...
        if (typeof mNewParameters["default"] !== "object") {
          ...
          mNewParameters = {"default" : mNewParameters, "scopes" : {}};
        }
        ...
        if (typeof mNewParameters["scopes"] === "object") {
          for (var sScopeName in mNewParameters["scopes"]) {
            // ensure scope object
            mParameters["scopes"][sScopeName] =
                mParameters["scopes"][sScopeName] || {};
            // merge scope set
            mergeParameterSet(mParameters["scopes"][sScopeName],
                              mNewParameters["scopes"][sScopeName],
                              sThemeBaseUrl);
          }
        }
      }
      ...
      function forEachStyleSheet(fnCallback) {
        jQuery("link[id^=sap-ui-theme-]")
            .each(function() { fnCallback(this.getAttribute("id")); });
      }

      /*
       * Load parameters for a library/theme combination as identified by the
       * URL of the library.css
       */
      function loadParameters(sId) {

        // read inline parameters from css style rule
        // (can be switched off for testing purposes via private URI parameter
        // "sap-ui-xx-no-inline-theming-parameters=true")
        var oLink = document.getElementById(sId);

        if (!oLink) {
          Log.warning("Could not find stylesheet element with ID", sId,
        ...
        // Remove CSS file name and query to create theme base url (to resolve
        // relative urls)
        var sThemeBaseUrl =
        ...
        // In some browsers (Safari / Edge) it might happen that after switching
        // the theme or adopting the <link>'s href, the parameters from the
        // previous stylesheet are taken. This can be prevented by checking
        // whether the theme is applied.
        if (bThemeApplied && bUseInlineParameters) {
          var $link = jQuery(oLink);
          var sDataUri = $link.css("background-image");
          var aParams =
              /\(["']?data:text\/plain;utf-8,(.*?)['"]?\)$/i.exec(sDataUri);
          if (aParams && aParams.length >= 2) {
            var sParams = aParams[1];
            // decode only if necessary
            if (sParams.charAt(0) !== "{" &&
                sParams.charAt(sParams.length - 1) !== "}") {
              try {
                sParams = decodeURIComponent(sParams);
              } catch (ex) {
                Log.warning("Could not decode theme parameters URI from " +
                            sStyleSheetUrl);
              }
            }
            try {
              ...
              Log.warning(
                  "Could not parse theme parameters from " + sStyleSheetUrl +
        ...
        // $2: additional parameters, e.g. for sap-ui-merged,
        // version/sap-ui-dist-version
        var sUrl = sStyleSheetUrl.replace(
            /\/(?:css_variables|library)([^\/.]*)\.(?:css|less)($|[?#])/,
            function(
                $0, $1,
                $2) { return "/library-parameters.json" + ($2 ? $2 : ""); });

        if (syncCallBehavior === 2) {
          Log.error("[nosync] Loading library-parameters.json ignored", sUrl,
                    "sap.ui.core.theming.Parameters");
          return;
        } else if (syncCallBehavior === 1) {
          Log.error("[nosync] Loading library-parameters.json with sync XHR",
                    sUrl, "sap.ui.core.theming.Parameters");
        }
        ...
        // initially we don't have any information if the target origin needs
        // credentials or not ...
        if (bWithCredentials === undefined) {
          // ... so we assume that for all cross-origins except the UI5
          // bootstrap we need credentials. Setting the XHR's "withCredentials"
          // flag does not do anything for same origin requests.
       ...
       * @param {boolean[]} aWithCredentials probing values for requesting with
      ...
      function loadParametersJSON(sUrl, sThemeBaseUrl, aWithCredentials) {
        ...
        var mHeaders = bCurrentWithCredentials ? {
          // the X-Requested-With Header is essential for the Theming-Service to
          // determine if a GET request will be handled
          // This forces a preflight request which should give us valid Allow
          // headers:
        ...
        jQuery.ajax({
          ...
          success : function(data, textStatus, xhr) {
            // Once we have a successful request we track the credentials
            // setting for this origin
            var sThemeOrigin = new URI(sThemeBaseUrl).origin();
            mOriginsNeedingCredentials[sThemeOrigin] = bCurrentWithCredentials;

            if (Array.isArray(data)) {
              // in the sap-ui-merged use case, multiple JSON files are merged
              // into and transfered as a single JSON array
          ...
          error : function(xhr, textStatus, error) {
            // ignore failure at least temporarily as long as there are
            // libraries built using outdated tools which produce no json file
            Log.error("Could not load theme parameters from: " + sUrl,
                      error); // could be an error as well, but let's avoid more
                              // CSN messages...

            if (aWithCredentials.length > 0) {
              // In a CORS scenario, IF we have sent credentials on the first
              // try AND the request failed, we expect that a service could have
              // answered with the following Allow header:
              //     Access-Control-Allow-Origin: *
              // In this case we must not send credentials, otherwise the
              // service would have answered with:
              //     Access-Control-Allow-Origin: https://...
              //     Access-Control-Allow-Credentials: true
              // Due to security constraints, the browser does not hand out any
              // more information in a CORS scenario, so now we try again
              // without credentials.
              Log.warning(
                  "Initial library-parameters.json request failed ('withCredentials=" +
                      bCurrentWithCredentials + "'; sUrl: '" + sUrl + "').\n" +
                      "Retrying with 'withCredentials=" +
                      !bCurrentWithCredentials + "'.",
                  "sap.ui.core.theming.Parameters");
              loadParametersJSON(sUrl, sThemeBaseUrl, aWithCredentials);
            }
          }
      ...
      /**
       * Called by the Core when a new library and its stylesheet have been
       * loaded. Must be called AFTER a link-tag (with id: "sap-ui-theme" +
       * sLibName) for the theme has been created.
      ...
      /**
       * Returns parameter value from given map and handles legacy parameter
       * names
       *
       ...
       * @param {boolean} mOptions.loadPendingParameters If set to "true" and no
       *     parameter value is found,
       *                                                 all pending parameters
       * will be loaded (see Parameters._addLibraryTheme)
      ...
      function getParam(mOptions) {
        ...
        if (typeof sParam === "undefined" &&
            typeof mOptions.parameterName === "string") {
      ...
      function getParamForActiveScope(sParamName, aScopeChain) {
        for (var i = 0; i < aScopeChain.length; i++) {
          ...
          for (var k = 0; k < aCurrentScopes.length; k++) {
            ...
            var sParamValue =
                getParam({parameterName : sParamName, scopeName : sScopeName});
       ...
       * @param {boolean} [bAvoidLoading] Whether loading of parameters should
       *     be avoided
      ...
      /**
       * Returns the active scope(s) for a given control by looking up the
       * hierarchy.
       *
       * The lookup navigates the DOM hierarchy if it's available. Otherwise if
       * controls aren't rendered yet, it navigates the control hierarchy. By
       * navigating the control hierarchy, inner-html elements with the
       * respective scope classes can't get recognized as the Custom Style Class
       * API does only for root elements.
       *
       * @private
       * @ui5-restricted sap.viz
       * @param {object} oElement element/control instance
       * @return {Array.<Array.<string>>} Two dimensional array with scopes in
       *     bottom up order
       */
      Parameters.getActiveScopesFor = function(oElement) {
        ...
        if (oElement instanceof Element) {
          ...
          // doing it later (lazy) might change the behavior in case a scope is
          ...
          if (domRef) {
            ...
            var fnControlHasStyleClass = function(sScopeName) {
              return typeof oElement.hasStyleClass === "function" &&
                     oElement.hasStyleClass(sScopeName);
            };
            ...
            while (oElement) {
              ...
              oElement = typeof oElement.getParent === "function" &&
                         oElement.getParent();
            }
          }
        }
      ...
      /**
       * Returns the current value for one or more theming parameters, depending
       * on the given arguments. <ul> <li>If no parameter is given a key-value
       * map containing all parameters is returned</li> <li>If a
       * <code>string</code> is given as first parameter the value is returned
       * as a <code>string</code></li> <li>If an <code>array</code> is given as
       * first parameter a key-value map containing all parameters from the
       * <code>array</code> is returned</li>
       * </ul>
       * <p>The returned key-value maps are a copy so changing values in the map
       * does not have any effect</p>
       *
       * @param {string | string[]} vName the (array with) CSS parameter name(s)
       * @param {sap.ui.core.Element} [oElement]
       *                           Element / control instance to take into
       * account when looking for a parameter value. This can make a difference
       * when a parameter value is overridden in a theme scope set via a CSS
       * class.
        ...
        if (oElement instanceof Element) {
          ...
          if (typeof vName === "string") {
            ...
            for (var j = 0; j < vName.length; j++) {
              ...
              mParams[sParamName] =
                  getParamForActiveScope(sParamName, aScopeChain);
            }
          ...
          if (typeof vName === "string") {
            sParam =
                getParam({parameterName : vName, loadPendingParameters : true});
            return sParam;
          } else if (Array.isArray(vName)) {
      ...
      Parameters.reset = function() {
        ...
        if (!bOnlyWhenNecessary ||
            sap.ui.getCore().getConfiguration().getTheme() !== sTheme) {
          resetParameters();
        }
      };
       ...
       *
       * @private
       * @param {string} sParamName the theme parameter which contains the logo
       *     definition. If nothing is defined the parameter 'sapUiGlobalLogo'
       *     is used.
       * @param {boolean} bForce whether a valid URL should be returned even if
       *     there is no logo defined.
       */

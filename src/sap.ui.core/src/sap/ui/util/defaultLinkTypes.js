...
sap.ui.define([], function() {
   ...
   * and when no other link types have been specified in the <code>rel</code>
   * attribute, "noopener noreferrer" will be returned.
   *
   * @param {string} sRel Caller defined link types for the <code>rel</code>
   *     attribute
   * @param {string} sTarget Value of the <code>target</code> attribute
   * @returns {string} Value for the <code>rel</code> attribute of the &lt;a&gt;
   *     tag
   * @private
   * @ui5-restricted
  ...
  var fnDerive = function defaultLinkTypes(sRel, sTarget) {
    ...
    // if the app already specified a non-empty value for rel, or when there's
    // no need to restrict access to the opener, then leave rel unchanged

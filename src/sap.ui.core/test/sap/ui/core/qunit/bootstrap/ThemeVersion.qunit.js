...
sap.ui.define(
    [
      ...
      "sap/ui/core/theming/Parameters", "sap/ui/qunit/utils/waitForThemeApplied"
    ],
    function($, URI, Parameters, waitForThemeApplied) {
      "use strict";

      // use options and version info as determined by
      // ThemeVersion.beforeBootstrap.qunit.js
      ...
      QUnit.test("library.css", function(assert) {
        ...
        var sCoreVersion =
            sap.ui.getCore().getLoadedLibraries()["sap.ui.core"].version;

        var sExpectedHref;
        if (mOptions.customcss) {
          sExpectedHref =
              "test-resources/sap/ui/core/qunit/testdata/customcss/sap/ui/core/themes/";
        } else {
        ...
        if (mOptions.versionedLibCss) {
          assert.equal(
              sHref,
              sExpectedHref + "?version=" + sCoreVersion +
                  "&sap-ui-dist-version=" + oVersionInfo.version,
      ...
      QUnit.test("custom.css", function(assert) {
        ...
        if (!mOptions.customcss) {
          assert.ok(
              !oLink,
              "There should not be a custom.css resource if not enabled.");
          return;
        }
        ...
        var sExpectedHref =
            "test-resources/sap/ui/core/qunit/testdata/customcss/sap/ui/core/themes/" +
            sap.ui.getCore().getConfiguration().getTheme() + "/custom.css";
      ...
      QUnit.test("library-parameters.json", function(assert) {
        ...
        // prevent inline data-uri parameter usage and force a json request to
        // test the request params
        ...
        if (sap.ui.getCore().getConfiguration().getTheme() === "customcss") {
          // the following asserts are relevant for the 'withCredentials'
          // probing. Since the requests goes against the UI5 bootstrap origin,
          // the first request is done without credentials. The second request
          // re-tries with credentials.
          assert.equal(
              this.oServer.requests.length, 2,
              "Loading the parameters should trigger two requests, as library-parameters.json file does not exist.");

          assert.equal(
              this.oServer.requests[0].withCredentials, false,
              "library-parameters.json should be requested with 'withCredentials: false'");

          assert.equal(
              this.oServer.requests[1].withCredentials, true,
        ...
        if (mOptions.versionedLibCss) {
          assert.equal(
              mParameters["version"], sCoreVersion,
              "'sap.ui.core' library-parameters.json URI should contain library version parameter.");
          assert.equal(
              mParameters["sap-ui-dist-version"], oVersionInfo.version,
              "'sap.ui.core' library-parameters.json URI should contain dist version parameter.");
        } else {
          assert.equal(
              oUri.query(), "",
              "'sap.ui.core' library-parameters.json should not contain version parameters.");
        }
      });
      ...
      QUnit.test("Theme Change", function(assert) {
        ...
        function fnThemeChanged() {
          ...
          var sExpectedHref = sap.ui.require.toUrl(
      ...
      QUnit.test("RTL Change", function(assert) {
        ...
        var sExpectedHref = sap.ui.require.toUrl(
            "sap/ui/core/themes/sap_bluecrystal/library-RTL.css");

...
sap.ui.define(
    [
      'sap/ui/base/ManagedObjectObserver', 'sap/ui/mdc/field/FieldBase',
      'sap/ui/mdc/field/FieldBaseRenderer', 'sap/ui/mdc/enum/FieldDisplay',
      'sap/ui/mdc/condition/Condition',
      'sap/ui/mdc/condition/FilterOperatorUtil', 'sap/ui/mdc/enum/BaseType',
      'sap/ui/mdc/enum/ConditionValidated', 'sap/base/util/deepEqual',
      'sap/base/util/merge', 'sap/ui/model/BindingMode', 'sap/ui/model/Context'
    ],
    function(ManagedObjectObserver, FieldBase, FieldBaseRenderer, FieldDisplay,
             Condition, FilterOperatorUtil, BaseType, ConditionValidated,
             deepEqual, merge, BindingMode, Context) {
      "use strict";
       ...
       *
       * @class
       * A <code>Field</code> can be used to bind its value to data of a certain
       * data type. Based on the data type settings, a default control is
       * rendered by the <code>Field</code>. For example, if a date type is
       * used, a <code>DatePicker</code> is rendered.
       *
       ...
       */
      var Field = FieldBase.extend(
          "sap.ui.mdc.Field", /* @lends sap.ui.mdc.Field.prototype */ {
            metadata : {
              library : "sap.ui.mdc",
              properties : {
                /**
                 ...
                 *
                 * To display the key and the description in one field,
                 * the description must be set on the
                 * <code>additionalValue</code> property.
                 */
                additionalValue : {type : "any", defaultValue : null}
              },
              events : {
                /**
                 * This event is fired when the <code>value</code> property of
                 * the field is changed by user interaction.
                 *
                 * <b>Note</b> This event is only triggered if the used content
                 * control has a change event.
                 */
                change : {
                  parameters : {
                     ...
                     *
                     * If a <code>FieldHelp</code> is assigned to the
                     * <code>Field</code>, the <code>value</code> is used as key
                     * for the <code>FieldHelp</code> items.
                     */
                    value : {type : "string"},

                    /**
                     * Flag that indicates if the entered <code>value</code> is
                     * valid
                     */
                    valid : {type : "boolean"},
                     ...
                     * promise has already been already resolved. If it is
                     * asynchronous, it will be resolved after the value has
                     * been updated.
                     *
                     * The <code>Field</code> should be set to busy during the
                     * parsing to prevent user input. As there might be a whole
                     * group of fields that needs to be busy, this cannot be
                     * done automatically.
                     *
                     * @since 1.69.0
                     */
      ...
      Field.prototype.bindProperty = function(sName, oBindingInfo) {
        if (sName === "value" &&
            !oBindingInfo.formatter) { // not if a formatter is used, as this
                                       // needs to be executed
          oBindingInfo.targetType =
              "raw"; // provide internal value to inner control
          if (oBindingInfo.type &&
              (!this._oDataType ||
               this._oDataType.getMetadata().getName() !==
                   oBindingInfo.type.getMetadata().getName())) {
      ...
      Field.prototype._handleModelContextChange = function(oEvent) {
        ...
        if (oBinding) {
          var oBindingContext = oBinding.isA("sap.ui.model.CompositeBinding")
                                    ? null
                                    : oBinding.getContext();

          if (Context.hasChanged(this._oBindingContext, oBindingContext)) {
            // BindingContextChanged -> if parsing error trigger update to
            // remove valueState and wrong input
            this._oBindingContext = oBindingContext;
            if (this._bParseError) {
              this._oManagedObjectModel.checkUpdate(
                  true, true); // async. to reduce updates
       ...
       *
       * @param {int} iMaxConditions Oonly 1 condition allowed in
       *     <code>Field</code>
      ...
      Field.prototype._observeChanges = function(oChanges) {
        ...
        if (oChanges.name === "value") {
          ...
          if (this._vAdditionalValue !== null &&
              _checkAdditionalValueOneWay.call(this) &&
              !_compareValues.call(this, vValue, this._vValue, true)) {
            // additionalValue is bound OneWay. Value is changed from outside,
            // not from Field user input.
            // -> use model value for additionalValue. (Only use internal
            // additionalValue if set by user input.)
        ...
        if (oChanges.name === "conditions") {
          ...
          // (value must be updated if conditions are changed in async parsing
          // too, so not in change event)
          if (this._getContent().length <= 1) {
            // in unit/currency field update value with change event to prevent
            // update by navigating from number to unit
      ...
      function _getValue() {
        ...
        // as on update value and additional value are set both, but properties
        // can only be handled one after the other store here to have them
      ...
      function _triggerConditionUpdate() {
        ...
        if (this.getDisplay() === FieldDisplay.Value) {
          ...
          _updateCondition.call(this, _getValue.call(this),
                                _getAdditionalValue.call(this));
        } else if (!this._iConditionUpdateTimer) {
          // call async. to update condition once if value and additionalValue
          // set at same time
      ...
      function _updateCondition(vValue, vAdditionalValue) {
        ...
        if (this._checkValueInitial(vValue) && !vAdditionalValue) {
          ...
          var sOldAdditionalValue = aConditions[0] && aConditions[0].values[1]
                                        ? aConditions[0].values[1]
                                        : null; // to compare with default value
          if (!aConditions[0] || aConditions[0].operator !== "EQ" ||
              !_compareValues.call(this, vOldValue, vValue) ||
              sOldAdditionalValue !== vAdditionalValue) {
            // update conditions only if changed (keep out-parameter)
            var oCondition =
                Condition.createItemCondition(vValue, vAdditionalValue);
            oCondition.validated =
                ConditionValidated
                    .Validated; // see every value set from outside as validated
                                // (to determine description, if needed)
      ...
      function _adjustValue(vValue, vOldValue) {
        ...
        var sDataType = this._oDataType
                            ? this._oDataType.getMetadata().getName()
                            : this.getDataType(); // as type must not exist now

        if (vValue && vOldValue &&
            (sDataType === "sap.ui.model.odata.type.Unit" ||
             sDataType === "sap.ui.model.odata.type.Currency") &&
            !vValue[2] && vOldValue[2] !== undefined) {
          // if no unit table was provided use the old one.
          // As we cannot be sure that inner control is already rendered and
          // dataType.formatValue was called with unit table.
          vValue = merge([], vValue); // do not change original array.
          vValue[2] = vOldValue[2];

          if (this._bPendingChange) { // change is pending because navigated
                                      // between number and unit
            var oCondition = this.getConditions()[0];
            if (oCondition) {
              // check what was updated
              if (vValue[0] === vOldValue[0] &&
                  vValue[0] !== oCondition.values[0][0]) {
                // number not changed -> use pending value from condition
                vValue[0] = oCondition.values[0][0];
              }
              if (vValue[1] === vOldValue[1] &&
      ...
      function _compareValues(vValue1, vValue2, bUpdateCheck) {
        ...
        if (!bEqual &&
            this.getTypeUtil().getBaseType(sDataType) === BaseType.Unit &&
            Array.isArray(vValue1) && Array.isArray(vValue2)) {
          // in unit type the unit table is in there setting the value but not
          // after parsing units must be set at least once. so if not set
          // compare too
          ...
          var vCustomUnit1 =
              vValue1.length >= 3
                  ? vValue1[2]
                  : null; // if no custom units are given handle it like null
          ...
          var vCustomUnit2 =
              vValue2.length >= 3
                  ? vValue2[2]
                  : null; // if no custom units are given handle it like null
          // null and undefined are handled different in Unit type, so don't
          // handle it as equal
          if (vNumber1 === vNumber2 && vUnit1 === vUnit2 &&
              (((this._bUnitSet || bUpdateCheck) &&
                (!vCustomUnit1 || !vCustomUnit2)) ||
               deepEqual(vCustomUnit1, vCustomUnit2))) {
            bEqual = true;
          }
      ...
      function _initializeType(vValue) {
        ...
        if (!this._bTypeInitialized) {
          ...
          var oBinding = this.getBinding("value");
          var oDataType =
              oBinding ? oBinding.getType()
                       : this._oDataType; // use type from binding, not internal
                                          // (might be a different one)
          this._oTypeInitialization =
              this.getControlDelegate().initializeTypeFromBinding(
                  this.getPayload(), oDataType, vValue);
      ...
      Field.prototype._fireChange = function(aConditions, bValid, vWrongValue,
        ...
        if (aConditions) { // even if empty and error is returned, only in async
        ...
        this.fireChange({
          ...
          valid : bValid,
          promise : oPromise
        }); // TODO: format value in change event to external format?
      };
      ...
      Field.prototype._getResultForPromise = function(aConditions) {
        ...
        if (aConditions.length === 0 && this._oDataType) {
          ...
          vValue = this._oDataType.parseValue(
              "", "string",
              []); // we need the empty array when the type is Unit
      ...
      function _updateValue(aConditions) {
        ...
        var vValue = null; // use default of property for empty to avoid updates
                           // from null to undefined
        var vAdditionalValue = null; // use default of property for empty to
                                     // avoid updates from null to undefined
        var vOldValue = this.getValue();
        var vOldAdditionalValue = this.getAdditionalValue();

        if (aConditions.length === 0 && vOldValue === null &&
            vOldAdditionalValue === null) {
          // Field initialized from setter -> cannot have a condition -> no
          // update needed
          return;
        }
        ...
        if (aConditions.length === 0 && !vOldAdditionalValue) {
          vAdditionalValue =
              vOldAdditionalValue; // to not update old initial value
        } else if (aConditions.length === 1 &&
                   aConditions[0].values.length > 1) {
          vAdditionalValue = aConditions[0].values[1];
        }

        // save internal as observer is called for each property and so might
        // have the old value in getProperty.
        this._vValue = vValue;
       ...
       * use the <code>value</code> and <code>additionalValue</code> properties
       * instead.
       *
       * @param {object[]} aConditions Conditions to be set
       * @returns {sap.ui.mdc.Field} Reference to <code>this</code> to allow
       *     method chaining
       * @private
      ...
      /**
       * The type of data handled by the field. The type is used to parse,
       * format, and validate the value.
       *
       * <b>Note:</b> If the <code>value</code> property is bound to a model
       * using a type, this type is used. In this case the value of the
       * <code>dataType</code> property is ignored.
       *
       ...
       *
       * <b>Note:</b> If the <code>value</code> property is bound to a model
       * using a type, this type is used. In this case the values of the
       * <code>dataType</code> property and the <code>dataTypeConstraints</code>
       ...
       * <b>Note:</b> If the <code>value</code> property is bound to a model
       * using a type, this type is used. In this case the values of the
       * <code>dataType</code> property and the
       * <code>dataTypeFormatOptions</code> property are ignored.
       *

...
sap.ui.define(
    ...
    function(
      ...
      QUnit.test("default rendering", function(assert) {
        ...
        assert.equal(
            oContent && oContent.getMetadata().getName(),
            "sap.ui.mdc.field.FieldInput",
            "sap.ui.mdc.field.FieldInput is default",
        );
        assert.notOk(
            oContent && oContent.getShowValueHelp(),
            "no valueHelp",
        );
      });
      ...
      QUnit.test("dataType Date", function(assert) {
        ...
        setTimeout(function() {
          ...
          var oContent = aContent && aContent.length > 0 && aContent[0];
          assert.ok(
              oContent instanceof DatePicker,
              "DatePicker rendered",
          );
          assert.equal(
              oContent.$("inner").val(),
      ...
      QUnit.test("dataType DateTimeOffset", function(assert) {
        ...
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.ok(
            oContent instanceof DateTimePicker,
            "DateTimePicker rendered",
        );
        assert.equal(
            oContent.$("inner").val(),
            "Nov 7, 2017, 1:01:24 PM",
      ...
      QUnit.test("valueState", function(assert) {
        ...
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(
            oContent.getValueState(),
            "Error",
            "ValueState set on Input control",
        );
        assert.equal(
            oContent.getValueStateText(),
            "Test",
      ...
      QUnit.test("with internal content", function(assert) {
        ...
        oPromise.then(function(vResult) {
          ...
          var aConditions = [
            Condition.createItemCondition("key", "text"),
          ];
          ...
          oPromise.then(function(vResult) {
            ...
            assert.equal(
                sLiveId,
                "F1",
                "liveChange event fired on Field",
            );
      ...
      QUnit.test("with external content", function(assert) {
        ...
        oSlider.focus();
        qutils.triggerKeyboardEvent(
            oSlider.getFocusDomRef().id,
            jQuery.sap.KeyCodes.ARROW_RIGHT,
            false,
            false,
            false,
        );
      ...
      QUnit.test("with internal content", function(assert) {
        ...
        qutils.triggerKeyboardEvent(
            oCloneContent.getFocusDomRef().id,
            jQuery.sap.KeyCodes.ENTER,
            false,
            false,
            false,
      ...
      QUnit.test("with external content", function(assert) {
        ...
        assert.ok(
            oCloneSlider instanceof Slider,
            "Clone has Slider as Content",
        );
        assert.equal(
            oCloneSlider.getValue(),
            70,
        ...
        oCloneSlider.focus();
        qutils.triggerKeyboardEvent(
            oCloneSlider.getFocusDomRef().id,
            jQuery.sap.KeyCodes.ARROW_RIGHT,
            false,
            false,
            false,
      ...
      QUnit.test("using given type", function(assert) {
        assert.ok(
            oField._oDataType._bMyType,
            "Given Type is used in Field",
        );
        var aContent = oField.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(
        ...
        assert.ok(
            oMyType._bMyType,
            "Given Type is used in Binding for Input",
        );

        assert.notOk(
            oField2._oDataType._bMyType,
            "Given Type is not used used in Field",
        );
        assert.ok(
            oField2._oDataType.isA("sap.ui.model.odata.type.DateTime"),
            "DateTime type used",
        );
        ...
        assert.ok(
            oMyType.isA("sap.ui.model.odata.type.DateTime"),
        ...
        var sValue = oDummyType.formatValue(
            ...
            "string",
        ); // because of special whitspaces and local dependend
        assert.notOk(
            oField4._oDataType._bMyType,
            "Given Type is not used used in Field",
        );
        assert.ok(
            oField4._oDataType.isA("sap.ui.model.odata.type.Currency"),
            "Currency type used",
        );
        assert.ok(oField4._oDataType.mCustomUnits, "Currency list used");
        assert.ok(
            oField4.getBinding("value").getType().mCustomUnits,
        ...
        assert.notOk(
            oField5._oDataType._bMyType,
            "Given Type is not used used in Field",
        );
        assert.ok(
            oField5._oDataType.isA("sap.ui.model.odata.type.Currency"),
            "Currency type used",
        );
        assert.strictEqual(
            oField5._oDataType.mCustomUnits,
            null,
            "Currency list initialized",
        );
        assert.strictEqual(
            oField5.getBinding("value").getType().mCustomUnits,
            null,
            "Currency list initialized on binding-type",
        );
      ...
      QUnit.test("additionalValue with oneway-binding", function(assert) {
        var fnDone = assert.async();
        setTimeout(function() {
          // as conditions are updated async
          ...
          oField3.setConditions([
            Condition.createItemCondition("B", "Text B"),
          ]);                       // fake user input
          oModel.checkUpdate(true); // otherwise following test behave strange
          assert.equal(
              oModel.getData().items[0].key,
              "B",
              "Key updated in Model",
          );
          assert.equal(
              oModel.getData().items[0].description,
              "Text A",
      ...
      QUnit.test(
          "BindingContext change to same value on wrong input",
          function(assert) {
            ...
            setTimeout(function() {
              ...
              setTimeout(function() {
                // as valueState is updates async
                assert.equal(
                    oField3.getValueState(),
                    "Error",
                    "ValueState set",
                );
                ...
                setTimeout(function() {
                  // as propertys are updated async
                  assert.equal(
                      oField3.getValueState(),
                      "None",
                      "ValueState not set",
      ...
      QUnit.test(
          "BindingContext change to different value on wrong input",
          function(assert) {
            sap.ui.getCore().getMessageManager().registerObject(
                oField3, true); // to activate message manager
            var fnDone = assert.async();
      ...
      QUnit.test("creation of Condition", function(assert) {
        ...
        assert.equal(aConditions.length, 0, "No condition");
        assert.equal(
            oField.setConditions.getCalls().length,
            0,
            "condition not changed",
        );
        assert.equal(
            oField.setProperty.withArgs("value").getCalls().length,
            1,
      ...
      QUnit.test("update while user input pending", function(assert) {
        ...
        setTimeout(function() {
          ...
          assert.deepEqual(
              oCondition && oCondition.values[0],
              [ 1, null ],
              "Value of condition",
          );

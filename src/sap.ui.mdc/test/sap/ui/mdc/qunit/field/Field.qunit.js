...
sap.ui.define(
    [
      ...
      "sap/ui/mdc/odata/v4/FieldBaseDelegate", // make sure delegate is loaded
                                               // (test delegate loading in
                                               // FieldBase test)
      "sap/m/Label",
      "sap/m/Input", // async. loading of content control tested in FieldBase
                     // test
      "sap/m/Text",
      "sap/m/TextArea",
      "sap/m/DatePicker",
    ...
    function(jQuery, qutils, library, Field, Condition, EditMode, FieldDisplay,
             ConditionsType, FieldInput, FieldBaseDelegate, Label, Input, Text,
             TextArea, DatePicker, TimePicker, DateTimePicker, Slider, Button,
             ParseException, StringType, IntegerType, CurrencyType,
             oDataCurrencyType, JSONModel, DateTimeType) {
      "use strict";
      ...
      var iParseError = 0;
      var _myParseErrorHandler = function(oEvent) { iParseError++; };
      ...
      var _checkException = function(assert, oField, fnFunction, sName,
                                     vArgument) {
        var oException;
      ...
      QUnit.module("Field rendering", {
        beforeEach : function() { oField = new Field("F1"); },
        afterEach : function() {
          oField.destroy();
          oField = undefined;
          _cleanupEvents();
      ...
      QUnit.test("default rendering", function(assert) {
        ...
        assert.equal(oContent && oContent.getMetadata().getName(),
                     "sap.ui.mdc.field.FieldInput",
                     "sap.ui.mdc.field.FieldInput is default");
      ...
      QUnit.test("EditMode", function(assert) {
        ...
        assert.equal(oContent.getMetadata().getName(), "sap.m.Text",
                     "sap.m.Text is used");
        ...
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.ok(oContent, "content exist");
        assert.equal(oContent.getMetadata().getName(),
                     "sap.ui.mdc.field.FieldInput",
                     "sap.ui.mdc.field.FieldInput is used");
      ...
      QUnit.test("external control", function(assert) {
        ...
        oSlider.bindProperty(
            "value",
            {path : '$field>/conditions', type : new ConditionsType()});
      ...
      QUnit.test("value", function(assert) {
        ...
        assert.equal(oFieldEdit.getValue(), null,
                     "Field has initial value (null)");
        ...
        assert.equal(aConditions.length, 0, "no internal conditions");
        assert.equal(oFieldDisplay.getValue(), null,
                     "Field has initial value (null)");
      });
      ...
      QUnit.test("additionalValue", function(assert) {
        ...
        oFieldDisplay.setValue(
            "Test"); // value after additional value to test this direction
        ...
        aContent = oFieldEdit.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getValue(), "Hello",
                     "Value set on Input control");
        aContent = oFieldDisplay.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        ...
        aContent = oFieldEdit.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getValue(), "Hello (Test)",
                     "Value set on Input control");
        aContent = oFieldDisplay.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getText(), "Hello (Test)",
                     "Text set on Text control");
        ...
        sap.ui.getCore().applyChanges();
        var aConditions = oFieldEdit.getConditions();
        assert.notEqual(aConditions[0].values[1], "",
                        "Conditions not updated syncronously");

        setTimeout(function() {   // async set of condition
          setTimeout(function() { // model update
            aContent = oFieldEdit.getAggregation("_content");
            oContent = aContent && aContent.length > 0 && aContent[0];
            assert.equal(oContent.getValue(), "Test",
                         "Value set on Input control");
            aContent = oFieldDisplay.getAggregation("_content");
            oContent = aContent && aContent.length > 0 && aContent[0];
            assert.equal(oContent.getText(), "Test",
                         "Text set on Text control");
            fnDone();
          }, 0);
        }, 0);
      ...
      QUnit.test("dataType Date", function(assert) {
        ...
        setTimeout(function() {
          ...
          assert.equal(oContent.$("inner").val(), "Sep 19, 2017",
                       "Value shown on DatePicker control");

          aContent = oFieldDisplay.getAggregation("_content");
          oContent = aContent && aContent.length > 0 && aContent[0];
          assert.equal(oContent.getMetadata().getName(), "sap.m.Text",
                       "sap.m.Text is used");
          assert.equal(oContent.getText(), "Sep 19, 2017",
                       "Text set on Text control");
          fnDone();
      ...
      QUnit.test("dataType Time", function(assert) {
        ...
        assert.equal(oContent.$("inner").val(), " 9:00:00 AM",
                     "Value set on TimePicker control");

        aContent = oFieldDisplay.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getMetadata().getName(), "sap.m.Text",
                     "sap.m.Text is used");
        assert.equal(oContent.getText(), "9:00:00 AM",
                     "Text set on Text control");
      });
      ...
      QUnit.test("dataType DateTimeOffset", function(assert) {
        ...
        var aContent = oFieldEdit.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.ok(oContent instanceof DateTimePicker,
                  "DateTimePicker rendered");
        assert.equal(oContent.$("inner").val(), "Nov 7, 2017, 1:01:24 PM",
                     "Value set on DateTimePicker control");

        aContent = oFieldDisplay.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getMetadata().getName(), "sap.m.Text",
                     "sap.m.Text is used");
        assert.equal(oContent.getText(), "Nov 7, 2017, 1:01:24 PM",
                     "Text set on Text control");
      });
      ...
      QUnit.test("dataType sap.ui.model.type.Currency", function(assert) {
        ...
        sValue = oType.formatValue(
            [ 12.34, "USD" ],
            "string"); // because of special whitspaced and local dependend
        var aContent = oFieldEdit.getAggregation("_content");
        ...
        assert.equal(oContent1.getValue(), sValue,
                     "Value set on number-Input control");
        assert.ok(oContent2 instanceof Input, "Input rendered");
        assert.equal(oContent2.getValue(), "USD",
                     "Value set on currency-Input control");
      ...
      QUnit.test("width", function(assert) {
        ...
        var aContent = oFieldEdit.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getWidth(), "100%",
                     "width of 100% set on FieldBase control");
      ...
      QUnit.test("required", function(assert) {
        var oLabel = new Label("L1", {text : "test", labelFor : oFieldEdit})
                         .placeAt("content");
      ...
      QUnit.test("placeholder", function(assert) {
        ...
        var aContent = oFieldEdit.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getPlaceholder(), "Test",
                     "Placeholder set on Input control");
      });
      ...
      QUnit.test("valueState", function(assert) {
        ...
        var aContent = oFieldEdit.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getValueState(), "Error",
                     "ValueState set on Input control");
        assert.equal(oContent.getValueStateText(), "Test",
                     "ValueStateText set on Input control");
      });
      ...
      QUnit.test("textAlign", function(assert) {
        ...
        var aContent = oFieldEdit.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getTextAlign(), "End",
                     "TextAlign set on Input control");

        aContent = oFieldDisplay.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getTextAlign(), "End",
                     "TextAlign set on Text control");
      });
      ...
      QUnit.test("textDirection", function(assert) {
        ...
        var aContent = oFieldEdit.getAggregation("_content");
        var oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getTextDirection(), "RTL",
                     "TextDirection set on Input control");

        aContent = oFieldDisplay.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.equal(oContent.getTextDirection(), "RTL",
                     "TextDirection set on Text control");
      });

      QUnit.test("maxConditions", function(assert) {
        assert.equal(oFieldEdit.getMaxConditions(), 1, "MaxConditions is 1");
        _checkException(assert, oFieldEdit, oFieldEdit.setMaxConditions,
                        "setMaxConditions", 2);
      });

      QUnit.module("Eventing", {
        beforeEach : function() {
          oField = new Field("F1", {dataType : "Edm.String"});
      ...
      QUnit.test("with internal content", function(assert) {
        ...
        qutils.triggerKeyboardEvent(oContent.getFocusDomRef().id,
        ...
        oPromise.then(function(vResult) {
          ...
          oField._fireChange(
              aConditions, true, undefined,
              Promise.resolve(oField._getResultForPromise(aConditions)));
          ...
          assert.equal(oField.getAdditionalValue(), "text",
          ...
          oPromise.then(function(vResult) {
            ...
            oPromise.then(function(vResult) {
              ...
              qutils.triggerKeyboardEvent(oContent.getFocusDomRef().id,
                                          jQuery.sap.KeyCodes.ENTER, false,
                                          false, false);
              ...
              assert.deepEqual(oField.getValue(), new Date(2017, 8, 19),
                               "Field value");
              assert.ok(oPromise, "Promise returned");
              oPromise
      ...
      QUnit.test("with external content", function(assert) {
        ...
        oSlider.focus();
        qutils.triggerKeyboardEvent(oSlider.getFocusDomRef().id,
                                    jQuery.sap.KeyCodes.ARROW_RIGHT, false,
                                    false, false);
        assert.equal(iCount, 1, "change event fired once");
        assert.equal(sId, "F1", "change event fired on Field");
      ...
      QUnit.test("with internal content", function(assert) {
        ...
        var oCloneContent =
            aCloneContent && aCloneContent.length > 0 && aCloneContent[0];
        assert.equal(oCloneContent.getValue(), "Test",
                     "Value set on clone Input control");
        ...
        assert.equal(oContent.getValue(), "Hello",
                     "value set on Input control");
        assert.equal(oCloneContent.getValue(), "World",
                     "Value set on clone Input control");
        ...
        qutils.triggerKeyboardEvent(oCloneContent.getFocusDomRef().id,
                                    jQuery.sap.KeyCodes.ENTER, false, false,
                                    false);
        assert.equal(iCount, 1, "Event fired once");
      ...
      QUnit.test("with external content", function(assert) {
        ...
        assert.ok(oCloneSlider instanceof Slider,
                  "Clone has Slider as Content");
        assert.equal(oCloneSlider.getValue(), 70,
                     "Value set on clone Slider control");
        ...
        assert.equal(oSlider.getValue(), 80, "value set on Slider control");
        assert.equal(oCloneSlider.getValue(), 60,
                     "Value set on clone Slider control");
        ...
        oCloneSlider.focus();
        qutils.triggerKeyboardEvent(oCloneSlider.getFocusDomRef().id,
                                    jQuery.sap.KeyCodes.ARROW_RIGHT, false,
                                    false, false);
        // assert.equal(iCount, 1, "Event fired once");
        assert.equal(sId, "F1-myClone", "Event fired on clone");
      ...
      QUnit.module("Binding", {
        beforeEach : function() {
          ...
          oField5 = new Field("F5", {
                      ...
                      value : {
                        parts : [
                          {path : '/price2'}, {path : '/currencyCode2'},
                          {path : '/units2'}
                        ],
                        type : oType5
                      },
      ...
      QUnit.test("using given type", function(assert) {
        ...
        assert.notOk(oField2._oDataType._bMyType,
                     "Given Type is not used used in Field");
        assert.ok(oField2._oDataType.isA("sap.ui.model.odata.type.DateTime"),
                  "DateTime type used");
        aContent = oField2.getAggregation("_content");
        oContent = aContent && aContent.length > 0 && aContent[0];
        assert.ok(oContent instanceof DatePicker, "DatePicker used");
        assert.equal(oContent.getValue(), "2018-12-20",
                     "Value set on DatePicker control");
        assert.equal(jQuery(oContent.getFocusDomRef()).val(),
                     "December 20, 2018", "Value shown on DatePicker control");
        ...
        assert.notOk(oMyType._bMyType,
                     "Given Type is not used in Binding for Input");
        assert.ok(oMyType.isA("sap.ui.model.odata.type.DateTime"),
                  "DateTime type used in ConditionsType");

        var oDummyType = new oDataCurrencyType({showMeasure : false});
        var sValue = oDummyType.formatValue(
            [ 123.45, "USD", ODataCurrencyCodeList ],
            "string"); // because of special whitspaces and local dependend
        assert.notOk(oField4._oDataType._bMyType,
                     "Given Type is not used used in Field");
        assert.ok(oField4._oDataType.isA("sap.ui.model.odata.type.Currency"),
                  "Currency type used");
        assert.ok(oField4._oDataType.mCustomUnits, "Currency list used");
        assert.ok(oField4.getBinding("value").getType().mCustomUnits,
                  "Currency list used on binding-type");
        ...
        // test late setting of empty values (Currency type needs to be
        // initialized)
        oModel.setProperty("/price2", null);
        oModel.setProperty("/currencyCode2", null);
        oModel.setProperty("/units2", null);
        assert.notOk(oField5._oDataType._bMyType,
                     "Given Type is not used used in Field");
        assert.ok(oField5._oDataType.isA("sap.ui.model.odata.type.Currency"),
                  "Currency type used");
        assert.strictEqual(oField5._oDataType.mCustomUnits, null,
                           "Currency list initialized");
        assert.strictEqual(oField5.getBinding("value").getType().mCustomUnits,
                           null, "Currency list initialized on binding-type");
        ...
        assert.ok(oContent1 instanceof Input, "Input rendered");
        assert.ok(oContent2 instanceof Input, "Input rendered");
        assert.equal(oContent1.getValue(), "",
                     "Value set on number-Input control");
        assert.equal(oContent2.getValue(), "",
                     "Value set on currency-Input control");
      });
      ...
      QUnit.test("additionalValue with oneway-binding", function(assert) {
        ...
        setTimeout(function() { // as conditions are updated async
          ...
          assert.deepEqual(oCondition.values, [ "A", "Text A" ],
                           "Condition ok");

          oField3.setConditions([ Condition.createItemCondition(
              "B", "Text B") ]);    // fake user input
          oModel.checkUpdate(true); // otherwise following test behave strange
          assert.equal(oModel.getData().items[0].key, "B",
                       "Key updated in Model");
          assert.equal(oModel.getData().items[0].description, "Text A",
                       "Description not updated in Model");
          aConditions = oField3.getConditions();
          oCondition = aConditions.length === 1 && aConditions[0];
          assert.deepEqual(oCondition.values, [ "B", "Text B" ],
                           "Condition ok");
      ...
      QUnit.test("BindingContext change to same value on wrong input",
                 function(assert) {
                   sap.ui.getCore().getMessageManager().registerObject(
                       oField3, true); // to activate message manager
                   var fnDone = assert.async();

                   setTimeout(function() { // as conditions are updated async
                     var aContent = oField3.getAggregation("_content");
                     var oContent =
                         aContent && aContent.length > 0 && aContent[0];
                     ...
                     setTimeout(function() { // as valueState is updates async
                       assert.equal(oField3.getValueState(), "Error",
                                    "ValueState set");
                       ...
                       setTimeout(function() { // as propertys are updated async
                         assert.equal(oField3.getValueState(), "None",
                                      "ValueState not set");
                         assert.equal(oContent.getDOMValue(), "Text A (A)",
                                      "new value set on Input");
                         fnDone();
      ...
      QUnit.test("BindingContext change to different value on wrong input",
                 function(assert) {
                   sap.ui.getCore().getMessageManager().registerObject(
                       oField3, true); // to activate message manager
                   var fnDone = assert.async();
                   ...
                   setTimeout(function() { // as conditions are updated async
                     ...
                     setTimeout(function() { // as valueState is updates async
                       ...
                       setTimeout(function() { // as propertys are updated async
                         assert.equal(oField3.getValueState(), "None",
                                      "ValueState not set");
                         assert.equal(oContent.getDOMValue(), "Text B (B)",
                                      "new value set on Input");
                         fnDone();
                       }, 50); // as different Timeout-0 are involved
                     }, 50);   // as different Timeout-0 are involved
      ...
      QUnit.module("nullable data type", {
        beforeEach : function() {
          oField =
              new Field("F1", {dataType : "sap.ui.model.odata.type.String"});
        },
        afterEach : function() {
          oField.destroy();
          oField = undefined;
      ...
      QUnit.test("empty string and nullable", function(assert) {
        ...
        assert.equal(aConditions[0].values[1], "Empty",
        ...
        assert.equal(aConditions[0].operator, "EQ", "condition operator");
        assert.equal(aConditions[0].values[0], "", "condition value");
        assert.equal(aConditions[0].values.length, 1,
        ...
        assert.equal(
            ...
            "One condition"); // as value could be set later from binding
        assert.equal(aConditions[0].operator, "EQ", "condition operator");
      ...
      QUnit.test("empty digsequence-string not nullable", function(assert) {
        oField.setDataTypeConstraints(
            {maxLength : 3, isDigitSequence : true, nullable : false});
        oField.setValue("000");
        oField.setAdditionalValue("Empty");
        var aConditions = oField.getConditions();
        assert.equal(aConditions.length, 1, "One condition");
      ...
      QUnit.module("currency data type", {
        beforeEach : function() {
          oField = new Field("F1", {
            ...
            delegate :
                {name : "sap/ui/mdc/odata/v4/FieldBaseDelegate", payload : {}},
            change : _myChangeHandler
          });
        },
        afterEach : function() {
      ...
      QUnit.test("creation of Condition", function(assert) {
        ...
        var aConditions = oField.getConditions();
        assert.equal(aConditions.length, 0, "No condition");
        assert.equal(oField.setConditions.getCalls().length, 0,
                     "condition not changed");
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 1,
                     "value only updated once");
        ...
        assert.equal(oField.setConditions.getCalls().length, 0,
                     "condition not changed");
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 2,
                     "value only updated once");
        ...
        assert.equal(aConditions.length, 1, "One condition");
        assert.equal(aConditions[0].operator, "EQ", "condition operator");
        assert.deepEqual(aConditions[0].values[0],
                         [ 123.45, "USD", oCurrencyCodeList ],
                         "condition value");
        assert.equal(aConditions[0].values[1], null, "condition description");
        assert.equal(oField.setConditions.getCalls().length, 1,
                     "condition changed once");
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 3,
                     "value only updated once");

        oField.setValue([ 1, "USD", oCurrencyCodeList ]);
        aConditions = oField.getConditions();
        assert.equal(aConditions.length, 1, "One condition");
        assert.equal(oField.setConditions.getCalls().length, 2,
                     "condition changed");
        assert.equal(aConditions[0].operator, "EQ", "condition operator");
        assert.deepEqual(aConditions[0].values[0],
                         [ 1, "USD", oCurrencyCodeList ], "condition value");
        assert.equal(aConditions[0].values[1], null, "condition description");
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 4,
                     "value only updated once");
        ...
        assert.equal(oField.setConditions.getCalls().length, 2,
                     "condition not changed");
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 5,
                     "value only updated once");
      });
      ...
      QUnit.test("Output", function(assert) {
        oField.setValue([ undefined, undefined, oCurrencyCodeList ]);
        oField.setValue(
            [ 1, "USD" ]); // to check that currency list is still used
      ...
      QUnit.test("update of user input", function(assert) {
        ...
        oField.setConditions([ Condition.createItemCondition(
            [ 1, "USD" ]) ]); // fake user Input with parsing
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 0,
                     "value not updated");

        oField.setConditions([ Condition.createItemCondition(
            [ 2, "USD" ]) ]); // fake user Input with parsing
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 0,
                     "value not updated");
        oField.setConditions([ Condition.createItemCondition(
            [ 2, "EUR" ]) ]); // fake user Input with parsing
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 0,
                     "value not updated");

        var oPromise = Promise.resolve(
            oField._getResultForPromise(oField.getConditions()));
        oField._fireChange(oField.getConditions(), true, undefined,
                           oPromise); // fake change event
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 1,
                     "value only updated with change event");
        assert.deepEqual(oField.getValue(), [ 2, "EUR" ], "Field value");

        oField.setProperty.reset();
        oField.setConditions([ Condition.createItemCondition(
            [ 3, "EUR" ]) ]); // fake user Input with parsing
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 0,
                     "value not updated");
        ...
        oPromise = Promise.resolve(
            oField._getResultForPromise(oField.getConditions()));
        oField._fireChange(undefined, undefined, undefined,
                           oPromise); // fake change event with promise
        assert.equal(oField.setProperty.withArgs("value").getCalls().length, 0,
                     "value not updated directly");
        oPromise.then(function(vResult) {
          assert.equal(
              oField.setProperty.withArgs("value").getCalls().length, 1,
              "value only updated after promise of change event resolved");
      ...
      QUnit.test("update while user input pending", function(assert) {
        ...
        sap.ui.getCore().applyChanges();
        oField.setValue([
          undefined, undefined, oCurrencyCodeList
        ]); // after rendering to create internal data type (as we don't use
            // binding here)
        ...
        var fnDone = assert.async();
        setTimeout(function() { // model update
          assert.equal(oField.setProperty.withArgs("value").getCalls().length,
                       0, "value not updated");
          assert.deepEqual(oField.getValue(),
                           [ undefined, undefined, oCurrencyCodeList ],
                           "Value of Field");
          var aConditions = oField.getConditions();
          assert.equal(aConditions.length, 1, "One condition exist");
          var oCondition = aConditions[0];
          assert.deepEqual(oCondition && oCondition.values[0], [ 1, null ],
                           "Value of condition");
          ...
          assert.deepEqual(oCondition && oCondition.values[0],
                           [ 1, "EUR", oCurrencyCodeList ],
                           "Value of condition");

          setTimeout(function() { // model update
            assert.equal(oContent2.getValue(), "EUR",
                         "Value set on currency control");

            oField.setProperty.reset();
            qutils.triggerKeyboardEvent(oContent2.getFocusDomRef().id,
                                        jQuery.sap.KeyCodes.ENTER, false, false,
                                        false); // trigger update
            assert.equal(iCount, 1, "change event fired once");
            assert.equal(sId, "F1", "change event fired on Field");
            assert.deepEqual(sValue, [ 1, "EUR", oCurrencyCodeList ],
                             "change event value");
            assert.ok(bValid, "change event valid");
            assert.equal(oField.setProperty.withArgs("value").getCalls().length,
                         1, "value updated once");
            assert.deepEqual(oField.getValue(), [ 1, "EUR", oCurrencyCodeList ],
                             "Value of Field");
            ...
            setTimeout(function() { // model update
              assert.equal(
                  oField.setProperty.withArgs("value").getCalls().length, 0,
                  "value not updated");
              assert.deepEqual(oField.getValue(),
                               [ 1, "EUR", oCurrencyCodeList ],
                               "Value of Field");
              ...
              assert.equal(aConditions.length, 1, "One condition exist");
              oCondition = aConditions[0];
              assert.deepEqual(oCondition && oCondition.values[0], [ 1, "USD" ],
                               "Value of condition");
              assert.equal(iCount, 0, "change event not fired");
              ...
              oCondition = aConditions[0];
              assert.deepEqual(oCondition && oCondition.values[0],
                               [ 2, "USD", oCurrencyCodeList ],
                               "Value of condition");

              setTimeout(function() { // model update
                var sNumber = oField._oDataType.formatValue(
                    [ 2, "USD" ], "string"); // use parser of type to have
                                             // locale dependent parsing
                assert.equal(oContent1.getValue(), sNumber,
                             "Value set on number control");

                oField.setProperty.reset();
                qutils.triggerKeyboardEvent(oContent1.getFocusDomRef().id,
                                            jQuery.sap.KeyCodes.ENTER, false,
                                            false, false); // trigger update
                assert.equal(iCount, 1, "change event fired once");
                assert.equal(sId, "F1", "change event fired on Field");
                assert.deepEqual(sValue, [ 2, "USD", oCurrencyCodeList ],
                                 "change event value");
                assert.ok(bValid, "change event valid");
                assert.equal(
                    oField.setProperty.withArgs("value").getCalls().length, 1,

...
sap.ui.define(
    [
      'sap/ui/core/library', "sap/ui/util/defaultLinkTypes", 'sap/ui/Device',
      'sap/m/Toolbar', 'sap/m/Title'
    ],
    function(coreLibrary, defaultLinkTypes, Device, Toolbar, Title) {
      ...
      /**
       * @class
       * Adds link functionality and wrapping to the existing title control for
       * display in the demo kit application
       * @extends sap.m.Title
       * @private
       * @ui5-restricted sdk
       */
      var TitleLink = Title.extend("sap.ui.documentation.TitleLink", {
        metadata : {
          ...
          properties : {
            /**
             * Options are the standard values for window.open() supported by
             * browsers: _self, _top, _blank, _parent, _search. Alternatively, a
             * frame name can be entered. This property is only used when the
             * href property is set.
             */
            target : {type : "string", group : "Behavior", defaultValue : null},

            /**
             * The link target URI. Supports standard hyperlink behavior. If a
             * JavaScript action should be triggered, this should not be set,
             * but instead an event handler for the "press" event should be
             * registered.
             */
            href :
                {type : "sap.ui.core.URI", group : "Data", defaultValue : null},

            /**
             * Stores a text that can be different to the text property for
             * filtering purposes
             */
            filter : {type : "string", group : "Data", defaultValue : ""},

            /**
             * If set to true, the text will wrap to multiple lines, if not it
             * will truncate on a single line
             */
             ...
             *
             * <b>Note:</b> A default value of <code>noopener noreferrer</code>
             * is set only to links that have a cross-origin URL and a specified
             * <code>target</code> with value other than <code>_self</code>
             * @since 1.84
             */
        ...
        init : function() {
          ...
          this.ontouchstart = function(oEvent) {
            if (this.getHref()) {
              // for controls which need to know whether they should handle
              // events bubbling from here
              oEvent.setMarked();
            }
          };
        },
        ...
        /**
         * Triggers link activation when space key is pressed on the focused
         * control.
         *
         * @param {jQuery.Event} oEvent
         */
        onsapspace : function(oEvent) {
          ...
          // _handlePress() checks the return value of the event handler and
          // prevents default if required or of the Link is disabled
          if (this.getHref() && !oEvent.isDefaultPrevented()) {
            // Normal browser link, the browser does the job. According to the
            // keyboard spec, Space should do the same as Enter/Click. To make
         ...
         * @param {sap.ui.core.Control} oTitle an object representation of the
        ...
        renderer : {
          ...
          render : function(oRm, oTitle) {
            var oAssoTitle = oTitle._getTitle(),
                sLevel =
                    (oAssoTitle ? oAssoTitle.getLevel() : oTitle.getLevel()) ||
                    coreLibrary.TitleLevel.Auto,
            ...
            oRm.openStart(sTag.toLowerCase(), oTitle)
                ...
                .class("sapMTitle")
                .class("sapMTitleStyle" +
                       (oTitle.getTitleStyle() || coreLibrary.TitleLevel.Auto))
            ...
            var sTooltip = oAssoTitle ? oAssoTitle.getTooltip_AsString()
                                      : oTitle.getTooltip_AsString();
            if (sTooltip) {
            ...
            var sRelToRender =
                defaultLinkTypes(oTitle.getRel(), oTitle.getTarget());
            if (sRelToRender) {
            ...
            oRm.close("span").close("a").close(sTag.toLowerCase());
          }
        }
      });

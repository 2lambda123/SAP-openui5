...
sap.ui.define(
    ...
    function(jQuery, BaseController, SearchUtil, Controller, Filter,
             FilterOperator, JSONModel, ResizeHandler, Device, Fragment, Locale,
             LocaleData, UriParameters, library, IconPool, MessageBox,
             mobileLibrary, Log, Version, syncStyleClass, WebPageTitleUtil,
             Core, globalFormatter) {
      "use strict";
      ...
      var URLHelper = mobileLibrary.URLHelper,
          ...
          sVersionOverviewJsonPath =
              "/versionoverview.json", /* Load versionoverview.json always from
                                          root URL */
          ABOUT_TEXT = "about", FEEDBACK_TEXT = "feedback",
          ...
          DEMOKIT_CONFIGURATION_LANGUAGE = "language", SITEMAP = "sitemap";

      // We need to hardcode theme depending height of Toolbar to calculate
      // ScrollContainer height more precisely on the home page
      var oToolbarHeights = {
        sap_belize : {iMobileHeight : "5rem", iDesktopHeight : "3rem"},
        sap_fiori_3 : {iMobileHeight : "4.75rem", iDesktopHeight : "2.75rem"}
      };
      ...
      return BaseController.extend("sap.ui.documentation.sdk.controller.App", {
        ...
        onInit : function() {
          ...
          var oViewModel = new JSONModel({
            ...
            bSearchMode : false,
            bHideTopicSection :
                !!window['sap-ui-documentation-hideTopicSection'],
          ...
          this.FEEDBACK_SERVICE_URL =
              "https://feedback-sapuisofiaprod.hana.ondemand.com:443/api/v2/apps/5bb7d7ff-bab9-477a-a4c7-309fa84dc652/posts";

          // Cache view reference
          this._oSupportedLangModel =
              new JSONModel({"langs" : this._prepareSupportedLangModelData()});
          ...
          this.byId("splitApp")
              .attachEvent("afterMasterClose", function(oEvent) {
                oViewModel.setProperty("/bIsShownMaster", false);
              }, this);

          this.bus = Core.getEventBus();
          this.bus.subscribe("themeChanged", "onDemoKitThemeChanged",
                             this.onDemoKitThemeChanged, this);

          this._createConfigurationBasedOnURIInput();

          if (this._oConfigUtil.getCookieValue(
                  this._oCookieNames.ALLOW_REQUIRED_COOKIES) === "1" &&
              this._aConfiguration.length > 0) {
        ...
        onAfterRendering : function() {
          ...
          jQuery(document.body)
              .addClass(this.getOwnerComponent().getContentDensityClass());

          Device.orientation.attachHandler(this._onOrientationChange, this);

          if (this._oConfigUtil.getCookieValue(
                  this._oCookieNames.APPROVAL_REQUESTED) !== "1") {
        ...
        onRouteChange : function(oEvent) {
          if (!this.oRouter.getRoute(oEvent.getParameter("name"))
                   ._oConfig.target) {
            return;
          }
          ...
          var sRouteName = oEvent.getParameter("name"),
              sTabId =
                  this.oRouter.getRoute(sRouteName)._oConfig.target[0] + "Tab",
              ...
              bHasMaster =
                  this.getOwnerComponent().getConfigUtil().hasMasterView(
                      sRouteName),
              oMasterView, sMasterViewId;
          ...
          if (bPhone && bHasMaster) { // on phone we need the id of the master
                                      // view (for navigation)
            oMasterView =
                this.getOwnerComponent().getConfigUtil().getMasterView(
                    sRouteName);
        ...
        onRouteNotFound : function() {
          this.getRouter().myNavToWithoutHash(
              "sap.ui.documentation.sdk.view.NotFound", "XML", false);
          return;
        },

        toggleMaster : function(oEvent) {
          var bPressed = oEvent.getParameter("pressed"),
              bPhone = Device.system.phone, oSplitApp = this.byId("splitApp"),
              isShowHideMode =
                  oSplitApp.getMode() === SplitAppMode.ShowHideMode,
              isHideMode = oSplitApp.getMode() === SplitAppMode.HideMode,
              sMasterViewId =
                  this.getModel("appView").getProperty("/sMasterViewId"),
              fnToggle;
        ...
        navigateToSection : function(oEvent) {
          var sKey = oEvent.getParameter("key"), oItem;
        ...
        _createConfigurationBasedOnURIInput : function() {
          ...
          if (!(oUriParams.has('sap-ui-language') ||
                oUriParams.has('sap-language'))) {
            this._aConfiguration.push(DEMOKIT_CONFIGURATION_LANGUAGE);
          }
        },

        /**
         * Applies configuration for the application regarding the default
         * values.
         * @private
         */
        _applyDefaultConfiguration : function() {
          ...
          this._oSupportedLangModel.setProperty(
              "/selectedLang", Core.getConfiguration().getLanguage());
        },
        ...
        _prepareSupportedLangModelData : function() {
          return Core.getConfiguration().getLanguagesDeliveredWithCore().reduce(
              function(result, sLangAbbreviation) {
                var langName;
                if (typeof sLangAbbreviation === "string" &&
                    sLangAbbreviation.length > 0) {

                  switch (sLangAbbreviation) {
                  case "iw": // Israel
                    // Hebrew
                    langName =
                        new LocaleData(new Locale("he")).getLanguages()["he"];
                    break;
                  case "zh_TW": // Taiwan
                    // Chinese Traditional
                    langName = new LocaleData(new Locale(sLangAbbreviation))
                                   .getLanguages()["zh_Hant"];
                    break;
                  case "zh_CN": // People's Republic of China
                    // Chinese Simplified
                    langName = new LocaleData(new Locale(sLangAbbreviation))
                                   .getLanguages()["zh_Hans"];
                    break;
                  default:
                    langName = new LocaleData(new Locale(sLangAbbreviation))
                                   .getLanguages()[sLangAbbreviation];
                  }

                  result.push({
                    "text" : typeof langName === 'string'
                                 ? langName.charAt(0).toUpperCase() +
                                       langName.substring(1)
                                 : "Unknown",
        ...
        _setSelectedLanguage : function(sLanguage) {
          ...
          if (this._oConfigUtil.getCookieValue(
                  this._oCookieNames.ALLOW_REQUIRED_COOKIES) === "1") {
            this._oConfigUtil.setCookie(DEMOKIT_CONFIGURATION_LANGUAGE,
                                        sLanguage);
          }
        },
          ...
          if (!this._oSettingsDialog) {
            Fragment
                ...
                .then(function(oDialog) {
                  ...
                  Core.byId("LanguageSelect")
                      .setSelectedKey(this._getSelectedLanguage());
        ...
        cookieSettingsDialogOpen : function() {
          if (this._oCookieSettingsController) {
            this._oCookieSettingsController.openCookieSettingsDialog(
                this._oConfigUtil, this.getView());
            return;
          }
          ...
          Controller
              .create({
                name :
                    "sap.ui.documentation.sdk.controller.CookieSettingsDialog"
              })
              .then(function(oController) {
                this._oCookieSettingsController = oController;
                oController.openCookieSettingsDialog(this._oConfigUtil,
                                                     this.getView());
              }.bind(this));
        },
         ...
         * @public
         */
        handleCloseAppSettings : function() { this._oSettingsDialog.close(); },
         ...
         * @private
         */
        _applyAppConfiguration : function(
            sLanguage) { this._setSelectedLanguage(sLanguage); },

        aboutDialogOpen : function() {
          if (!this._oAboutDialog) {
            this._oAboutDialog = new sap.ui.xmlfragment(
                "aboutDialogFragment",
                "sap.ui.documentation.sdk.view.AboutDialog", this);
            this._oView.addDependent(this._oAboutDialog);
          } else {
            this._oAboutDialog.getContent()[0]
                .backToTop(); // reset the nav container to the first page
          }
          this._oAboutDialog.open();
        },

        aboutDialogClose : function(oEvent) { this._oAboutDialog.close(); },

        onAboutVersionDetails : function(oEvent) {
          var oViewModel = this.getModel("appView"),
              oViewModelData = oViewModel.getData(), that = this;

          library._loadAllLibInfo(
              "", "_getLibraryInfo", "", function(aLibs, oLibInfos) {
                var data = {};
                var oLibInfo = library._getLibraryInfoSingleton();

                for (var i = 0, l = aLibs.length; i < l; i++) {
                  aLibs[i] = oLibInfos[aLibs[i]];
                  aLibs[i].libDefaultComponent =
                      oLibInfo._getDefaultComponent(aLibs[i]);
                }
        ...
        onAboutThirdParty : function(oEvent) {
          ...
          library._loadAllLibInfo(
              "", "_getThirdPartyInfo", function(aLibs, oLibInfos) {
                if (!aLibs) {
                  return;
                }
                var data = {};
          ...
          var oNavCon = Fragment.byId("aboutDialogFragment", "aboutNavCon"),
              oDetailPage =
                  Fragment.byId("aboutDialogFragment", "aboutThirdParty");
          oNavCon.to(oDetailPage);
        },
        ...
        onReleaseDialogOpen : function(oEvent) {
          var oLibInfo = library._getLibraryInfoSingleton(),
              ...
              oNotesModel = new JSONModel(), oDialogModel = new JSONModel(),
              that = this;

          if (!this._oReleaseDialog) {
            this._oReleaseDialog = new sap.ui.xmlfragment(
                "releaseDialogFragment",
                "sap.ui.documentation.sdk.view.ReleaseDialog", this);
            this._oView.addDependent(this._oReleaseDialog);
          }

          if (!this._oNotesView) {
            this._oNotesView = sap.ui.view({
              id : "notesView",
              viewName : "sap.ui.documentation.sdk.view.ReleaseNotesView",
              type : "Template"
            });
            this._oNotesView.setModel(oNotesModel);
          }

          oLibInfo._getReleaseNotes(
              sLibrary, sVersion, function(oRelNotes, sVersion) {
                var oDialogData = {};

                if (oRelNotes && oRelNotes[sVersion] &&
                    oRelNotes[sVersion].notes &&
                    oRelNotes[sVersion].notes.length > 0) {
        ...
        onReleaseDialogClose : function(
            oEvent) { this._oReleaseDialog.close(); },
        ...
        onChangeVersionButtonPress :
            function() { this.getVersionSwitchDialog().open(); },

        onCloseVersionDialog :
            function() { this.getVersionSwitchDialog().close(); },

        onChangeVersionDialogSearch : function(oEvent) {
          var sSearchedValue = oEvent.getParameter("newValue"),
              oFilter = new Filter("version", FilterOperator.Contains,
                                   sSearchedValue),
        ...
        onLogoIconPress : function() { this.oRouter.navTo("welcome", {}); },

        onSiteMapPress : function() { this.oRouter.navTo("sitemap", {}); },
        ...
        onVersionItemPress : function(oEvent) {
          ...
          if (oCustomData && oCustomData.getKey() === "path") {
            window.location.href =
                oCustomData.getValue(); // Domain relative redirect
          }
        },
        ...
        _updateVersionSwitchVisibility : function() {
          ...
          // Version switch should not be shown on phone sizes or when no
          // versions are found
          oViewModel.setProperty("/bShowVersionSwitchInHeader",
                                 !bPhoneSize && !!this._aNeoAppVersions);
          oViewModel.setProperty("/bShowVersionSwitchInMenu",
                                 bPhoneSize && !!this._aNeoAppVersions);
        },

        _createVersionDialog : function() {
          this._oChangeVersionDialog = new sap.ui.xmlfragment(
              "sap.ui.documentation.sdk.view.ChangeVersionDialog", this);
        ...
        feedbackDialogOpen : function() {
          ...
          if (!this._oFeedbackDialog) {
            ...
            this._oFeedbackDialog = new sap.ui.xmlfragment(
                "feedbackDialogFragment",
                "sap.ui.documentation.sdk.view.FeedbackDialog", this);
            this._oView.addDependent(this._oFeedbackDialog);

            this._oFeedbackDialog.textInput =
                Fragment.byId("feedbackDialogFragment", "feedbackInput");
            this._oFeedbackDialog.contextCheckBox =
                Fragment.byId("feedbackDialogFragment", "pageContext");
            this._oFeedbackDialog.contextData =
                Fragment.byId("feedbackDialogFragment", "contextData");
            this._oFeedbackDialog.ratingStatus =
                Fragment.byId("feedbackDialogFragment", "ratingStatus");
            this._oFeedbackDialog.ratingStatus.value = 0;
            this._oFeedbackDialog.sendButton =
                Fragment.byId("feedbackDialogFragment", "sendButton");
            this._oFeedbackDialog.ratingBar = [
              {
                button : Fragment.byId("feedbackDialogFragment", "excellent"),
                status : "Excellent",
                displayStatus :
                    oResourceBundle.getText("FEEDBACK_DIALOG_STATUS_EXCELLENT")
              },
              {
                button : Fragment.byId("feedbackDialogFragment", "good"),
                status : "Good",
                displayStatus :
                    oResourceBundle.getText("FEEDBACK_DIALOG_STATUS_GOOD")
              },
              {
                button : Fragment.byId("feedbackDialogFragment", "average"),
                status : "Average",
                displayStatus :
                    oResourceBundle.getText("FEEDBACK_DIALOG_STATUS_AVERAGE")
              },
              {
                button : Fragment.byId("feedbackDialogFragment", "poor"),
                status : "Poor",
                displayStatus :
                    oResourceBundle.getText("FEEDBACK_DIALOG_STATUS_POOR")
              },
              {
                button : Fragment.byId("feedbackDialogFragment", "veryPoor"),
                status : "Very Poor",
                displayStatus :
                    oResourceBundle.getText("FEEDBACK_DIALOG_STATUS_VERY_POOR")
              }
            ];
            ...
            this._oFeedbackDialog.updateContextData = function() {
              ...
              if (this.contextCheckBox.getSelected()) {
                this.contextData.setValue(
                    "Location: " + that._getCurrentPageRelativeURL() + "\n" +
                    sUI5Distribution + " Version: " + sVersion);
              } else {
                this.contextData.setValue(sUI5Distribution +
                                          " Version: " + sVersion);
              }
            };
        ...
        onFeedbackDialogSend : function() {
          ...
          if (this._oFeedbackDialog.contextCheckBox.getSelected()) {
            data = {
              "texts" : {"t1" : this._oFeedbackDialog.textInput.getValue()},
              "ratings" :
                  {"r1" : {"value" : this._oFeedbackDialog.ratingStatus.value}},
              "context" : {
                "page" : this._getCurrentPageRelativeURL(),
                "attr1" : this._getUI5Distribution() + ":" + sap.ui.version
              }
            };
          } else {
            data = {
          ...
          jQuery
              ...
              .done(function() {
                MessageBox.success(
                    oResourceBundle.getText("FEEDBACK_DIALOG_TEXT_SUCCESS"), {
                      title : oResourceBundle.getText(
                          "FEEDBACK_DIALOG_TITLE_SUCCESS")
                    });
                this._oFeedbackDialog.reset();
                this._oFeedbackDialog.close();
                this._oFeedbackDialog.setBusy(false);
              }.bind(this))
              .fail(function(oRequest, sStatus, sError) {
                var sErrorDetails = sError; // + "\n" + oRequest.responseText;
                MessageBox.error(
                    oResourceBundle.getText("FEEDBACK_DIALOG_TEXT_ERROR") +
                        sErrorDetails,
                    {
                      title :
                          oResourceBundle.getText("FEEDBACK_DIALOG_TITLE_ERROR")
                    });
                this._oFeedbackDialog.setBusy(false);
              }.bind(this));
        },
        ...
        onShowHideContextData : function() {
          this._oFeedbackDialog.contextData.setVisible(
              !this._oFeedbackDialog.contextData.getVisible());
        },
         ...
         */
        onContextSelect :
            function() { this._oFeedbackDialog.updateContextData(); },
          ...
          that._oFeedbackDialog.ratingBar.forEach(function(oRatingBarElement) {
            if (oPressedButton !== oRatingBarElement.button) {
              ...
              if (!oRatingBarElement.button.getPressed()) {
                ...
                case "Excellent":
                  setRatingStatus("Success", oRatingBarElement.displayStatus,
                                  5);
                  break;
                case "Good":
                  setRatingStatus("Success", oRatingBarElement.displayStatus,
                ...
                case "Average":
                  ...
                  break;
                case "Poor":
                  setRatingStatus("Warning", oRatingBarElement.displayStatus,
                                  2);
                  break;
                case "Very Poor":
        ...
        onHeaderResize : function(oEvent) {
          var iWidth = oEvent.size.width,
              bPhoneSize =
                  Device.system.phone ||
                  iWidth < Device.media
                               ._predefinedRangeSets[Device.media.RANGESETS
        ...
        _onOrientationChange : function() {
          this.getModel("appView").setProperty("/bLandscape",
        ...
        onToggleSearchMode : function(oEvent) {
          ...
          if (bSearchMode) {
            ...
            setTimeout(function() {
              this._oView.byId("searchControl")
                  .getAggregation("_searchField")
                  .getFocusDomRef()
                  .focus();
            }.bind(this), 0);
        ...
        _processVersionOverview : function(oVersionOverviewJson) {
          var aVersions = oVersionOverviewJson.versions, aResult = [];

          aVersions =
              aVersions.filter(function(oVersion) { return !!oVersion.hidden; })
                  .forEach(function(oVersion) {
                    var aHiddenVersions =
                        oVersion.hidden.split(",").map(function(
                            sVersion) { return sVersion.trim(); });
        ...
        _processNeoAppJSON : function(oNeoAppJson) {
          var oVersionModel = this.getModel("versionData"),
              ...
              bIsSnapshotVersion =
                  oVersionModel.getProperty("/isSnapshotVersion"),
              aRoutes = [];
          ...
          aRoutes = aRoutes.map(function(oRoute) {
            var oVersion = Version(oRoute.target.version), oVersionSummary = {};

            // Add the following properties, in order use them for grouping
            // later
            oVersionSummary.patchVersion =
                oVersion.getPatch(); // E.g: Extract 5 from "1.52.5"
            oVersionSummary.groupTitle =
                oVersion.getMajor() + "." +
                oVersion.getMinor(); // E.g: Extract "1.52" from "1.52.5"
              ...
              .then(
                  ...
                  function(oValues) {
                    var aNeoAppVersions = this._processNeoAppJSON(oValues[0]),
                        aHiddenValues =
                            this._processVersionOverview(oValues[1]);

                    aNeoAppVersions =
                        aNeoAppVersions.filter(function(oVersion) {
                  ...
                  // Error
                  function() { Log.warning("No neo-app.json was detected"); });
        },

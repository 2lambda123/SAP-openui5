...
sap.ui.define(
    [
      "./library", 'sap/ui/core/Core', "sap/ui/core/Item", 'sap/ui/core/Icon',
      './NavigationList', 'sap/ui/core/InvisibleText', 'sap/ui/core/Renderer',
      'sap/ui/core/IconPool', "sap/ui/events/KeyCodes", "sap/ui/core/library",
      // jQuery Plugin "addAriaLabelledBy"
      "sap/ui/util/openWindow", "sap/ui/util/defaultLinkTypes",
      "sap/ui/dom/jquery/Aria"
    ],
    function(library, Core, Item, Icon, NavigationList, InvisibleText, Renderer,
             IconPool, KeyCodes, coreLibrary, openWindow, defaultLinkTypes) {
      "use strict";
       ...
       *
       * @class
       * The NavigationListItem control represents an action, which can be
       * selected by the user. It can provide sub items.
       ...
       */
      var NavigationListItem = Item.extend(
          ...
          /** @lends sap.tnt.NavigationListItem.prototype */ {
            metadata : {
              ...
              properties : {
                 ...
                 * <code>select</code> event should be registered.
                 */
                href : {
                  type : "sap.ui.core.URI",
                  group : "Data",
                ...
                /**
                 * Specifies the browsing context where the linked content will
                 * open.
                 *
      ...
      /**
       * If the item doesn't have a key, the function returns the ID of the
       * NavigationListItem, so the NavigationList can remember the selected
       * item.
       *
       * @private
       */
      ...
      /**
       * Returns the <code>sap.ui.core.Icon</code> control used to display the
      ...
      NavigationListItem.prototype._getExpandIconControl = function() {
        ...
        if (!expandIconControl) {
          ...
          expandIconControl =
              new Icon({
                src : expanded ? NavigationListItem.collapseIcon
                               : NavigationListItem.expandIcon,
      ...
      NavigationListItem.prototype._isListExpanded = function() {
        ...
        return navList.getExpanded() ||
               navList.hasStyleClass("sapTntNavLIPopup");
      };
      ...
      NavigationListItem.prototype.createPopupList = function() {
        var newSubItems = [], list = this.getNavigationList(),
            selectedItem = list.getSelectedItem(), popupSelectedItem, subItem,
            popupSubItem, subItems = this.getItems();
      ...
      NavigationListItem.prototype._selectItem = function(event) {
        var params = {item : this}, navList = this.getNavigationList();
      ...
      NavigationListItem.prototype.onkeydown = function(event) {
        ...
        if ((event.shiftKey && event.which == 189) ||
          ...
          if (this.collapse()) {
            ...
            // prevent ItemNavigation to move the focus to the next/previous
            // item
      ...
      NavigationListItem.prototype.ontap = function(event) {
        var navList = this.getNavigationList(),
            source = Core.byId(event.target.id), level = this.getLevel(),
            parent, list;
        ...
        if (navList.getExpanded() || !this.getItems().length) {
          ...
          if (!source ||
              source.getMetadata().getName() !== 'sap.ui.core.Icon' ||
              !source.$().hasClass('sapTntNavLIExpandIcon')) {
            this._selectItem(event);
            return;
          }
      ...
      NavigationListItem.prototype.onsapenter =
          NavigationListItem.prototype.ontap;
      NavigationListItem.prototype.onsapspace =
          NavigationListItem.prototype.ontap;
      ...
      NavigationListItem.prototype.renderGroupItem = function(rm, control) {
        var isListExpanded = this._isListExpanded(),
            ...
            items = this._getVisibleItems(this), childrenLength = items.length,
            text = this.getText(), href = this.getHref(),
            target = this.getTarget(), tooltip, ariaProps = {
              level : '1',
              role : 'treeitem',
              selected : false,
              roledescription : this._resourceBundleTNTLib.getText(
        ...
        if (control.getExpanded()) {
          var expandIconControl = this._getExpandIconControl();
          expandIconControl.setVisible(this.getItems().length > 0 &&
                                       this.getHasExpander());
          expandIconControl.setSrc(this.getExpanded()
                                       ? NavigationListItem.collapseIcon
                                       : NavigationListItem.expandIcon);
          expandIconControl.setTooltip(
              this._getExpandIconTooltip(!this.getExpanded()));
          this._renderText(rm);
       ...
       * @private
       */
      NavigationListItem.prototype.renderFirstLevelNavItem = function(rm,
                                                                      control) {
        var item, items = this._getVisibleItems(this),
                  childrenLength = items.length, expanded = this.getExpanded(),
                  isListExpanded = this._isListExpanded(), tooltip,
                  ariaProps = {
                    role : 'menuitemradio',
                    checked : false,
                    roledescription : this._resourceBundleTNTLib.getText(
       ...
       * @private
       */
      NavigationListItem.prototype.renderSecondLevelNavItem = function(
          rm, control) {
        var group = this.getParent(), href = this.getHref(),
            target = this.getTarget(), ariaProps = {
              role : 'treeitem',
              level : '2',
              selected : false,
              roledescription : this._resourceBundleTNTLib.getText(
      ...
      NavigationListItem.prototype._renderIcon = function(rm) {
        var icon = this.getIcon(), iconInfo = IconPool.getIconInfo(icon);

        if (icon) {
          // Manually rendering the icon instead of using RenderManager's
          // writeIcon. In this way title attribute is not rendered and the
          // tooltip of the icon does not override item's tooltip
          rm.openStart('span');
          rm.class("sapUiIcon");
      ...
      NavigationListItem.prototype._unselect = function() {
        var $this = this.$(), navList = this.getNavigationList();
        ...
        if (this._isListExpanded()) {
          ...
          if (this.getParent().isA("sap.tnt.NavigationListItem")) {
            this.getParent()
                .$()
                .find('.sapTntNavLIGroup')
                .removeClass('sapTntNavLIItemSelected');
          }
        }
        ...
        if (this._isListExpanded()) {
          ...
          if (this.getParent().isA("sap.tnt.NavigationListItem")) {
            this.getParent()
                .$()
                .find('.sapTntNavLIGroup')
                .addClass('sapTntNavLIItemSelected');
          }
       ...
       * @param {sap.tnt.NavigationList|sap.tnt.NavigationListItem} control The
       *     control to check for visible items
       * @return {sap.tnt.NavigationListItem[]} All the visible
       *     NavigationListItems
       * @private
       */
      ...
      NavigationListItem.prototype.onclick = function(event) {
        // prevent click event on <a> element, in order to avoid unnecessary
        // href changing this will be handled by _openUrl
        if (this.getHref()) {
          event.preventDefault();
        }
      };
      ...
      NavigationListItem.prototype._updateAccessibilityText = function() {
        var invisibleText = NavigationListItem._getInvisibleText(),
            ...
            accType = navList.getExpanded()
                          ? bundle.getText("ACC_CTR_TYPE_TREEITEM")
                          : '',
            $focusedItem = this._getAccessibilityItem(),
            mPosition = this._getAccessibilityPosition(),
            itemPosition = bundle.getText("LIST_ITEM_POSITION",
                                          [ mPosition.index, mPosition.size ]),
            selected = navList._selectedItem === this
                           ? bundle.getText("LIST_ITEM_SELECTED")
                           : '',
            itemText = navList.getExpanded() ? this.getText() : "",
            text =
                accType + " " + itemPosition + " " + selected + " " + itemText;
      ...
      NavigationListItem.prototype._getAccessibilityPosition = function() {
        var parent = this.getParent(),
            ...
            size = visibleItems.length, index = visibleItems.indexOf(this) + 1;
      ...
      NavigationListItem.prototype._getAccessibilityItem = function() {
        var $accItem = this.$(), navList = this.getNavigationList(),

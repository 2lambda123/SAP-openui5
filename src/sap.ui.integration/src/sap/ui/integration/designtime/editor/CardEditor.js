...
sap.ui.define(
    ...
    function (
        ...
        var REGEXP_TRANSLATABLE =
            ...
            oResourceBundle =
                Core.getLibraryResourceBundle("sap.ui.integration");
       ...
       */
                    ...
                    if (oControl.isReady()) {
                        ...
                        if (aItems) {
                            ...
                            for (var i = 0; i < aItems.length; i++) {
                                ...
                                if (oItem._cols === 1) {
                                    oRm.addClass(
                                        "sapUiIntegrationCardEditorItemCol" +
                                            ++iCol,
                                    );
                                } else {
                                    iCol = 0;
                                ...
                                if (oItem.isA("sap.m.Title")) {
                                    ...
                                    if (oLabel) {
                                        var oDependent =
                                            oLabel.getDependents() &&
                                            oLabel.getDependents()[0];
                                        oRm.openStart("div");
                                        oRm.addClass(
        ...
        CardEditor.prototype._filterManifestChangesByLayer = function (
            ...
            oManifestSettings.manifestChanges.forEach(function (oChange) {
                ...
                // value is needed
                var iLayer = oChange.hasOwnProperty(":layer")
                    ? oChange[":layer"]
                    : 1000;
        ...
        CardEditor.prototype._initCard = function (vCardIdOrSettings) {
            if (typeof vCardIdOrSettings === "string") {
                try {
                    ...
                    if (!instance) {
                        ...
                        var element =
                            document.getElementById(vCardIdOrSettings);
                        if (
                            element &&
                            element.tagName &&
            ...
            if (typeof vCardIdOrSettings === "object") {
                ...
                var that = this;
                this._oEditorCard._prepareToApplyManifestSettings =
                    function () {
                        ...
                        var oManifestData =
                            that._oEditorCard.getManifestEntry("/");
                ...
                for (var n in oSettings.form.items) {
                    ...
                    if (oItem.editable && oItem.visible) {
                        ...
                        if (oItem._next && this.getAllowSettings()) {
                            ...
                            if (
                                typeof oItem._next.allowDynamicValues ===
                                    "boolean" &&
                                this.getAllowDynamicValues()
                            ) {
                                mNext = mNext || {};
                                mNext[
                                    oItem._settingspath + "/allowDynamicValues"
        ...
        CardEditor.prototype.checkCurrentSettings = function () {
            ...
            if (oSettings && oSettings.form && oSettings.form.items) {
                for (var n in oSettings.form.items) {
                    ...
                    if (oItem.editable) {
                        if (
                            (oItem.isValid || oItem.required) &&
                            !(
                                this.getMode() === "translation" &&
                                oItem.translatable
                            )
                        ) {
                            if (oItem.isValid) {
                                mChecks[oItem.manifestpath] =
                                    oItem.isValid(oItem);
                            }
        ...
        CardEditor.prototype._createContextModel = function () {
            ...
            oContextDataPromise.then(function (oContextData) {
                ...
                // card internal
                oData["card.internal"] =
                    CardEditor._contextEntries["card.internal"];
            ...
            oContextModel.getProperty = function (sPath, oContext) {
                ...
                if (sAbsolutePath.endsWith("/value")) {
                    ...
                    // ask the host
                    oHost.getContextValue(sAbsolutePath.substring(1)).then(
                        function (vValue) {
            ...
            return new Promise(function (resolve) {
                sap.ui.require(Object.values(CardEditor.fieldMap), function () {
                    ...
                    for (var n in CardEditor.fieldMap) {
                        CardEditor.Fields[n] =
                            arguments[
                                Object.keys(CardEditor.fieldMap).indexOf(n)
                            ];
                    }
                    resolve();
                });
        ...
        CardEditor.prototype._createLabel = function (oConfig) {
            ...
            if (oConfig.description) {
                ...
                oIcon.onmouseover = function () {
                    this._getPopover()
                        .getContent()[0]
                        .applySettings({ text: oConfig.description });
                    ...
                    if (sData) {
                        ...
                        if (aResult) {
                            ...
                            for (var i = 0; i < aResult.length; i++) {
                                var sValueKey =
                                        aResult[i].indexOf("parameters.") === 0
                                            ? "/value"
                                            : "/name",
                                    sDependentPath =
                                        "/sap.card/configuration/" +
        ...
        CardEditor.prototype._addItem = function (oConfig) {
            ...
            if (
                this.getAllowDynamicValues() === false ||
        ...
        CardEditor.prototype._startEditor = function () {
            ...
            if (oSettings.form && oSettings.form.items) {
                if (this.getMode() === "translation") {
                    ...
                    this._addItem({
                        ...
                        translatable: true,
                        label:
                            CardEditor._languages[this._language] ||
                            this.getLanguage(),
                    });
                }
                this._mItemsByPaths = {};
                    ...
                    if (oItem) {
                        ...
                        if (oItem.type === "string") {
                            ...
                            if (sPlaceholder) {
                                ...
                                // defaultValue
                                if (
                                    this._isValueWithHandlebarsTranslation(
                                        sPlaceholder,
                                    )
                                ) {
                                    sTranslationTextKey =
                                        sPlaceholder.substring(
                                            2,
            ...
            if (this.getMode() !== "translation") {
                this._initPreview().then(
                    function () {
                        Promise.all(this._aFieldReadyPromise).then(
                            function () {
                                this._ready = true;
                                this.fireReady();
                            }.bind(this),
        ...
        CardEditor.prototype._applyDesigntimeLayers = function (oSettings) {
            ...
            if (
                ...
                for (
                    var i = 0;
                    i < this._appliedLayerManifestChanges.length;
                    i++
                ) {
                    var oChanges =
                        this._appliedLayerManifestChanges[i][":designtime"];
                    if (oChanges) {
        ...
        CardEditor.prototype._addDestinationSettings = function (
            ...
            if (oSettings && oConfiguration && oConfiguration.destinations) {
                ...
                var oItems = oSettings.form.items;
                Object.keys(oConfiguration.destinations).forEach(
                    function (n) {
                        ...
                        if (
                            typeof oItems[n + ".destinaton"].label ===
                            "undefined"
                        ) {

...
sap.ui.define(
    ...
    function(ui5loader, Control, Core, deepClone, merge, Card, Designtime,
             JSONModel, CardMerger, Label, Title, Icon, RPopover, Popover, Text,
             Log, Popup, ResourceBundle, URI, includeStylesheet,
             LoaderExtension, Parameters, ObjectPath) {
      "use strict";

      // workaround issue of orientation change fired that reapplies position
      // and closes the popup issue is not predictable and depends on host
      // environment. Solution - apply all, simply do not close for position
      // changes.
      var popoverInit = Popover.prototype.init;
      Popover.prototype.init = function() {
        popoverInit.apply(this, arguments);
        var fn = this.oPopup._applyPosition, that = this;
      ...
      function getHigherZIndex(source) {
        ...
        var z = parseInt(
            window.getComputedStyle(source).getPropertyValue('z-index'));
        if (isNaN(z)) {
          return getHigherZIndex(source.parentNode);
        }
        return z + 1;
      }
      var REGEXP_TRANSLATABLE =
              /\{\{(?!parameters.)(?!destinations.)([^\}\}]+)\}\}/g,
       ...
       *
       * @class
       * A control allows to edit manifest settings for a card based on a
       * configuration from a designtime module.
       *
      ...
      var CardEditor = Control.extend(
           ...
           */
          {
            metadata : {
              ...
              properties : {
                /**
                 * Set an id to an already existing card instance as string or
                 * provide the settings for a card as an object
                 * {
                 *    manifest:        manifest for the card as a json object
                 *    baseUrl:         base url for the card
                 *    ...
                 * }
                 * Depending on the scenario there is a card instance already
                 * available. In this case an id to this card instance should be
                 * provided, to avoid an additional card instance creation.
                 */
                card : {type : "any", defaultValue : null},
                /**
                 * admin, content, translation
                 * Used to control the editors capabilities
                 */
                mode : {type : "string", defaultValue : "admin"},
                language : {type : "string", defaultValue : ""},
                allowDynamicValues : {type : "boolean", defaultValue : false},
                allowSettings : {type : "boolean", defaultValue : false},
                designtime : {type : "object"}
              },
              aggregations : {
                /**
            ...
            renderer : function(oRm, oControl) {
              // surrounding div tag <div class="sapUiIntegrationCardEditor
              // sapUiIntegrationCardEditor(Translation|Preview)"
              oRm.openStart("div");
              oControl.getMode() === "translation"
                  ? oRm.addClass("sapUiIntegrationCardEditorTranslation")
                  : oRm.addClass("sapUiIntegrationCardEditorPreview");
              oRm.addClass("sapUiIntegrationCardEditor");
              oRm.writeClasses();
              oRm.writeElementData(oControl);
              oRm.openEnd();
              if (oControl.isReady()) {
                // surrounding div tag for form <div
                // class="sapUiIntegrationCardEditorForm"
                oRm.openStart("div");
                ...
                if (aItems) {
                  ...
                  for (var i = 0; i < aItems.length; i++) {
                    ...
                    if (oItem.isA("sap.m.Label")) {
                      oLabel = oItem; // store the label and render it together
                                      // with the next field
                      continue;
                    }
                    oRm.openStart("div");
                    ...
                    // if multiple cols are used in the form, set class
                    // sapUiIntegrationCardEditorItemCol1 or
                    // sapUiIntegrationCardEditorItemCol2
                    if (oItem._cols === 1) {
                      oRm.addClass("sapUiIntegrationCardEditorItemCol" +
                                   (++iCol));
                    } else {
                      iCol = 0;
                    }
                    ...
                    if (oItem.isA("sap.m.Title")) {
                      ...
                      if (oLabel) {
                        var oDependent =
                            oLabel.getDependents() && oLabel.getDependents()[0];
                        oRm.openStart("div");
                    ...
                    iCol = iCol == 2 ? 0 : iCol; // reset the cols if 2, we only
                                                 // support 2 cols currently
                  }
                }
                oRm.close("div");
                // render the preview
       ...
       */
      CardEditor.prototype.isReady = function() { return this._ready; };
          ...
          if (!oData[s]) {
            ...
            if (oData.type) {
              a.push({
                ...
                value : oData.pathvalue ||
                            "{context>" + path.substring(1) + "/value}",
                object : oData
              });
            } else {
              a.push({path : path.substring(1), object : oData});
              for (var n in oData) {
                flattenData(oData[n], s, a, path + "/" + n);
              }
            }
       ...
       * All changes that are done for layers > than current layer are removed
       * (see also CardMerger.layers) The current layers changes are stored in
       * this._currentLayerManifestChanges to be applied later in the editor
       * code. All changes that are done for layers < that the current layer are
       * kept in oManifestSettings.manifestChanges
       *
       * @param {*} oManifestSettings
       */
      CardEditor.prototype._filterManifestChangesByLayer = function(
          oManifestSettings) {
        var aChanges = [], oCurrentLayerChanges = {
          ":layer" : CardMerger.layers[this.getMode()]
        },
            iCurrentModeIndex = CardMerger.layers[this.getMode()];
        oManifestSettings.manifestChanges.forEach(function(oChange) {
          // filter manifest changes. only the changes before the current layer
          // are needed card editor will merge the last layer locally to allow
          // "reset" or properties also for translation layer, the "original"
          // value is needed
          var iLayer =
              oChange.hasOwnProperty(":layer") ? oChange[":layer"] : 1000;
      ...
      /**
       * Sets the card property as a string, object {manifest:{}, baseUrl:{}} or
       * a reference to a card instance
       * @param {any} vCardIdOrSettings
       * @param {boolean} bSuppressRerendering
       */
      CardEditor.prototype.setCard = function(vCardIdOrSettings,
        ...
        // Waiting for additional settings on the card that are applied
        // synchronously
        Promise.resolve().then(
            function() { this._initCard(vCardIdOrSettings); }.bind(this));
        return this;
      };
      /**
       * Sets the language of the editor
       *
       * @param {string} sValue the language in the format language_region or
       *     language-region
      ...
      CardEditor.prototype.setLanguage = function(sValue, bSuppress) {
        ...
        if (!CardEditor._languages[this._language]) {
          Log.warning("The language: " + sValue +
                      " is currently unknown, some UI controls might show " +
                      sValue + " instead of the language name.");
        }
        return this;
      };
      /**
      ...
      /**
       * Returns the original manifest json without processed parameters,
       * handlebar translation
       */
      ...
      CardEditor.prototype._initCard = function(vCardIdOrSettings) {
          ...
          try {
            ...
            if (!instance) { // not a card instance, but a string
              ...
              if (element && element.tagName &&
                  element.tagName === "ui-integration-card") {
        ...
        if (vCardIdOrSettings && vCardIdOrSettings.isA &&
            vCardIdOrSettings.isA("sap.ui.integration.widgets.Card")) {
          // a card instance
          vCardIdOrSettings = {
        ...
        if (typeof vCardIdOrSettings === "object") {
          ...
          this._oEditorCard._prepareToApplyManifestSettings = function() {
            Card.prototype._prepareToApplyManifestSettings.apply(this,
                                                                 arguments);
            if (!that._oEditorCard._isManifestReady) {
              // TODO: manifestReady is fired even if the manifest is not ready.
              // Check why.
              return;
            }
            ...
            that._appliedLayerManifestChanges =
                vCardIdOrSettings.manifestChanges;
            ...
            if (iCurrentModeIndex < CardMerger.layers["translation"] &&
                that._currentLayerManifestChanges) {
              // merge if not translation

              oManifestData = CardMerger.mergeCardDelta(
                  oManifestData, [ that._currentLayerManifestChanges ]);
            }
            // create a manifest model after the changes are merged
            that._manifestModel = new JSONModel(oManifestData);
            // create a manifest model for the original "raw" manifest that was
            // initially loaded

            that._originalManifestModel =
                new JSONModel(that._getOriginalManifestJson());
            that._initInternal();
      ...
      CardEditor.prototype._initInternal = function() {
        ...
        var sDesigntime =
                this._oEditorCard.getManifestEntry("/sap.card/designtime"),
            oConfiguration =
                this._manifestModel.getProperty("/sap.card/configuration"),
            oPromise, oDesigntimeConfig = this.getDesigntime();
        if (oDesigntimeConfig) {
          if (typeof oDesigntimeConfig === "function") {
            ...
            oPromise = new Promise(function(resolve, reject) {
              sap.ui.require(
                  [ "sap/ui/integration/Designtime" ], function(Designtime) {
                    var AdvancedDesigntime =
                        Designtime.extend("test.Designtime");
                    AdvancedDesigntime.prototype.create =
                        function() { return oDesigntimeConfig; };
          ...
          oPromise =
              this._oEditorCard.loadDesigntime().then(function(oDesigntime) {
                this._applyDesigntimeDefaults(oDesigntime.getSettings());
                return oDesigntime;
              }.bind(this));
        } else {
          // stay compatible and create designtime configuration based on
          // parameters/destinations
          oPromise =
              Promise.resolve(this._createParameterDesigntime(oConfiguration));
        }
        oPromise.then(function(oDesigntime) {
          ...
          if (this.getMode() === "admin" || this.getMode() === "all") {
            ...
            this._addDestinationSettings(oConfiguration,
                                         this._oDesigntimeInstance);
          }
          // create a settings model
          this._settingsModel =
              new JSONModel(this._oDesigntimeInstance.getSettings());
          ...
          this._applyDesigntimeLayers(); // changes done from admin to content
                                         // on the dt values
          this._requireFields().then(function() { this._startEditor(); }.bind(
              this));
        }.bind(this));
      };
      /**
       * Returns the current settings as a json with a manifest path and the
       * current value additionally there is a layer number added as ":layer"
       */
      CardEditor.prototype.getCurrentSettings = function() {
        var oSettings = this._settingsModel.getProperty("/"), mResult = {},
            mNext;
          ...
          for (var n in oSettings.form.items) {
            ...
            if (oItem.editable && oItem.visible) {
              if (this.getMode() !== "translation") {
                if (oItem.translatable && !oItem._changed &&
                    oItem._translatedDefaultPlaceholder &&
                    !this._currentLayerManifestChanges[oItem.manifestPath]) {
                  // do not save a value that was not changed and comes from a
                  // translated default value mResult[oItem.manifestpath] =
                  // oItem._translatedDefaultPlaceholder; if we would save it
                  continue;
                } else {
              ...
              if (oItem._next && (this.getAllowSettings())) {
                ...
                if (typeof oItem._next.allowDynamicValues === "boolean" &&
                    this.getAllowDynamicValues()) {
                  mNext = mNext || {};
                  mNext[oItem._settingspath + "/allowDynamicValues"] =
                      oItem._next.allowDynamicValues;
                }
              }
            }
          }
      ...
      /**
       * Checks for invalid values in the current settings and reports the
       * errors
       * TODO: highlight issues and add states...
       */
      CardEditor.prototype.checkCurrentSettings = function() {
        var oSettings = this._settingsModel.getProperty("/"), mChecks = {};
        if (oSettings && oSettings.form && oSettings.form.items) {
          for (var n in oSettings.form.items) {
            var oItem = oSettings.form.items[n];
            if (oItem.editable) {
              if ((oItem.isValid || oItem.required) &&
                  !(this.getMode() === "translation" && oItem.translatable)) {
                if (oItem.isValid) {
      ...
      CardEditor.prototype._createContextModel = function() {
        ...
        var oContextDataPromise = new Promise(function(resolve, reject) {
          if (oHost && oHost.getContext) {
            ...
            setTimeout(function() {
              ...
              Log.error("Card Editor context could not be determined with " +
                        CONTEXT_TIMEOUT + ".");
              bResolved = true;
              resolve({});
            }, CONTEXT_TIMEOUT);
            oHost.getContext().then(function(oContextData) {
              if (bResolved) {
                Log.error("Card Editor context returned after more than " +
                          CONTEXT_TIMEOUT + ". Context is ignored.");
              }
              bResolved = true;
        ...
        oContextModel.getProperty = function(sPath, oContext) {
          ...
          if (sAbsolutePath.endsWith("/value")) {
            ...
            // ask the host
            oHost.getContextValue(sAbsolutePath.substring(1))
      ...
      /**
       * Loads all field modules registered in CardEditor.fieldMap and stores
       * the classes in CardEditor.Fields
       */
        ...
        return new Promise(function(resolve) {
          sap.ui.require(Object.values(CardEditor.fieldMap), function() {
            ...
            for (var n in CardEditor.fieldMap) {
              CardEditor.Fields[n] =
                  arguments[Object.keys(CardEditor.fieldMap).indexOf(n)];
            }
            resolve();
          });
        });
      ...
      CardEditor.prototype._createLabel = function(oConfig) {
        var oLabel = new Label({
          ...
          // otherwise this is confusing because user will not be able to
          // correct it
        ...
        if (oConfig.description) {
          ...
          oIcon.onmouseover = function() {
            this._getPopover().getContent()[0].applySettings(
                {text : oConfig.description});
      ...
      CardEditor.prototype._getPopover = function() {
        ...
        oText.addStyleClass(
            "sapUiTinyMargin sapUiIntegrationCardEditorDescriptionText");
        this._oPopover =
            new RPopover({showHeader : false, content : [ oText ]});
      ...
      CardEditor.prototype._createField = function(oConfig) {
        ...
        var oBinding =
            this._settingsModel.bindProperty(oConfig._settingspath + "/value");
        oBinding.attachChange(function() {
          if (!this._bIgnoreUpdates) {
            oConfig._changed = true;
            if (oConfig._dependentFields &&
                oConfig._dependentFields.length > 0) {
      ...
      /**
       * Creates a unnamed model if a values.data section exists in the
       * configuration
      ...
      CardEditor.prototype._addValueListModel = function(oConfig, oField,
        ...
        if (oConfig.values && oConfig.values.data && this._oProviderCard &&
            this._oProviderCard._oDataProviderFactory) {
          var oValueModel = new JSONModel({}),
              oPromise = this._oProviderCard._oDataProviderFactory
                             .create(oConfig.values.data)
                             .getData();
          this._settingsModel.setProperty(oConfig._settingspath + "/_loading",
                                          true);
          oPromise
              .then(function(oData) {
                if (oConfig._cancel) {
                ...
                this._settingsModel.setProperty(
                    oConfig._settingspath + "/_loading", false);
              }.bind(this))
              .catch(function() {
                oConfig._values = {};
                oValueModel.setData({});
          ...
          // in the designtime the item bindings will not use a named model,
          // therefore we add a unnamed model for the field to carry the values,
          // also we use the binding context to connect the given path from
          // oConfig.values.data.path with that the result of the data request
          // can be have also other structures.
          oField.setModel(oValueModel, undefined);
          oField.bindObject({path : oConfig.values.data.path || "/"});
          if (!bIgnore) {
            ...
            if (sData) {
              ...
              if (aResult) {
                // add the field to dependency to either the parameter or
                // destination
                for (var i = 0; i < aResult.length; i++) {
                  var sValueKey = aResult[i].indexOf("parameters.") === 0
                                      ? "/value"
                                      : "/name",
                      sDependentPath = "/sap.card/configuration/" +
                                       aResult[i].replace(".", "/") + sValueKey,
                      oItem = this._mItemsByPaths[sDependentPath];
                  if (oItem) {
                    oItem._dependentFields = oItem._dependentFields || [];
                    oItem._dependentFields.push(
                        {field : oField, config : oConfig});
                  }
                }
              }
            }
      ...
      CardEditor.prototype._addItem = function(oConfig) {
        ...
        if (sMode === "translation") {
          if (typeof oConfig.value === "string" &&
              oConfig.value.indexOf("{") === 0) {
            // do not show dynamic values for translation
            return;
          }
          // adding an internal _language object to save the original value for
          // the UI
          oConfig._language = {value : oConfig.value};

          // force a 2 column layout in the form, remember the original to reset

          oConfig.cols = 1;

          // create a configuration clone. map the _settingspath setting to
          // _language, and set it to not editable
          ...
          if (!origLangField.value) {
            // the original language field shows only a text control. If empty
            // we show a dash to avoid empty text.
            origLangField.value = "-";
          }
          this.addAggregation("_formContent", this._createLabel(origLangField));
          this.addAggregation("_formContent", this._createField(origLangField));
          oConfig.value = oConfig._translatedDefaultValue || "";
          // even if a item is not visible or not editable by another layer for
          // translations it should always be editable and visible
          oConfig.editable = oConfig.visible = oConfig.translatable;
          // if there are changes for the current layer, read the already
          // translated value from there now merge these changes for translation
          // into the item configs
          if (this._currentLayerManifestChanges) {
            oConfig.value =
                this._currentLayerManifestChanges[oConfig.manifestpath] ||
                oConfig.value;
          }
        ...
        this.addAggregation("_formContent", this._createLabel(oConfig));
      ...
      /**
       * Returns the current language specific text for a given key or "" if no
       * translation for the key exists
       */
      ...
      CardEditor.prototype._getBaseUrl = function() {
        if (this._oEditorCard && this._oEditorCard.isReady()) {
          return this._oEditorCard.getBaseUrl() ||
                 this.oCardEditor._oEditorCard._oCardManifest.getUrl();
        }
        return "";
      };
      ...
      CardEditor.prototype._startEditor = function() {
        ...
        if (oSettings.form && oSettings.form.items) {
          if (this.getMode() === "translation") {
            // add 2 group items to show over the columns to avoid language
            // repetition in the labels
            this._addItem({
              type : "group",
              cols : 1,
              translatable : true,
            ...
            this._addItem({
              ...
              translatable : true,
              label :
                  CardEditor._languages[this._language] || this.getLanguage()
            });
          }
          this._mItemsByPaths = {};
          for (var n in oSettings.form.items) {
            ...
            if (oItem) {
              ...
              // check if the provided value from the parameter or designtime
              // default value is a translated value restrict this to string
              // types for now
              if (oItem.type === "string") {
                oItem._translatedDefaultPlaceholder =
                    this._getManifestDefaultValue(oItem.manifestpath) ||
                    oItem.defaultValue;
                var sTranslationTextKey = null,
                    sPlaceholder = oItem._translatedDefaultPlaceholder;
                if (sPlaceholder) {
                  // parameter translated value wins over designtime
                  // defaultValue
                  if (this._isValueWithHandlebarsTranslation(sPlaceholder)) {
                    sTranslationTextKey =
                        sPlaceholder.substring(2, sPlaceholder.length - 2);
                  } else if (sPlaceholder.startsWith("{i18n>")) {
                    sTranslationTextKey =
                        sPlaceholder.substring(6, sPlaceholder.length - 1);
                  }
                  // only if there is a translation key
                  if (sTranslationTextKey) {
                    // force translatable, even if it was not explicitly set
                    // already
                    oItem.translatable = true;
                    oItem._translatedDefaultValue =
                        this.getModel("i18n").getResourceBundle().getText(
                            sTranslationTextKey);
                    if (oItem._changed) {
                    ...
                    if (this.getMode() === "translation") {
                      // if we are in translation mode the default value differs
                      // and depends on the language
                      // TODO this does not work in SWZ, the base path is not
                      // taken into account... get the translated default value
                      // for the language we want to translate
                      // this.getLanguage()
                      oItem._translatedDefaultValue =
                ...
                if (this.getMode() === "translation") {
                  if (this._isValueWithHandlebarsTranslation(oItem.label)) {
                    oItem._translatedLabel =
                        this._getCurrentLanguageSpecificText(
                            oItem.label.substring(2, oItem.label.length - 2),
                            true);
                  } else if (oItem.label && oItem.label.startsWith("{i18n>")) {
                    // TODO this does not work in SWZ, the base path is not
                    // taken into account... get the translated default value
                    // for the language we want to translate this.getLanguage()
                    oItem._translatedLabel =
                        this._getCurrentLanguageSpecificText(
                            oItem.label.substring(6, oItem.label.length - 1),
                            true);
                  }
                }
              }
      ...
      CardEditor.prototype._initPreview = function() {
        return new Promise(function(resolve, reject) {
          sap.ui.require([ "sap/ui/integration/designtime/editor/CardPreview" ],
                         function(Preview) {
      ...
      CardEditor.prototype._applyDesigntimeDefaults = function(oSettings) {
        ...
        oSettings.preview = oSettings.preview || {modes : "Abstract"};
        ...
        for (var n in mItems) {
          ...
          // only if the value is undefined from the
          // this._manifestModel.getProperty(oItem.manifestpath) false, "", 0...
          // are valid values and should not apply the default
      ...
      CardEditor.prototype._applyDesigntimeLayers = function(oSettings) {
        // pull current values
        if (this._appliedLayerManifestChanges &&
            Array.isArray(this._appliedLayerManifestChanges)) {
        ...
        if (this._currentLayerManifestChanges) {
          ...
          if (oChanges) {
            var aKeys = Object.keys(oChanges);
            for (var j = 0; j < aKeys.length; j++) {
              // apply the values to a "_next/editable", "_next/visible" entry
              // to the settings. the current layer needs to be able to change
              // those values
              var sPath = aKeys[j],
                  sNext =
                      sPath.substring(0, sPath.lastIndexOf("/") + 1) + "_next";
      ...
      /**
       * Creates a designtime instance based on an configuration section within
       * the manifest. This is valid if there is no explicit sap.card/designtime
       * module in the manifest itself.
       */
      CardEditor.prototype._createParameterDesigntime = function(
          oConfiguration) {
        var oSettings = {}, sBasePath = "/sap.card/configuration/parameters/",
            sMode = this.getMode();
        ...
        if (oSettings && oConfiguration && oConfiguration.destinations) {
          if (!oSettings.form.items["destination.group"]) {
            ...
            oSettings.form.items["destination.group"] = {
              label : oResourceBundle.getText("CARDEDITOR_DESTINATIONS") ||
                          "Destinations",
              type : "group",
              visible : true
            };
          ...
          Object.keys(oConfiguration.destinations).forEach(function(n) {
            ...
            oItems[n + ".destinaton"] = merge({
              manifestpath :
                  sBasePath + n +
                      "/name", // destination points to name not value
              visible : true,
              type : "destination",
              editable : true,
            ...
            if (oHost) {
              ...
              this._oEditorCard.getHostInstance().getDestinations().then(
                  function(n, a) {
      ...
      /**
       * Returns the default value that was given by the developer for the given
       * path
       * @param {string} sPath
       */
      CardEditor.prototype._getManifestDefaultValue = function(
          sPath) { return this._originalManifestModel.getProperty(sPath); };
      /**
       * Returns whether the value is translatable via the handlbars translation
       * syntax {{KEY}} For other than string values false is returned
       * @param {any} vValue
       */
      CardEditor.prototype._isValueWithHandlebarsTranslation = function(
          vValue) {
      ...
      CardEditor._contextEntries = {
        empty : {
          ...
          type : "string",
          description :
              oResourceBundle.getText("CARDEDITOR_CONTEXT_EMPTY_DESC"),
          placeholder : "",
          value : ""
        },
        "card.internal" : {
          label :
              oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_INTERNAL_VAL"),
          todayIso : {
            type : "string",
            label :
                oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_TODAY_VAL"),
            description :
                oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_TODAY_DESC"),
            tags : [],
            placeholder :
                oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_TODAY_VAL"),
          ...
          nowIso : {
            ...
            description :
                oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_NOW_DESC"),
            tags : [],
            placeholder :
                oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_NOW_VAL"),
          ...
          currentLanguage : {
            ...
            description :
                oResourceBundle.getText("CARDEDITOR_CONTEXT_CARD_LANG_VAL"),
            tags : [ "technical" ],
      ...
      // map of language strings in their actual language representation,
      // initialized in CardEditor.init
      CardEditor._languages = {};

      // theming from parameters to css valiables if css variables are not
      // turned on find out if css vars are turned on
      CardEditor._appendThemeVars = function() {
        var oOldElement =
            document.getElementById("sap-ui-integration-editor-style");
        if (oOldElement && oOldElement.parentNode) {
          oOldElement.parentNode.removeChild(oOldElement);
        }
        var aVars =
                [
                  "sapUiButtonHoverBackground", "sapUiBaseBG",
                  "sapUiContentLabelColor", "sapUiTileSeparatorColor",
                  "sapUiHighlight", "sapUiListSelectionBackgroundColor",
        ...
        oStyle.innerHTML =
            ".sapUiIntegrationCardEditor, .sapUiIntegrationFieldSettings, .sapUiIntegrationIconSelectList {" +
            aVars.join(";") + "}";
      ...
      CardEditor.init = function() {
        ...
        // if
        // (!window.getComputedStyle(document.documentElement).getPropertyValue('--sapBackgroundColor')) {
        CardEditor._appendThemeVars();
        Core.attachThemeChanged(function() { CardEditor._appendThemeVars(); });
        //}

        var sCssURL = sap.ui.require.toUrl(
            "sap.ui.integration.designtime.editor.css.CardEditor".replace(/\./g,
                                                                          "/") +
            ".css");
        includeStylesheet(sCssURL);
        LoaderExtension
            .loadResource(
                "sap/ui/integration/designtime/editor/languages.json",
                {dataType : "json", failOnError : false, async : true})
            .then(function(o) { CardEditor._languages = o; });
      };
      CardEditor.init();

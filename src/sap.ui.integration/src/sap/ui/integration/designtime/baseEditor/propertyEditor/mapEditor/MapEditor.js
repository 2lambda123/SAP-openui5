...
sap.ui.define(
    [
      ...
      "sap/base/util/deepClone", "sap/base/util/deepEqual",
      "sap/ui/model/json/JSONModel", "sap/base/util/restricted/_merge",
      "sap/base/util/restricted/_omit", "sap/base/util/isPlainObject",
      "sap/base/util/includes"
    ],
    function(BasePropertyEditor, deepClone, deepEqual, JSONModel, _merge, _omit,
             isPlainObject, includes) {
      "use strict";
      ...
      /**
       * @class
       * Constructor for a new <code>MapEditor</code> for editing key-value
       * pairs with primitive values. To get notified about changes made with
       * the editor, you can attach a handler to the <code>valueChange</code>
       * event.
       *
       * <h3>Configuration</h3>
       *
       ...
       * 	<td>Whether to allow editing the key attribute of map
       * entries</td>
       * </tr>
       * <tr>
       ...
       *  <td><code>boolean</code></td>
       * 	<td><code>true</code></td>
       * 	<td>Whether to show entries with invalid types if the
       * <code>StringEditor</code> cannot be used as a fallback</td>
       * </tr>
       * <tr>
       ...
       * @ui5-restricted
       */
      var MapEditor = BasePropertyEditor.extend(
          "sap.ui.integration.designtime.baseEditor.propertyEditor.mapEditor.MapEditor",
          {
            xmlFragment :
                "sap.ui.integration.designtime.baseEditor.propertyEditor.mapEditor.MapEditor",
            ...
            init : function() {
              ...
              this.attachModelContextChange(function() {
                if (this.getModel("i18n")) {
                  ...
                  this._aSupportedTypes =
                      Object.keys(SUPPORTED_TYPE_LABELS).map(function(sKey) {
                        return {
                          key : sKey,
                          label : oResourceBundle.getText(
                              SUPPORTED_TYPE_LABELS[sKey])
                        };
                      });
            ...
            setValue : function(mValue) {
              ...
              this.setDesigntimeMetadata(_merge(
                  ...
                  Object.keys(mPositions).reduce(function(mNewMetadata, sKey) {
                    mNewMetadata[sKey] = {
                      __value : {position : mPositions[sKey]}
                    };
                    return mNewMetadata;
                  }, {})));
              ...
              aItems = aItems
                           .sort(function(oValue1, oValue2) {
                             return mPositions[oValue1.key] -
                                    mPositions[oValue2.key];
                           })
            ...
            _processValue : function(mValue) {
              return Object.keys(mValue)
                  .map(
                      function(sKey) {
                        var mFormattedValue =
                            this._prepareInputValue(mValue[sKey], sKey);
                        ...
                        return this.getConfig().includeInvalidEntries ||
                                       this._isValidItem(
                                           oItem, deepClone(mValue[sKey]))
                                   ? oItem
                                   : undefined;
                      },
                      this)
            ...
            _prepareInputValue : function(oValue, sKey) {
              var mFormattedValue =
                  this.processInputValue(deepClone(oValue), sKey);
              if (!mFormattedValue.type) {
                mFormattedValue.type =
                    this._mTypes[sKey] ||
                    this._getDesigntimeMetadataValue(sKey).type ||
                    this._getDefaultType(mFormattedValue.value);
              }
            ...
            /**
             * Hook which is called for every map item to filter invalid
             * entries.
            ...
            _getDefaultType : function(vValue) {
              ...
              var sChosenType =
                  includes(aAllowedTypes, sType) ? sType : undefined;
            ...
            _getAllowedTypes : function() {
              return (this.getConfig() || MapEditor.configMetadata)
                  .allowedTypes;
            },

            _setSupportedTypesModel : function() {
              var aAllowedTypes = this._getAllowedTypes();
              this._supportedTypesModel.setData(
                  this._aSupportedTypes.filter(function(oSupportedType) {
             ...
             * @param {object} oConfigValue.designtime - Item designtime
             *     metadata
              ...
              return [
                {
                  ...
                  validators : [ {
                    type : "isUniqueKey",
                    config : {
                      keys : function() { return Object.keys(this.getValue()); }
                                 .bind(this),
                ...
                },
                {
                  ...
                  path : "value",
                  value : vValue,
                  type : sType && includes(this._getAllowedTypes(), sType)
                             ? sType
                             : this._getDefaultType(vValue),
                  visible : sType !== "group",
                  itemKey : sKey,
            ...
            getExpectedWrapperCount : function(mValue) {
              // Process the value first to not wait for filtered out invalid
              // items
              return this._processValue(mValue).length;
            },

            /**
             * Hook which is called for every map item value coming from the
             * JSON. Can be overridden in order to format the value to fit the
             * editor's requirements.
             * @param {object} oValue - Original map item value
             * @returns {object} Formatted map item value
             */
            processInputValue : function(oValue) { return {value : oValue}; },

            /**
             * Hook which is called for every map item value coming from the
             * editor. Can be overridden in order to format the value to fit the
             * JSON schema definition.
             * @param {object} oValue - Original map item value
             * @returns {object} Formatted map item value
             */
            processOutputValue : function(oValue) { return oValue.value; },

            _onRemoveElement : function(oEvent) {
              var sKeyToDelete = oEvent.getSource()
                                     .getBindingContext("itemsModel")
                                     .getObject()
                                     .key;
              var mValue = this.getValue();
              this.setValue(_omit(mValue, sKeyToDelete));
              var oNewDesigntimeMetadata =
                  deepClone(this.getDesigntimeMetadata());
            ...
            _moveUp : function(oEvent) {
              ...
              if (iIndex > 0) {
                ...
                this._swapPositions(aValues[iIndex].key,
                                    aValues[iIndex - 1].key);
              }
            },
            ...
            _swapPositions : function(sKey1, sKey2) {
              ...
              oNewDesigntimeMetadata[sKey1] = {
                __value : {
                  position :
                      this.getNestedDesigntimeMetadataValue(sKey2).position
                }
              };
              oNewDesigntimeMetadata[sKey2] = {
                __value : {
                  position :
                      this.getNestedDesigntimeMetadataValue(sKey1).position
                }
              };
              this.setDesigntimeMetadata(_merge(
                  {}, this.getDesigntimeMetadata(), oNewDesigntimeMetadata));
            ...
            _isNewItem : function(mItem) {
              return deepEqual(mItem.value,
                               this._prepareInputValue(this.processOutputValue(
            ...
            _propertyEditorsChange : function(oEvent) {
              var aPreviousPropertyEditors =
                  oEvent.getParameter("previousPropertyEditors");
              var aPropertyEditors = oEvent.getParameter("propertyEditors");
              if (Array.isArray(aPreviousPropertyEditors)) {
                aPreviousPropertyEditors
                    .forEach(function(oPreviousPropertyEditor) {
                      oPreviousPropertyEditor.detachValueChange(
                          this._onItemChange, this);
                      oPreviousPropertyEditor.detachDesigntimeMetadataChange(
                          this._onDesigntimeValueChange, this);
                    }, this);
              }
            ...
            _onItemChange : function(oEvent) {
              ...
              if (typeof fnHandler !== 'function') {
                // No specific handler is registered for the change, use generic
                // handler
            ...
            _onDesigntimeChange : function(sKey, oEvent) {
              ...
              newDesigntimeValue.__value[oEvent.getParameter("path")] =
                  oEvent.getParameter("value");

              oDesigntime[sKey] =
                  _merge({}, oDesigntime[sKey], newDesigntimeValue);
              this.setDesigntimeMetadata(oDesigntime);
              this.setValue(this.getValue());
            },

            getItemChangeHandlers : function() {
              return {"key" : this._onKeyChange, "type" : this._onTypeChange};
            },

            _onKeyChange : function(sOldKey, oEvent) {
              // Ignore changes triggered by editor initialization to avoid
              // duplicate key errors for the initial value
              ...
              if (sNewKey !== sOldKey) {
                ...
                if (oNewValue[sNewKey] && oNewValue[sNewKey].type !== "group" &&
                    oNewValue[sNewKey].manifestpath &&
                    oNewValue[sNewKey].manifestpath.startsWith(
                        "/sap.card/configuration/parameters/")) {
                  oNewValue[sNewKey].manifestpath =
                      "/sap.card/configuration/parameters/" + sNewKey +
                      "/value";
                }
                ...
                if (oDesigntime.hasOwnProperty(sOldKey)) {
                  ...
                  if (oDesigntime[sNewKey].__value &&
                    ...
                    oDesigntime[sNewKey].__value.manifestpath =
                        oDesigntime[sNewKey].__value.manifestpath.replace(
                            sOldKey, sNewKey);
                  }
            ...
            _onTypeChange : function(sKey, oEvent) {
              // Ignore changes triggered by editor initialization as the item
              // type has not actually changed
              if (oEvent.getParameter("previousValue") === undefined) {
                return;
              }
      ...
      MapEditor.configMetadata =
          Object.assign({}, BasePropertyEditor.configMetadata, {
            allowKeyChange :
                {defaultValue : true, mergeStrategy : "mostRestrictiveWins"},
            allowTypeChange :
                {defaultValue : true, mergeStrategy : "mostRestrictiveWins"},
            allowAddAndRemove :
                {defaultValue : true, mergeStrategy : "mostRestrictiveWins"},
            allowedTypes :
                {defaultValue : [ "string" ], mergeStrategy : "intersection"},
            allowSorting :
                {defaultValue : true, mergeStrategy : "mostRestrictiveWins"},

...
sap.ui.define(
    ...
    function(createPromise, PropertyEditorFactory, ValidatorRegistry,
             PropertyEditors, resolveBinding, ObjectBinding, hasTag,
             cleanupDesigntimeMetadata, Control, ResourceModel, ObjectPath,
             each, deepClone, deepEqual, values, includes, isPlainObject,
             isEmptyObject, _intersection, _mergeWith, _merge, _omit, _union,
             _isNil, _castArray, JSONModel, ResourceBundle, Log, unset) {
      "use strict";
       ...
       * <h4>Example</h4>
       * <pre>
       * sap.ui.require(["sap/ui/integration/designtime/baseEditor/BaseEditor"],
       * function (Editor) { var oJson = { root: { context: { id: "404", name:
       * "Kate", role: "End User"
       *             },
       *             foo: {
       *                 bar: true
       ...
       *         "propertyEditors": {
       *             "enum" :
       * "sap/ui/integration/designtime/baseEditor/propertyEditors/enumStringEditor/EnumStringEditor",
       *             "string" :
       * "sap/ui/integration/designtime/baseEditor/propertyEditors/stringEditor/StringEditor"
       *         }
       *     });
       ...
       * @ui5-restricted
       */
      var BaseEditor = Control.extend(
          "sap.ui.integration.designtime.baseEditor.BaseEditor", {
            metadata : {
              library : "sap.ui.integration",
              properties : {
                /**
                 * JSON to be changed in the editor. Note: If an object is
                 * passed as a parameter, it won't be mutated.
                 * <code>.getJson()</code> or <code>.attachJsonChange()</code>
                ...
                /**
                 * Configuration Map
                 *   config.context {string} Path in the JSON that will be
                 * edited e.g. <code>"path/subpath"</code> for
                 * <code>json.path.subpath</code>
                 *   config.properties {Object<string,object>} Defines which
                 * fields in the context are editable
                 *     config.properties.<key>.label {string} of the property to
                 * show on the UI config.properties.<key>.type {string} of the
                 * property (property editor for this type will be shown)
                 *     config.properties.<key>.path {string} that will be
                 * changed, relative to the context. Example: If the context is
                 * <code>root</code> and the path is <code>header/name</code>,
                 * the <code>json.root.header.name</code> field is to be changed
                 *     config.properties.<key>.value {string|boolean} (Optional)
                 * value of the property. A binding relative to the context
                 * (model name) should be used. Example:
                 * <code>{context>header/name}</code> will create a binding
                 * <code>json.root.header.name</code>
                 *     config.properties.<key>.tags {array} Strings to
                 * categorize the property
                 *     config.properties.<key>.visible {string|boolean} Should
                 * be used as a binding relative to the context to define the
                 * conditions under which this property should be changeable,
                 * e.g. <code>{= ${context>anotherProperty} ===
                 * 'someValue'}</code> config.properties.<key>.<other
                 * configurations> {any} It is possible to define additional
                 * configurations in this namespace. These configurations will
                 * be passed to the dedicated property editor. Binding strings
                 * relative to context model are supported as well, e.g.
                 * <code>{= ${context>someProperty} +
                 * ${context>anotherProperty}}</code>
                 *   config.propertyEditors {Object<string,string>} Defines
                 * which property editors should be loaded. Key is the property
                 * type and value is the editor module path. Example:
                 * <code>propertyEditors: {"string":
                 * "sap/ui/integration/designtime/controls/propertyEditors/StringEditor"}</code>
                 * defines the module responsible for all properties with the
                 * type <code>string</code> config.i18n {string|array} Module
                 * path or array of paths for i18n property files. i18n binding,
                 * for example, <code>{i18n>key}</code> is available in the
                 * <code>/properties<code> section, e.g. for <code>label</code>
                 */
                "config" : {
                  type : "object",
                  defaultValue : {
                ...
                /**
                 * Designtime-specific metadata to be changed in the editor.
                 * Note: If an object is passed as a parameter, it won't be
                 * mutated. <code>.getDesigntimeMetadata()</code> or
                 * <code>.attachDesigntimeMetadataChange()</code> should be used
                 * instead to get the changed object.
                 */
                "designtimeMetadata" : {type : "object"},

                /**
                 * Layout name. Standard layout types: list | form
                 */
                "layout" : {type : "string", defaultValue : "list"}
              },
              ...
              events : {
                 ...
                 */
                jsonChange : {parameters : {json : {type : "object"}}},
                /**
                 * Fired when designtime metadata has been changed by a
                 * <code>propertyEditor</code>.
                 */
                designtimeMetadataChange :
                    {parameters : {designtimeMetadata : {type : "object"}}},
                /**
                 * Fired when config has been changed.
                 */
                configChange : {parameters : {config : {type : "object"}}},
                /**
                 * Fired when all property editors for the given JSON and
                 * configuration are created.
                 * TODO: remove this public event.
                 */
                propertyEditorsReady :
                    {parameters : {propertyEditors : {type : "array"}}}
              }
            },
            ...
            renderer : function(oRm, oControl) {
              ...
              if (aContent.length) {
                aContent.forEach(function(
                    oChildControl) { oRm.renderControl(oChildControl); });
              } else {
                oControl.getPropertyEditorsSync().forEach(function(
                    oPropertyEditor) { oRm.renderControl(oPropertyEditor); });
              }
      ...
      BaseEditor.prototype._prepareData = function(oJson) {
        ...
        each(this._mObservableConfig, function(sPropertyName, mPropertyConfig) {
          ...
          if (typeof ObjectPath.get(sPath.split("/"), oJsonCopy) ===
                  "undefined" &&
              typeof mPropertyConfig.defaultValue !== "undefined") {
            ObjectPath.set(sPath.split("/"),
                           deepClone(mPropertyConfig.defaultValue), oJsonCopy);
          }
        });
      ...
      BaseEditor.prototype.setJson = function(vJson) {
        ...
        if (typeof vJson === "string") {
          try {
            ...
            Log.error(
                "sap.ui.integration.designtime.baseEditor.BaseEditor: invalid JSON string is specified");
          }
          ...
          Log.error(
              "sap.ui.integration.designtime.baseEditor.BaseEditor: unsupported data type specified in setJson()");
        }

        if (oJson && JSON.stringify(this.getProperty("json")) !==
                         JSON.stringify(oJson)) {
          this.setProperty("json", oJson);
          this.fireJsonChange({json : oJson});
        }
      };

      BaseEditor.prototype.setPreventInitialization = function(
          bPreventInitialization) {
      ...
      BaseEditor.prototype.setConfig = function(oConfig, bIsDefaultConfig) {
        ...
        oConfig = oConfig || {};
        this._oSetConfigPromise =
            this._oSetConfigPromise
                .then(this._registerPropertyEditorTypes.bind(
                    this, oConfig.propertyEditors))
                .then(this._setConfig.bind(this, oConfig, bIsDefaultConfig));

        return this._oSetConfigPromise;
      };

      BaseEditor.prototype._registerPropertyEditorTypes = function(
          mPropertyEditors) {
        PropertyEditorFactory.deregisterAllTypes();
        return PropertyEditorFactory.registerTypes(mPropertyEditors || {});
      };

      BaseEditor.prototype._setConfig = function(oConfig, bIsDefaultConfig,
                                                 mPropertyEditors) {
        this._initValidators(oConfig.validators || {});

        // Backwards compatibility. If no i18n configuration specified, we use
        // default one.
        var oTarget = {propertyEditors : {}, properties : {}};
        ...
        if (this._oSpecificConfig) {
          ...
          oNewConfig =
              bIsDefaultConfig
                  ...
                  : mergeSpecificConfig(oNewConfig, this._oSpecificConfig,
                                        mPropertyEditors);
        }
        ...
        this.fireConfigChange({config : deepClone(oNewConfig)});
        ...
        this._oSetConfigPromise =
            this._oSetConfigPromise
                .then(function() {
                  ...
                  oCurrentConfig.propertyEditors = addMissingPropertyEditors(
                      oCurrentConfig, oSpecificConfig);

                  return oCurrentConfig.propertyEditors;
                }.bind(this))
                .then(this._registerPropertyEditorTypes)
                .then(function(mPropertyEditors) {
                  this._setConfig(oCurrentConfig, this._bIsDefaultConfig,
                                  mPropertyEditors);
                }.bind(this));
      ...
      function addMissingPropertyEditors(oCurrentConfig, oSpecificConfig) {
        ...
        _union(Object.keys(oCurrentEditors), Object.keys(oSpecificEditors))
            .forEach(function(sEditorName) {
              oPropertyEditors[sEditorName] =
                  oSpecificEditors[sEditorName] || oCurrentEditors[sEditorName];
            });

        return oPropertyEditors;
      }

      function mergeSpecificConfig(oCurrentConfig, oSpecificConfig,
                                   mPropertyEditors) {
        // merge i18n
        ...
        each(oCurrentConfig.properties, function(sPropertyName, oProperty) {
          var sEditor =
              oCurrentConfig.propertyEditors[oProperty.type] &&
              oCurrentConfig.propertyEditors[oProperty.type].split("/").join(
                  ".");
          var oConfigMetadata =
              sEditor && mPropertyEditors[sEditor].configMetadata;

          if (oConfigMetadata && oSpecificConfig.properties[sPropertyName]) {
            each(oProperty, function(sKey, vTargetValue) {
              var vNewValue;
              var sMergeStrategy =
                  oConfigMetadata[sKey] && oConfigMetadata[sKey].mergeStrategy;
              if (sMergeStrategy) {
                // only applicable for boolean values
                if (sMergeStrategy === "mostRestrictiveWins") {
                  var bMostRestrictiveValue =
                      oConfigMetadata[sKey].mostRestrictiveValue || false;
                  ...
                  vNewValue = _intersection(
                      vTargetValue,
                      oSpecificConfig.properties[sPropertyName][sKey]);
                }
              } else {
                vNewValue = vTargetValue;
              }
              oNewConfig.properties[sPropertyName] =
                  oNewConfig.properties[sPropertyName] || {};
      ...
      BaseEditor.prototype.setDesigntimeMetadata = function(
          oDesigntimeMetadata, bIsInitialMetadata) {
        ...
        if (!deepEqual(oNextMetadata, this.getDesigntimeMetadata())) {
          ...
          if (!bIsInitialMetadata) {
            this.fireDesigntimeMetadataChange({
              designtimeMetadata :
                  this._formatExportedDesigntimeMetadata(oNextMetadata)
            });
          }
        }
      };

      BaseEditor.prototype._formatExportedDesigntimeMetadata = function(
          oMetadata) {
        var oFlatMetadata = {};
      ...
      BaseEditor.prototype._reset = function() {
        ...
        this._aCancelHandlers.forEach(function(fnCancel) { fnCancel(); });
      ...
      BaseEditor.prototype._initialize = function() {
        ...
        if (mConfig) {
          ...
          this._loadI18nBundles(mConfig.i18n).then(function(aBundles) {
            this._oI18nModel = this._createI18nModel(aBundles);
            this.setModel(this._oI18nModel, "i18n");

            // Setup config observer
            this._oConfigObserver.addToIgnore([
              "template", "itemLabel"
            ]); // Ignore array templates and itemLabels
            this._oConfigObserver.setModel(this._oDataModel);
            this._oConfigObserver.setModel(this._oDesigntimeMetadataModel,
                                           "designtimeMetadata");
            ...
            if (sContextPath) {
              ...
              this._oConfigObserver.setBindingContext(
                  this._oDataModel.getContext(sContextPath), "context");
            }

            var mPropertiesConfig =
                resolveBinding(mConfig.properties, {"i18n" : this._oI18nModel});

            this._mObservableConfig =
                Object.assign(this._mObservableConfig,
                              this._prepareConfig(mPropertiesConfig));
      ...
      BaseEditor.prototype._onConfigChange = function(oEvent) {
        var oChangeMap = oEvent.getParameter("changes").reduce(
            function(oCurrentChangeMap, oOriginalChange) {
        ...
        var aPropertyEditors =
            Object.keys(oChangeMap).reduce(function(aList, sPropertyName) {
              var aAffectedEditors =
                  (this.getPropertyEditorsByNameSync(sPropertyName) ||
        ...
        var aIndependentEditors =
            aPropertyEditors.filter(function(oPropertyEditor) {
              return !this._oRootWrapper ||
                     !includes(this._oRootWrapper._aEditorWrappers,
                               oPropertyEditor.editor);
            }.bind(this));
        ...
        aIndependentEditors.forEach(function(oPropertyEditor) {
          ...
          var mPropertyConfig =
              _omit(deepClone(mProperties[sPropertyKey]), "value");
        ...
        if (aIndependentEditors.length < aPropertyEditors.length) {
          var aModifiedConfigs =
              deepClone(this._oRootWrapper.getConfig()).map(function(mConfig) {
       ...
       * @returns {Promise} Array of {@link sap.base.i18n.ResourceBundle i18n
       *     resource bundles}
       * @private
       */
      BaseEditor.prototype._loadI18nBundles = function(aBundlePaths) {
        return this._createPromise(function(fnResolve, fnRejected) {
          Promise
              .all(aBundlePaths.map(function(sI18nPath) {
                return new Promise(function(fnResolve, fnReject) {
                  ResourceBundle
                      .create(
                          {url : sap.ui.require.toUrl(sI18nPath), async : true})
       ...
       *
       * The i18n model is used as an interface (read-only) for property
       * editors. It is created from all i18n bundles in the merged
       * configuration. To separate properties from different bundles,
       * namespacing should be used, e.g. <code>i18n>BASE_EDITOR.PROPERTY</code>
       *
       * @param {sap.base.i18n.ResourceBundle[]} aBundles - List of i18n
       *     resource bundles
       * @returns {sap.ui.model.resource.ResourceModel} I18n model of composed
       *     bundles
       * @private
       */
      BaseEditor.prototype._createI18nModel = function(aBundles) {
        var aBundlesList = aBundles.slice();
        var oI18nModel = new ResourceModel({bundle : aBundlesList.shift()});

        oI18nModel.setDefaultBindingMode("OneWay");

        aBundlesList.forEach(function(
            oBundle) { oI18nModel.enhance(oBundle); });
      ...
      BaseEditor.prototype._prepareConfig = function(mProperties) {
        ...
        each(mProperties, function(sKey, mPropertyConfig) {
          mResult[sKey] =
              Object.assign({}, this._preparePropertyConfig(mPropertyConfig),
                            {__propertyName : sKey});
        }.bind(this));
      ...
      BaseEditor.prototype._createEditors = function(mPropertiesConfig) {
        var vLayoutConfig =
            ObjectPath.get([ "layout", this.getLayout() ], this.getConfig());

        if (isPlainObject(vLayoutConfig) || Array.isArray(vLayoutConfig)) {
          vLayoutConfig =
              resolveBinding(vLayoutConfig, {"i18n" : this._oI18nModel});
        }
        ...
        return (Promise
                    .all(values(this._mPropertyEditors)
                             .reduce(
                                 function(aResult, aCurrent) {
      ...
      BaseEditor.prototype._getRegistrationKey = function(oPropertyEditor,
                                                          sKey) {
        if (typeof sKey !== "string") {
          if (oPropertyEditor.isA(
       ...
       * @param {sap.ui.integration.designtime.baseEditor.PropertyEditor|sap.ui.integration.designtime.baseEditor.propertyEditor.BasePropertyEditor}
       *     oPropertyEditor - Property editor instance
       * @param {string} [sKey] - Optional key. The property editor will be
       *     available in {@link #getPropertyEditorSync} under the specified
       *     key.
       */
      BaseEditor.prototype.registerPropertyEditor = function(oPropertyEditor,
                                                             sKey) {
        sKey = this._getRegistrationKey(oPropertyEditor, sKey);
        var aList = Array.isArray(this._mPropertyEditors[sKey])
                        ? this._mPropertyEditors[sKey]
                        : [];
        ...
        var vValue =
            ObjectPath.get([ sKey ], this._oConfigObserver.getObject()).value;

        oPropertyEditor.setValue(vValue);
        oPropertyEditor.attachValueChange(this._onValueChange, this);
        oPropertyEditor.attachDesigntimeMetadataChange(
            this._onDesigntimeMetadataChange, this);
       ...
       * @param {sap.ui.integration.designtime.baseEditor.PropertyEditor|sap.ui.integration.designtime.baseEditor.propertyEditor.BasePropertyEditor}
       *     oPropertyEditor - Property editor instance
       * @param {string} [sKey] - Optional key which was used for registration.
       *     See {@link #registerPropertyEditor}.
       */
      BaseEditor.prototype.deregisterPropertyEditor = function(oPropertyEditor,
                                                               sKey) {
        sKey = this._getRegistrationKey(oPropertyEditor, sKey);
        ...
        oPropertyEditor.detachDesigntimeMetadataChange(
            this._onDesigntimeMetadataChange, this);

        if (Array.isArray(aList)) {
          this._mPropertyEditors[sKey] = aList.filter(function(
              oItem) { return oPropertyEditor !== oItem; });
      ...
      BaseEditor.prototype._setReady = function(bReadyState) {
        ...
        if (bPreviousReadyState !== true && bReadyState === true) {
          ...
          this.firePropertyEditorsReady(
              {propertyEditors : this.getPropertyEditorsSync()});
        }
      };
      ...
      BaseEditor.prototype._checkReady = function() {
        ...
        // Workaround to support first-level of async dependencies of BaseEditor
        // ¯\_(ツ)_/¯
        aLayoutDependecies.forEach(function(oEditor) {
          if (!sap.ui.base.EventProvider.hasListener(oEditor, "ready",
                                                     this._checkReady, this)) {
            oEditor.attachReady(this._checkReady, this);
          }
        }, this);

        var aAsyncDependencies =
            [].concat(aLayoutDependecies, this.getPropertyEditorsSync());

        var bIsReady = (this._bInitFinished && this._bValidatorsReady &&
                        aAsyncDependencies.every(function(
                            oEditor) { return oEditor.isReady(); }));
        this._setReady(bIsReady);
      };

      BaseEditor.prototype.isReady = function() { return this._bIsReady; };

      /**
       * Wait for the BaseEditor to be ready.
       * @returns {Promise} Promise which will resolve once the editor is ready.
       *     Resolves immediately if the editor is currently ready.
       */
      ...
      BaseEditor.prototype._createPromise = function(fn) {
        ...
        var removeHandler = function(fnCancel, vResolvedValue) {
          this._aCancelHandlers = this._aCancelHandlers.filter(function(
              fn) { return fn !== fnCancel; });
       ...
       * @param {string} sPropertyName - Property name in the editor
       *     configuration
       * @returns {object} Configuration object, e.g. <code>{ path: "/foo",
       *     label: "Foo", ... }</code>
       */
      BaseEditor.prototype.getPropertyConfigByName = function(sPropertyName) {
        return _omit(ObjectPath.get([ sPropertyName ],
                                    this._oConfigObserver.getObject()),
                     "value");
      };

      /**
       * Returns a list of property editors corresponding to a specified
       * property name.
       * @param {string} sPropertyName - Property name
       * @returns {Promise<sap.ui.integration.designtime.baseEditor.PropertyEditor[]|sap.ui.integration.designtime.baseEditor.propertyEditor.BasePropertyEditor[]|null>}
       *     List of property editors for the specified name
       */
      BaseEditor.prototype.getPropertyEditorsByName = function(sPropertyName) {
        return new Promise(function(fnResolve) {
                 if (!this._mPropertyEditors ||
                     Object.keys(this._mPropertyEditors).length === 0) {
       ...
       * property name.
       * @param {string} sPropertyName - Property name
       * @returns {sap.ui.integration.designtime.baseEditor.PropertyEditor[]|sap.ui.integration.designtime.baseEditor.propertyEditor.BasePropertyEditor[]|null}
       *     List of property editors for the specified name
       */
      BaseEditor.prototype.getPropertyEditorsByNameSync = function(
          sPropertyName) {
      ...
      /**
       * Returns a list of property editors corresponding to a specified list of
       * tags.
       * @param {string|string[]} vTag - List of tags
       * @returns {Promise<sap.ui.integration.designtime.baseEditor.PropertyEditor[]|sap.ui.integration.designtime.baseEditor.propertyEditor.BasePropertyEditor[]|null>}
       *     List of property editors for the specified tags
       */
      BaseEditor.prototype.getPropertyEditorsByTag = function(vTag) {
        return new Promise(function(fnResolve) {
            ...
            .then(function() { return this.getPropertyEditorsByTagSync(vTag); }
                      .bind(this));
      };

      /**
       * Returns list of configurations for properties which match the specified
       * list of the tags.
       * @param {string|string[]} vTag - Tag or an array of tags.
       * @returns {object[]} Array of the configuration objects which match all
       *     the specified tags
       */
      BaseEditor.prototype.getConfigsByTag = function(vTag) {
        ...
        return Object.keys(aProperties)
            ...
            .map(function(
                sPropertyName) { return aProperties[sPropertyName]; });
      };
      ...
      BaseEditor.prototype.getPropertyEditorsSync = function() {
        return values(this._mPropertyEditors)
            .reduce(function(aResult,
                             aCurrent) { return aResult.concat(aCurrent); },
                    [])
            .sort(function(oPropertyEditor1, oPropertyEditor2) {
              return parseInt(oPropertyEditor1.getId().match(/\d+$/)) -
                     parseInt(oPropertyEditor2.getId().match(/\d+$/));
            });
      };

      BaseEditor.prototype.getJson = function() {
        return _merge(
            {}, this.getProperty("json")); // To avoid manipulations with the
                                           // json outside of the editor
      };
      ...
      BaseEditor.prototype._onValueChange = function(oEvent) {
        ...
        if (sPath[0] === "/") {
          ...
          throw new Error(
              "BaseEditor._onValueChange: unknown relative path - '" + sPath +
              "'");
        }

        var aParts = sPath.split("/");

        ObjectPath.set(aParts, vValue, oJson);

        if (typeof vValue === "undefined" ||
            deepEqual(vValue,
                      oPropertyEditor.getRuntimeConfig().defaultValue) ||
      ...
      BaseEditor.prototype._onDesigntimeMetadataChange = function(oEvent) {
        ...
        if (sPath[0] === "/") {
          ...
          throw new Error(
              "BaseEditor._onDesigntimeMetadataChange: unknown relative path - '" +
              sPath + "'");
        }

        var aParts = sPath.split("/");

        ObjectPath.set(aParts, vValue, oDesigntimeMetadata);

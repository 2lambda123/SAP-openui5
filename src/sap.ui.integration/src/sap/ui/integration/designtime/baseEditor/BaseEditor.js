...
sap.ui.define(
    ...
    function (
            ...
            "sap.ui.integration.designtime.baseEditor.BaseEditor",
            {
                ...
                renderer: function (oRm, oControl) {
                    ...
                    if (aContent.length) {
                        ...
                        oControl
                            .getPropertyEditorsSync()
                            .forEach(function (oPropertyEditor) {
        ...
        BaseEditor.prototype._prepareData = function (oJson) {
            ...
            each(
                ...
                function (sPropertyName, mPropertyConfig) {
                    ...
                    if (
                        typeof ObjectPath.get(sPath.split("/"), oJsonCopy) ===
                            "undefined" &&
        ...
        BaseEditor.prototype.setJson = function (vJson) {
            ...
            if (
                oJson &&
                JSON.stringify(this.getProperty("json")) !==
                    ...
                    if (
                        ...
                        each(oProperty, function (sKey, vTargetValue) {
                            ...
                            if (sMergeStrategy) {
                                ...
                                if (sMergeStrategy === "mostRestrictiveWins") {
                                    var bMostRestrictiveValue =
                                        oConfigMetadata[sKey]
                                            .mostRestrictiveValue || false;
                                    if (
                                        vTargetValue === bMostRestrictiveValue
                                    ) {
                            ...
                            oNewConfig.properties[sPropertyName] =
                                oNewConfig.properties[sPropertyName] || {};
                            oNewConfig.properties[sPropertyName][sKey] =
                                vNewValue;
                        });
                    }
                },
            ...
            if (!deepEqual(oNextMetadata, this.getDesigntimeMetadata())) {
                ...
                if (!bIsInitialMetadata) {
                    this.fireDesigntimeMetadataChange({
                        designtimeMetadata:
                            this._formatExportedDesigntimeMetadata(
                                oNextMetadata,
                            ),
                    });
                }
        ...
        BaseEditor.prototype._initialize = function () {
            ...
            if (mConfig) {
                ...
                this._loadI18nBundles(mConfig.i18n).then(
                    function (aBundles) {
        ...
        BaseEditor.prototype._onConfigChange = function (oEvent) {
            var oChangeMap = oEvent.getParameter("changes").reduce(function (
                oCurrentChangeMap,
                oOriginalChange,
            ) {
            ...
            var aPropertyEditors = Object.keys(oChangeMap).reduce(
            ...
            if (aIndependentEditors.length < aPropertyEditors.length) {
                var aModifiedConfigs = deepClone(
        ...
        BaseEditor.prototype.getPropertyEditorsSync = function () {
            return values(this._mPropertyEditors)
                ...
                .sort(function (oPropertyEditor1, oPropertyEditor2) {
                    return (
                        parseInt(oPropertyEditor1.getId().match(/\d+$/)) -
        ...
        BaseEditor.prototype.getJson = function () {
            return _merge({}, this.getProperty("json")); // To avoid manipulations with the
        ...
        BaseEditor.prototype._onValueChange = function (oEvent) {
            ...
            if (sPath[0] === "/") {
                ...
                throw new Error(
                    "BaseEditor._onValueChange: unknown relative path - '" +
                        sPath +
                        "'",
                );
            }

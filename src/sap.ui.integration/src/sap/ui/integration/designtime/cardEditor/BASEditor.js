...
sap.ui.define(
    [
      ...
      "sap/base/util/restricted/_isEqual", "sap/base/util/restricted/_omit",
      "sap/base/util/merge", "sap/base/util/deepClone",
      "sap/base/util/deepEqual", "sap/base/util/ObjectPath", "./CardEditor",
      "sap/ui/integration/designtime/baseEditor/BaseEditor",
      "sap/ui/integration/Designtime", "sap/ui/integration/util/CardMerger",
      "sap/base/util/LoaderExtensions", "sap/base/Log"
    ],
    function(CancelablePromise, _isEqual, _omit, merge, deepClone, deepEqual,
             ObjectPath, CardEditor, BaseEditor, Designtime, CardMerger,
             LoaderExtensions, Log) {
      "use strict";

      var configurationTemplate = "";
      configurationTemplate = LoaderExtensions.loadResource(
          "sap/ui/integration/designtime/cardEditor/ConfigurationTemplate.js",
          {dataType : "text", failOnError : false, async : false});
       ...
       * @experimental
       */
      var BASEditor = CardEditor.extend(
      ...
      BASEditor.prototype.getManifest =
          function() { return this._oCurrent.manifest; };

      BASEditor.prototype.getConfigurationClass =
          function() { return this._oCurrent.configurationclass; };

      BASEditor.prototype.getConfiguration =
          function() { return this._oCurrent.configuration; };

      BASEditor.prototype.getConfigurationString =
          function() { return this._oCurrent.configurationstring; };

      BASEditor.prototype._generateDesigntimeJSConfig = function() {
        var oMetadata = this._formatExportedDesigntimeMetadata(
        ...
        this._eventTimeout = setTimeout(function() {
          var oEmptyItems = {form : {items : {}}};
          var oCopyConfig = merge(oEmptyItems, this._oDesigntimeJSConfig);
          var oNewItems = {};
          var aItems = [];
          var oItem;
          if (!ObjectPath.get([ "sap.card", "configuration" ], oJson)) {
            ObjectPath.set([ "sap.card", "configuration" ], {"parameters" : {}},
                           oJson);
          } else if (!ObjectPath.get(
                         [ "sap.card", "configuration", "parameters" ],
                         oJson)) {
            ObjectPath.set([ "sap.card", "configuration", "parameters" ], {},
                           oJson);
          }
          var mParameters = ObjectPath.get(
              [ "sap.card", "configuration", "parameters" ], oJson);
          if (oJson) {
            // parameters content changed added, removed
            var mParametersInDesigntime =
                ObjectPath.get([ "sap.card", "configuration", "parameters" ],
                               this._oDesigntimeMetadataModel.getData());
            if (deepEqual(mParameters, {}) && !mParametersInDesigntime) {
              ...
              this._oCurrent = {
                ...
                configurationstring :
                    this._cleanConfig(this._oDesigntimeJSConfig, true)
              };
            ...
            for (var n in oCopyConfig.form.items) {
              ...
              if (!mParameters[n]) {
                if (mParametersInDesigntime[n]) {
                  if (oItem.type === "group") {
                    mParameters[n] = {};
                  } else if (oItem.manifestpath &&
                             !oItem.manifestpath.startsWith(
                                 "/sap.card/configuration/parameters")) {
                  ...
                  // delete the item because it is not part of parameters
                  // anymore
              ...
              if (oItem.type === "group") {
                ...
                mParametersInDesigntime[n].manifestpath =
                    "/sap.card/configuration/parameters/" + n + "/value";
                mParametersInDesigntime[n].__value.manifestpath =
                    "/sap.card/configuration/parameters/" + n + "/value";
                oCopyConfig.form.items[n].manifestpath =
                    "/sap.card/configuration/parameters/" + n + "/value";
              }
            }
            if (aCurrentKeys.length > 0) {
              ...
              for (var i = 0; i < aCurrentKeys.length; i++) {
                ...
                oCopyConfig.form.items[sNewItem] = {
                  manifestpath : "/sap.card/configuration/parameters/" +
                                     sNewItem + "/value",
              ...
              mParameters[sNewItem] = merge(oCopyConfig.form.items[sNewItem],
                                            mParameters[sNewItem]);
            }
          }
          if (oMetadata) {
            if (oCopyConfig) {
          ...
          this._fnDesigntime = function(o) { return new Designtime(o); }.bind(
              this, oCleanConfig);
          this._oCurrent = {
      ...
      BASEditor.prototype._cleanJson = function(oJson, bCleanParameters) {
        ...
        var sDesigntimePath = sanitizePath(
            ObjectPath.get([ "sap.card", "designtime" ], oJson) || "");
        if (!sDesigntimePath) {
          ObjectPath.set([ "sap.card", "designtime" ], "dt/configuration",
                         oJson);
        }
        ...
        if (bCleanParameters) {
          ...
          for (var n in mParameters) {
            ...
            if (this._oDesigntimeJSConfig && this._oDesigntimeJSConfig.form &&
                this._oDesigntimeJSConfig.form.items) {
              ...
              if (oParamConfig.manifestpath &&
                  !oParamConfig.manifestpath.startsWith(
                      "/sap.card/configuration/parameters")) {
            ...
            mParameters[n] = {value : mParameters[n].value};
          }
        }
        if (this._i18n) {
      ...
      BASEditor.prototype._cleanConfig = function(oConfig, bString) {
        ...
        if (bString) {
          ...
          sConfig = sConfig.replace(
              /\"\$\$([a-zA-Z]*)\$\$\"/g,
              function(s) { return s.substring(3, s.length - 3); });
          return sConfig;
        }
        return oConfig;
      };
      ...
      BASEditor.prototype._generateMetadataFromJSConfig = function(
        ...
        if (oDesigntimeJSConfig) {
          this._oDesigntimeJSConfig =
              merge(this._oDesigntimeJSConfig, oDesigntimeJSConfig);
        }
        if (this._oDesigntimeJSConfig) {
          ...
          for (var n in mItems) {
            ...
            if (oMetaItem.manifestpath &&
                (!oMetaItem.manifestpath.startsWith(
                     "/sap.card/configuration/parameters/") ||
                 !ObjectPath.get(aPath, this._oInitialJson))) {
            ...
            if (!oMetaItem.hasOwnProperty("type")) {
              this.fireError({
                ...
                "detail" : {"message" : "Type of parameter " + n + " not exist"}
              });
            ...
            if (oMetaItem.type !== "group") {
              ...
              if (ObjectPath.get(aPath, this._oInitialJson)) {
                if (ObjectPath.get(aPath, this._oInitialJson).value ===
                    undefined) {
                  // set the value from the metadata to the original data
                  ObjectPath.get(aPath, this._oInitialJson).value =
                      oMetaItem.value;
                }
              }
            }
      ...
      BASEditor.prototype.setJson = function(oJson) {
        ...
        if (!this.__generateDesigntimeJSConfigAttached) {
          this.attachDesigntimeMetadataChange(
              this._generateDesigntimeJSConfig.bind(this));
        ...
        if (!this._bDesigntimeInit) {
          ...
          if (!sDesigntimePath) {
            ...
            // sDesigntime = sDesigntime.replace(/\$\$CARDID\$\$/, sCardId +
            // ".Configuration");
            ObjectPath.set([ "sap.card", "designtime" ], "dt/configuration",
                           oJson);
            sTempDesigntimeUrl =
                "sap/ui/integration/designtime/cardEditor/ConfigurationTemplate";
          ...
          if (sBaseUrl && sDesigntimePath) {
            ...
            var sDesigntimeRelativePath =
                trimCurrentFolderPrefix(sDesigntimePath);
            var sDesigntimeAbsolutePath =
                sSanitizedBaseUrl + "/" + sDesigntimeRelativePath;
            var sNamespace =
                sCardId.replace(/\./g, "/") + "/" + sDesigntimeRelativePath;

            mPaths[sNamespace] = sDesigntimeAbsolutePath;
            mPaths[sNamespace + "js"] = sDesigntimeAbsolutePath.substring(
                0, sDesigntimeAbsolutePath.lastIndexOf("/"));
            var sFileName = sDesigntimeAbsolutePath.replace(
                mPaths[sNamespace + "js"] + "/", "");
            ...
            // var sDesigntimeMetadataPath = sDesigntimeAbsolutePath +
            // "/metadata.json";
            var that = this;
            this._oDesigntimePromise = new CancelablePromise(function(
                fnResolve) {
              var sUrl = sNamespace + "js" +
                         "/" + sFileName + ".js";
              if (sTempDesigntimeUrl) {
                sUrl = sTempDesigntimeUrl + ".js";
              }
              sap.ui.loader._.loadJSResourceAsync(sUrl)
                  .then(function(DesigntimeClass) {
                    if (!DesigntimeClass) {
                  ...
                  .catch(function(o) {
                    ...
                    Log.error(o);
                    that.fireError({"name" : "Designtime Error", "detail" : o});
                  });
            });
            ...
            this._oDesigntimePromise.then(function(oMetadata) {
              ...
              oDesigntimeMetadata = CardMerger.mergeCardDesigntimeMetadata(
                  oDesigntimeMetadata, this.getDesigntimeChanges());

              this._oInitialDesigntimeMetadata = oDesigntimeMetadata;
              this.setDesigntimeMetadata(
                  formatImportedDesigntimeMetadata(oDesigntimeMetadata), true);
      ...
      BASEditor.prototype.getConfigurationTemplate =
          function() { return configurationTemplate; };

      BASEditor.prototype.updateDesigntimeMetadata = function(
          oDesigntimeJSConfig, bIsInitialMetadata) {
        var oDesigntimeMetadata =
            this._generateMetadataFromJSConfig(oDesigntimeJSConfig);
        this._oInitialDesigntimeMetadata = oDesigntimeMetadata;
        this.setDesigntimeMetadata(
            formatImportedDesigntimeMetadata(oDesigntimeMetadata),
            bIsInitialMetadata);
      };
      ...
      function formatImportedDesigntimeMetadata(oFlatMetadata) {
        ...
        Object.keys(oFlatMetadata).forEach(function(sPath) {
          ObjectPath.set(sPath.split("/"),
                         {__value : deepClone(oFlatMetadata[sPath])},
                         oFormattedMetadata);
        });
        return oFormattedMetadata;
      }
      function sanitizePath(sPath) { return sPath.trim().replace(/\/*$/, ""); }

...
sap.ui.define(
    [
      "sap/base/util/isEmptyObject", "sap/f/cards/Header",
      "sap/f/cards/HeaderRenderer", "sap/m/library",
      "sap/ui/integration/util/BindingHelper", 'sap/ui/model/json/JSONModel',
      "sap/ui/integration/util/LoadingProvider"
    ],
    function(isEmptyObject, FHeader, FHeaderRenderer, mLibrary, BindingHelper,
             JSONModel, LoadingProvider) {
      "use strict";
      ...
      var Header = FHeader.extend("sap.ui.integration.cards.Header", {
        ...
        constructor : function(mConfiguration, oActionsToolbar, sAppId,
          ...
          if (mConfiguration.status && mConfiguration.status.text &&
          ...
          if (mConfiguration.icon) {
            ...
            mSettings.iconBackgroundColor =
                mConfiguration.icon.backgroundColor ||
                (mConfiguration.icon.text ? AvatarColor.Accent6
                                          : AvatarColor.Transparent);
          }

          if (mSettings.iconSrc) {
            mSettings.iconSrc = BindingHelper.formattedProperty(
                mSettings.iconSrc, function(sValue) {
       ...
       */
      Header.prototype.isReady = function() { return this._bReady; };

      Header.prototype.isLoading = function() {
        var oLoadingProvider = this._oLoadingProvider, oCard = this.getParent(),
            cardLoading = oCard.getMetadata()._sClassName ===
                                  'sap.ui.integration.widgets.Card'
                              ? oCard.isLoading()
                              : false;

        return !oLoadingProvider.getDataProviderJSON() &&
               (oLoadingProvider.getLoadingState() || cardLoading);
      };

      Header.prototype._updateModel = function(
          oData) { this.getModel().setData(oData); };
      ...
      Header.prototype._awaitEvent = function(sEvent) {
        this._aReadyPromises.push(new Promise(function(resolve) {
          this.attachEventOnce(sEvent, function() { resolve(); });
        }.bind(this)));
      };
      ...
      Header.prototype._setDataConfiguration = function(oDataSettings) {
        ...
        this._oDataProvider = this._oDataProviderFactory.create(
            oDataSettings, this._oServiceManager);
        ...
        if (this._oDataProvider) {
          ...
          this._oDataProvider.attachDataRequested(
              function() { this.onDataRequested(); }.bind(this));

          // TODO Designers to decide if we have to keep loading status when an

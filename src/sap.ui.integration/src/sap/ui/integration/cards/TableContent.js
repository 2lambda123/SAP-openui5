...
sap.ui.define(
    [
      "./BaseListContent", "./TableContentRenderer",
      "sap/ui/integration/library", "sap/m/Table", "sap/m/Column",
      "sap/m/ColumnListItem", "sap/m/Text", "sap/m/Link",
      "sap/m/ProgressIndicator", "sap/m/ObjectIdentifier", "sap/m/ObjectStatus",
      "sap/m/Avatar", "sap/ui/core/library", "sap/m/library",
      "sap/ui/integration/util/BindingResolver",
      "sap/ui/integration/util/BindingHelper", "sap/ui/util/openWindow"
    ],
    function(BaseListContent, TableContentRenderer, library, ResponsiveTable,
             Column, ColumnListItem, Text, Link, ProgressIndicator,
             ObjectIdentifier, ObjectStatus, Avatar, coreLibrary, mobileLibrary,
             BindingResolver, BindingHelper, openWindow) {
      "use strict";
       ...
       */
      var TableContent =
          BaseListContent.extend("sap.ui.integration.cards.TableContent", {
      ...
      /**
       * Setter for configuring a
       * <code>sap.ui.integration.cards.TableContent</code>.
       *
       * @public
       * @param {Object} oConfiguration Configuration object used to create the
       *     internal table.
       * @returns {sap.ui.integration.cards.TableContent} Pointer to the control
      ...
      /**
       * Binds/Sets properties to the inner item template based on the
       * configuration object row template which is already parsed. Attaches all
       * required actions.
       *
       * @private
      ...
      TableContent.prototype._setColumns = function(oRow) {
        var aCells = [], oTable = this._getTable(), aColumns = oRow.columns;
        ...
        this._oItemTemplate =
            new ColumnListItem({cells : aCells, vAlign : VerticalAlign.Middle});

        this._oActions.setAreaType(AreaType.ContentItem);
        this._oActions.attach(oRow, this);

        var oBindingInfo = {template : this._oItemTemplate};
      ...
      TableContent.prototype._setStaticColumns = function(aRows, aColumns) {
        ...
        aRows.forEach(function(oRow) {
          var oItem = new ColumnListItem({vAlign : VerticalAlign.Middle});
      ...
      /**
       * Factory method that returns a control from the correct type for each
       * column.
       *
      ...
      TableContent.prototype._createCell = function(oColumn) {
        ...
        if (oColumn.identifier) {
          ...
          if (oColumn.identifier.url) {
            vTitleActive = BindingHelper.formattedProperty(
                oColumn.identifier.url, function(sValue) {
          ...
          var oIdentifier = new ObjectIdentifier(
              {title : oColumn.value, titleActive : vTitleActive});

          if (oColumn.identifier.url) {
            // TO DO: move this part to CardActions
            oIdentifier.attachTitlePress(function(oEvent) {
              var oSource = oEvent.getSource(),
                  oBindingContext = oSource.getBindingContext(), sPath, sUrl,
                  sTarget;
              ...
              sUrl = BindingResolver.resolveValue(oColumn.identifier.url,
                                                  oSource, sPath);
              sTarget = BindingResolver.resolveValue(oColumn.identifier.target,
                                                     oSource, sPath);
        ...
        if (oColumn.state) {
          return new ObjectStatus(
              {text : oColumn.value, state : oColumn.state});
        }

        if (oColumn.value) {
          return new Text({text : oColumn.value});
        }

        if (oColumn.icon) {
          var vSrc = BindingHelper.formattedProperty(
              oColumn.icon.src, function(sValue) {
          ...
          return new Avatar({
                   ...
                   backgroundColor :
                       oColumn.icon.backgroundColor ||
                           (oColumn.icon.text ? undefined
       ...
       */
      TableContent.prototype.getInnerList =
          function() { return this._getTable(); };

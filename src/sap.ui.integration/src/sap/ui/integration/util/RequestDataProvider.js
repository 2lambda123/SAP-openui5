...
sap.ui.define(
    [
      "sap/ui/integration/util/DataProvider", "jquery.sap.global",
      "sap/base/Log"
    ],
    function(DataProvider, jQuery, Log) {
      ...
      var RequestDataProvider =
          DataProvider.extend("sap.ui.integration.util.RequestDataProvider", {

            metadata : {

              /**
               * Defines whether it's possible to later provide a dataType
               * property to the Request Configuration object, which declares
      ...
      /**
       * @override
       * @returns {Promise} A promise resolved when the data is available and
      ...
      RequestDataProvider.prototype._fetch = function(oRequestConfig) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          if (!this.getAllowCustomDataType() && oRequestConfig.dataType) {
            Log.error(
                "To specify dataType property in the Request Configuration, first set allowCustomDataType to 'true'.");
          }

          var vData = oRequestConfig.parameters;

          // if not 'application/x-www-form-urlencoded', data has to be
          // serialized manually
          if (this._hasHeader(oRequestConfig, "Content-Type",
                              "application/json")) {
          ...
          var oRequest = {
            ...
            "method" : (oRequestConfig.method &&
                        oRequestConfig.method.toUpperCase()) ||
                           "GET",
            "dataType" :
                (this.getAllowCustomDataType() && oRequestConfig.dataType) ||
                    "json",
            "data" : vData,
          ...
          if (this._isValidRequest(oRequest)) {
            jQuery.ajax(oRequest)
                .done(function(oData) { resolve(oData); })
                .fail(function(jqXHR, sTextStatus, sError) { reject(sError); });
          } else {
            Log.error(sMessage);
            reject(sMessage);
          }
       ...
       * Header name is case-insensitive, but the value is case-sensitive
       ...
       */
      RequestDataProvider.prototype._hasHeader = function(oRequestConfig,
                                                          sHeader, sValue) {
        ...
        for (var sKey in oRequestConfig.headers) {
          if (sKey.toLowerCase() === sHeader.toLowerCase() &&
              oRequestConfig.headers[sKey] === sValue) {
            return true;
          }
        }

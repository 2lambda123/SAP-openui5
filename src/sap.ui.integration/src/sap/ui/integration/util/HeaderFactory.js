...
sap.ui.define(
    [
      "./BaseFactory", "./CardActions", "sap/ui/integration/library",
      "sap/ui/integration/cards/NumericHeader",
      "sap/ui/integration/cards/Header", "sap/base/strings/formatMessage"
    ],
    function(BaseFactory, CardActions, library, NumericHeader, Header,
             formatMessage) {
      "use strict";
      ...
      function bindStatusText(mFormat, oHeader) {
        ...
        if (mFormat.parts && mFormat.translationKey &&
            mFormat.parts.length === 2) {
          var oBindingInfo = {
            parts : [
              mFormat.translationKey, mFormat.parts[0].toString(),
       ...
       */
      var HeaderFactory =
          BaseFactory.extend("sap.ui.integration.util.HeaderFactory");

      HeaderFactory.prototype.create = function(mConfiguration, oToolbar) {
        var oHeader, oCard = this._oCard, oActions;

        mConfiguration = this.createBindingInfos(mConfiguration,
                                                 oCard.getBindingNamespaces());

        oActions = new CardActions({card : oCard, areaType : AreaType.Header});
        ...
        case "Numeric":
          ...
          break;
        default:
          oHeader = new Header(mConfiguration, oToolbar, oCard._sAppId,
                               oCard._oIconFormatter);
          break;
        }

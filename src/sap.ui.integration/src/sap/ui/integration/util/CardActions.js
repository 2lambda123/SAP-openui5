...
sap.ui.define(
    ...
    function(
      ...
      var AreaType = library.AreaType, CardActionType = library.CardActionType,
          ListType = mLibrary.ListType;

      /**
       * Constructor for a new <code>CardActions</code>.
       *
       * @param {string} [sId] ID for the new data provider, generated
       ...
       */
      var CardActions = ManagedObject.extend(
          "sap.ui.integration.util.CardActions",
          {
            metadata : {
              ...
              properties : {
                card : {type : "object"},
                areaType : {
                  type : "sap.ui.integration.AreaType",
                  defaultValue : AreaType.None,
                },
              },
            },
          },
      ...
      CardActions.prototype.exit = function() { this._oAreaControl = null; };
      ...
      CardActions.prototype.attach = function(mItem, oAreaControl) {
        ...
        if (oAction && oAction.type) {
          // todo - check if the type is valid
          this._attachAction(mItem, oAction);
        } else {
          // For now firing the event here, after refactor need to think of a
          // way to sync async navigation setters
          this._fireActionReady();
        }
      };

      CardActions.prototype._setItemTemplateTypeFormatter = function(
          oAction,
      ) {
        var that = this, oAreaControl = that._oAreaControl,
        ...
        // if the list item context is a correct navigation target (decided by
        // the navigation service).
        oBindingInfo.formatter = function(vValue) {
          var oBindingContext = this.getBindingContext(), sPath, mParameters;

          if (oBindingContext) {
            sPath = oBindingContext.getPath();
          }

          mParameters = BindingResolver.resolveValue(
              oAction.parameters,
              oAreaControl,
              sPath,
          );
          ...
          if (!vValue.__promise) {
            ...
            oAreaControl._oServiceManager
                .getService(_getServiceName(oAction.service))
                .then(function(oNavigationService) {
                  if (oNavigationService) {
                    oNavigationService.enabled({parameters : mParameters})
      ...
      CardActions.prototype._setSingleActionEnabledState = function(
          mItem,
          oAction,
      ) {
        var oAreaControl = this._oAreaControl,
            oBindingContext = oAreaControl.getBindingContext(), mParameters,
            sPath;
        ...
        return new Promise(function(resolve) {
          oAreaControl._oServiceManager
              ...
              .then(function(oNavigationService) {
                if (oNavigationService) {
                  oNavigationService.enabled({parameters : mParameters})
                      .then(function(bEnabled) { resolve(bEnabled); })
      ...
      /**
       * Sets 'type' property of the list item template or binds it with a
       * formatter.
       *
       * @param {object} oAction The action object which contains binding infos.
       */
      CardActions.prototype._setItemTemplateEnabledState = function(
          oAction,
      ) {
        var oBindingInfo, sType,
        ...
        if (oBindingInfo) {
          ...
          sType = oAction.enabled === false || oAction.enabled === "false"
                      ? ListType.Inactive
                      : ListType.Navigation;
      ...
      CardActions.prototype._handleServiceAction = function(
          oSource,
          oAction,
      ) {
        var oBindingContext = oSource.getBindingContext(), sPath;

        if (oBindingContext) {
          sPath = oBindingContext.getPath();
        }

        this._oAreaControl._oServiceManager
            .getService(_getServiceName(oAction.service))
            .then(function(oService) {
              if (oService) {
                oService.navigate({
                  // only for navigation?
                  parameters : BindingResolver.resolveValue(
                      oAction.parameters,
                      oSource,
                      sPath,
                      ),
            ...
            .catch(function(
                e) { Log.error("Navigation service unavailable", e); })
            .finally(
                function() { this._processAction(oSource, oAction, sPath); }
                    .bind(this),
            );
      };
      ...
      CardActions.prototype._attachPressEvent = function(
          oActionControl,
          oAction,
          bSingleAction,
      ) {
        oActionControl.attachPress(
            function(oEvent) {
      ...
      CardActions.prototype._attachAction = function(mItem, oAction) {
        var oActionControl = this.getAreaType() === AreaType.ContentItem
                                 ? this._oAreaControl._oItemTemplate
                                 : this._oAreaControl,
            bCheckEnabledState = true, sAreaType = this.getAreaType(),
            bSingleAction =
                sAreaType === AreaType.Header || sAreaType === AreaType.Content,
        ...
        if (oAction.service) {
          ...
          // When there is a list item template handle the "enabled" state with
          // bindProperty + formatter
          bCheckEnabledState = false;
        }

        if (bSingleAction && oAction.service) {
          this._setSingleActionEnabledState(mItem, oAction)
              .then(
                  function(bEnabled) {
                    if (bEnabled) {
          ...
          if (bCheckEnabledState) {
            // Handle the "enabled" state when there is no service and item
            // template with formatter.
            bActionEnabled =
                oAction.enabled !== false && oAction.enabled !== "false";
          }
      ...
      CardActions.prototype._processAction = function(
          oSource,
          oAction,
          sPath,
      ) {
        var oHost = this._getHostInstance(), oCard = this.getCard(),
            sUrl = oAction.url;
      ...
      CardActions.prototype.fireAction = function(
          oSource,
          sType,
          mParameters,
      ) {
        var oHost = this._getHostInstance(), oCard = this.getCard(),
            oActionHandlingConfiguration =
                this._extractActionConfigurations(oCard, mParameters),
            oEventData = {
              card : oCard,
              host : oHost,
              action : {type : sType},
      ...
      CardActions.fireAction = function(mConfig) {
        var oHost = mConfig.host, oCard = mConfig.card,
            oExtension = oCard.getAggregation("_extension"),
            oAction = mConfig.action, mParameters = mConfig.parameters || {},
            mActionParams = {
              type : oAction.type,
              card : oCard,
      ...
      CardActions._doPredefinedAction = function(mConfig) {
        var oAction = mConfig.action, mParameters = mConfig.parameters,
            sType = oAction.type, sUrl, sTarget, sParametersUrl,
            sParametersTarget;
        ...
        case CardActionType.Submit:
          if (mConfig.source && mConfig.source.isA(
       ...
       * @private
       */
      CardActions.openUrl = function(sUrl,
                                     sTarget) { openWindow(sUrl, sTarget); };
      ...
      CardActions.handleSubmitAction = function(mConfig) {
        var oDataProvider, oCard = mConfig.card,
        ...
        oDataProvider = oDataProviderFactory.create({
          request : oActionParameters.configuration,
        });
        ...
        oDataProvider.getData()
            .then(
                ...
                function(oError) {
                  Log.error(oError);
                  oBaseContentInstance.onActionSubmitEnd(null, {
                    error : oError,
                  });
                },
                )
       ...
       * @private
       */
      CardActions.prototype._extractActionConfigurations = function(
          oCard,
          mParameters,
      ) {
        var oRequestConfig =
                oCard && oCard.getManifestEntry(
                             "/sap.card/configuration/actionHandlers/submit",
                             ),

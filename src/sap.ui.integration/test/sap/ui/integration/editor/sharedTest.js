...
var baseUrl = document.location.pathname.substring(
        0, document.location.pathname.lastIndexOf("/") + 1),
    localStorageKey = document.querySelector("script[localstoragekey]")
                          .getAttribute("localstoragekey"),
    manifest = {
      "sap.app" : {
        "id" : "test.sample",
        "type" : "card",
...
function createCardEditorTag(id, changes, mode, language, designtime) {
  ...
  return '<ui-integration-card-editor id="' + id + '" card=\'' +
         JSON.stringify(card) + '\' mode="' + mode + '" language="' + language +
...
function loadCurrentValues(id) {
  ...
  if (!dom)
    return;
  var settings = getItem(id), div = document.createElement("div");
  div.innerHTML = createCardEditorTag(
      id, [ settings ], dom.getAttribute("mode"),
      dom.getAttribute("language") || "", dom.getAttribute("designtime") || "");
...
function updateAllLayerCard() {
  ...
  if (!target)
    return;
  target.innerHTML = "";
  var settings = [], admin = getItem("cardEditorAdmin"),
...
function updateAdminContentLayerCard() {
  ...
  target.innerHTML =
      createCardEditorTag("cardEditorAdminContent", settings, "content", "");
}
function updateAdminContentTranslationLayerCard() {
  ...
  var settings = [], admin = getItem("cardEditorAdmin"),
      ...
      translation = getItem("cardEditorTranslation");
  settings.push(admin, content, translation);
  target.innerHTML = createCardEditorTag("cardEditorAdminContentTranslation",
                                         settings, "translation", "ru");
}

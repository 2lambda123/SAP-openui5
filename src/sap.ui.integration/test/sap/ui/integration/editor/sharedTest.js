...
function createCardEditorTag(id, changes, mode, language, designtime) {
  ...
  return (
      '<ui-integration-card-editor id="' + id + "\" card='" +
...
function loadCurrentValues(id) {
  ...
  div.innerHTML = createCardEditorTag(
      id,
      [ settings ],
      dom.getAttribute("mode"),
      dom.getAttribute("language") || "",
      dom.getAttribute("designtime") || "",
  );
...
function updateAllLayerCard() {
  ...
  target.innerHTML = createCardEditorTag(
      "cardEditorAll",
      settings,
      "all",
      "",
...
function updateAdminContentTranslationLayerCard() {
  ...
  settings.push(admin, content, translation);
  target.innerHTML = createCardEditorTag(
      "cardEditorAdminContentTranslation",
      settings,
      "translation",
      "ru",
  );

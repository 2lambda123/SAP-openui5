sap.ui.define(
    ...
    function(
      ...
      return BaseController.extend(
          "sap.ui.demo.cardExplorer.controller.ExploreSamples",
          {
            formatter : formatter,

            constructor : function() {
              this.onFileEditorManifestChangeDebounced = _debounce(
                  this.onFileEditorManifestChangeDebounced,
                  Constants.DEBOUNCE_TIME,
              );
              this.onFileEditorDesigntimeChangeDebounced = _debounce(
                  this.onFileEditorDesigntimeChangeDebounced,
                  Constants.DEBOUNCE_TIME,
              );
              this.onCardEditorConfigurationChangeDebounced = _debounce(
             ...
             */
            onFileEditorManifestChangeDebounced : function(
                sValue,
                bRerender,
            ) {
             ...
             * @param {boolean} bRerender If in rerender model
             */
            onFileEditorDesigntimeChangeDebounced : function(
                sValue,
                bRerender,
            ) {
              if ((this._sEditSource !== "cardEditor" || bRerender === true) &&
                  this._oCardEditor) {
                var oDesigntimeMetadata = JSON.parse(
                    sValue.slice(137, -11),
                );
            ...
            onCardEditorConfigurationChangeDebounced : function(oValues) {
              if (this._sEditSource === "cardEditor") {
                ...
                var sDesigntime = sDesigntimeHeader +
                                  oValues.configurationstring +
                                  ");\n	};\n});\n";
            ...
            onChangeSplitterOrientation : function(oEvent) {
              ...
              exploreSettingsModel.setProperty(
                  "/splitViewVertically",
                  !exploreSettingsModel.getProperty(
                      "/splitViewVertically",
                      ),
              );
              var isOrientationVertical = exploreSettingsModel.getProperty(
                  "/splitViewVertically",
              );
              this.getView()
                  .byId("splitView")
            ...
            _onDownloadCompressed : function(sExtension) {
              Promise
                  ...
                  .then(function(aArgs) {
                    ...
                    var sArchiveName = formatter.formatExampleName(
                        JSON.parse(aArgs[MANIFEST]),
                    );

                    FileUtils.downloadFilesCompressed(
                        aArgs[FILES],
                        sArchiveName,
                        sExtension,
                    );
            ...
            _initCardEditor : function() {
              ...
              if (!sBaseUrl || sBaseUrl === "") {
                ...
                var sManifestFileName = sBaseUrl.split("/").pop(),
                    sBaseUrl =
                        "." + sBaseUrl.substring(
                                  0,
                                  sBaseUrl.length - sManifestFileName.length,
                              );
              }
              var sJson;
              ...
              this._pLoadCardEditor
                  .then(
                      function(CardEditor) {
                        ...
                        this._oCardEditor = new CardEditor({
                          ...
                          configurationChange :
                              this.onCardEditorConfigurationChange.bind(
                                  this,
                                  ),
                          baseUrl : sBaseUrl,
                        });

                        this._oCardEditor.addStyleClass(
                            "sapUiSmallMargin",
                        );
                  ...
                  .then(
                      function(sDesigntimeContent) {
                        if (sDesigntimeContent) {
                          var oDesigntimeMetadata = JSON.parse(
                              sDesigntimeContent.slice(137, -11),
                          );
                          this._oCardEditor.updateDesigntimeMetadata(
                              oDesigntimeMetadata,
                          );
              ...
              Promise
                  ...
                  .then(
                      function() {
                        ...
                        this._fileEditor.setFiles(
                            oCurrentSample.files ||
                                [
                                  {
                        ...
                        if (bUseIFrame) {
                          ...
                          var sManifestUrl =
                                  this._fileEditor.getManifestFile().url,
                              oLayoutSettings = {
                                minRows : 1,
                                columns : 4,
                              };
                          ...
                          oLayoutSettings = Object.assign(
                              oLayoutSettings,
                              oCurrentSample.settings,
                          );
                        ...
                        .then(
                            function() {
                              return Fragment.load({
                                name :
                                    "sap.ui.demo.cardExplorer.view.CardSample",
                                controller : this,
                              });
                            }.bind(this),
                            )
            ...
            _updateSample : function(sValue) {
              ...
              if (exploreSettingsModel.getProperty("/useIFrame")) {
                ...
                try {
                  var sManifestFileName =
                          this._sSampleManifestUrl.split("/").pop(),
                      sBaseUrl = this._sSampleManifestUrl.substring(
                          0,
                          this._sSampleManifestUrl.length -
             ...
             */
            validateManifest :
                function() { this._fileEditor.validateManifest(); },

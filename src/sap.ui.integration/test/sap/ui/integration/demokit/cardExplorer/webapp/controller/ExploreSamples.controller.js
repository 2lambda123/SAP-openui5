sap.ui.define(
    ...
    function(BaseController, Constants, exploreNavigationModel,
             exploreSettingsModel, formatter, FileUtils,
             SEPMRA_PROD_MAN_mockServer, graphql_mockServer, MessageToast,
             Dialog, Button, library, FormattedText,
             GridContainerItemLayoutData, Core, Fragment, JSONModel,
             BindingMode, Device, loadCardEditor, _debounce) {
      "use strict";
      ...
      return BaseController.extend("sap.ui.demo.cardExplorer.controller.ExploreSamples", {
        ...
        constructor : function() {
          this.onFileEditorManifestChangeDebounced =
              _debounce(this.onFileEditorManifestChangeDebounced,
                        Constants.DEBOUNCE_TIME);
          this.onFileEditorDesigntimeChangeDebounced =
              _debounce(this.onFileEditorDesigntimeChangeDebounced,
                        Constants.DEBOUNCE_TIME);
          this.onCardEditorConfigurationChangeDebounced =
              _debounce(this.onCardEditorConfigurationChangeDebounced,
                        Constants.DEBOUNCE_TIME);
        ...
        onInit : function() {
          ...
          oRouter.getRoute("exploreSamples")
        ...
        onExit : function() { this._deregisterResize(); },
        ...
        onFileSwitch : function(oEvent) {
          exploreSettingsModel.setProperty("/editable",
                                           oEvent.getParameter("editable"));
        },

        onRunPressed : function(oEvent) {
          this._fileEditor.getManifestContent().then(
        ...
        onChangeSplitterOrientation : function(oEvent) {
          ...
          exploreSettingsModel.setProperty(
              "/splitViewVertically",
              !exploreSettingsModel.getProperty("/splitViewVertically"));
          var isOrientationVertical =
              exploreSettingsModel.getProperty("/splitViewVertically");
          this.getView()
              .byId("splitView")
              .getRootPaneContainer()
              .setOrientation(isOrientationVertical ? "Vertical"
                                                    : "Horizontal");
        },
        ...
        onDownloadManifestFile : function() {
          this._fileEditor.getManifestContent().then(function(sJSON) {
            FileUtils.downloadFile(sJSON, "manifest", "json",
                                   "application/json");
          });
        },
        ...
        _onDownloadCompressed : function(sExtension) {
          Promise
              ...
              .then(function(aArgs) {
                var MANIFEST = 0, FILES = 1;

                var sArchiveName =
                    formatter.formatExampleName(JSON.parse(aArgs[MANIFEST]));

                FileUtils.downloadFilesCompressed(aArgs[FILES], sArchiveName,
                                                  sExtension);
              });
        },
        ...
        _initCardEditor : function() {
          ...
          if (!sBaseUrl || sBaseUrl === "") {
            ...
            var sManifestFileName = sBaseUrl.split("/").pop(),
                sBaseUrl =
                    "." + sBaseUrl.substring(0, sBaseUrl.length -
                                                    sManifestFileName.length);
          }
          var sJson;
          this._pLoadCardEditor = this._pLoadCardEditor ||
                                  loadCardEditor(); // only trigger request once
          this._pLoadCardEditor
              .then(function(CardEditor) {
                ...
                this._oCardEditor = new CardEditor({
                  ...
                  configurationChange :
                      this.onCardEditorConfigurationChange.bind(this),
                  baseUrl : sBaseUrl
                });
              ...
              .then(function() { return this._fileEditor.getManifestContent(); }
                        .bind(this))
              ...
              .then(function(sDesigntimeContent) {
                if (sDesigntimeContent) {
                  var oDesigntimeMetadata =
                      JSON.parse(sDesigntimeContent.slice(137, -11));
                  this._oCardEditor.updateDesigntimeMetadata(
                      oDesigntimeMetadata);
                }
        ...
        _deregisterResize :
            function() { Device.media.detachHandler(this._onResize, this); },
        ...
        _onResize : function() {
          ...
          if (sRangeName == "Tablet" ||
              sRangeName == "Phone" && !isOrientationVertical) {
        ...
        _onRouteMatched : function(oEvent) {
          var oArgs = oEvent.getParameter("arguments"),
              sSampleKey = oArgs.sample, oSample = this._findSample(sSampleKey),
              sSubSampleKey = oArgs.subSample, oSubSample;
          ...
          exploreSettingsModel.setProperty("/isApplication",
                                           !!oSubSampleOrSample.isApplication);
        ...
        _onCardAction : function(oEvent) {
          ...
          if (sKey === "dataSources") {
            ...
            if (mParameters) {
              sMessage +=
                  " with parameters '" + JSON.stringify(mParameters) + "'";
            }

            MessageToast.show(sMessage,
                              {at : "center center", width : "25rem"});
          }
        ...
        _showSample : function(oSample, oSubSample) {
          ...
          Promise
              .all([
                this._initCardSample(), this._initMockServers(oCurrentSample)
              ])
              .then(function() {
                ...
                if (bUseIFrame) {
                  oFrameWrapperEl._sSample =
                      oSubSample ? oSample.key + "/" + oSubSample.key
                                 : oSample.key;
                  oFrameWrapperEl.invalidate();
                } else {
                  var sManifestUrl = this._fileEditor.getManifestFile().url,
                      oLayoutSettings = {minRows : 1, columns : 4};

                  oFrameWrapperEl._sSample = '';

                  oLayoutSettings =
                      Object.assign(oLayoutSettings, oCurrentSample.settings);

                  if (this._oCardSample) {
                    this._oCardSample.setLayoutData(
                        new GridContainerItemLayoutData(oLayoutSettings));
                    this.byId("cardContainer").invalidate();
                  }

                  sManifestUrl = sap.ui.require.toUrl(
                      "sap/ui/demo/cardExplorer" + sManifestUrl);
        ...
        _initMockServers : function(oSample) {
          var pAwait = Promise.resolve(), bMockServer = !!oSample.mockServer;

          // init mock server only on demand
          if (bMockServer) {
            pAwait = Promise.all([
              SEPMRA_PROD_MAN_mockServer.init(), graphql_mockServer.init()
            ]);
          }
        ...
        _initIFrameCreation : function() {
          var oFrameWrapperEl = this.byId("iframeWrapper"), oDelegate = {
            onAfterRendering : function() {
              var oFrameWrapperElDomRef = oFrameWrapperEl.getDomRef(), oFrame;

              if (oFrameWrapperElDomRef.firstChild) {
                oFrameWrapperElDomRef.removeChild(
                    oFrameWrapperElDomRef.firstChild);
              }
        ...
        createFrame : function(sSample) {
          ...
          // index.html will load separate scripts to enable editing the
          // manifest. in the file editor indexTemplate.html will be shown
          oFrameEl.src = sap.ui.require.toUrl(
              "sap/ui/demo/cardExplorer/samples/" + sSample + "/index.html");
          oFrameEl.width = "100%";
        ...
        _updateSample : function(sValue) {
          ...
          if (!sValue) {
            // TODO hide the card or something like that. Currently it shows
            // busy indicator which might be confusing
          ...
          if (exploreSettingsModel.getProperty("/useIFrame")) {
            ...
            try {
              var sManifestFileName = this._sSampleManifestUrl.split("/").pop(),
                  sBaseUrl = this._sSampleManifestUrl.substring(
                      0, this._sSampleManifestUrl.length -
                             sManifestFileName.length);

              this._oCardSample.setBaseUrl(sBaseUrl)
                  .setManifest(oValue)
                  .setParameters(null);
            } catch (oException) {
              this._oCardSample.setManifest(null);
          ...
          var oDialog =
              new Dialog({
                ...
                content : [ new FormattedText({
                  htmlText :
                      "<p class='sapUiNoMargin'><span class='sapMText'>You are about to open </span></p>" +
                          "<cite class='sapMText'>" + mParameters.url +
                          "</cite>" +
                ...
                afterClose : function() { oDialog.destroy(); }
                  ...
                  .addStyleClass(
                      "sapUiSizeCompact sapUiResponsiveContentPadding");
         ...
         */
        validateManifest : function() { this._fileEditor.validateManifest(); },

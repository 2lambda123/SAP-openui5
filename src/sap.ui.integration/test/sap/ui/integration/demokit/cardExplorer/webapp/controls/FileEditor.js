...
sap.ui.define(
    [
      "../util/FileUtils", "../util/SchemaValidator", "../model/formatter",
      "sap/base/util/extend", "sap/m/IconTabHeader", "sap/m/IconTabFilter",
      "sap/m/Image", "sap/m/MessageStrip", "sap/ui/codeeditor/CodeEditor",
      "sap/ui/core/Control"
    ],
    function(FileUtils, SchemaValidator, formatter, extend, IconTabHeader,
             IconTabFilter, Image, MessageStrip, CodeEditor, Control) {
      "use strict";
      var FileEditor =
          Control.extend("sap.ui.demo.cardExplorer.controls.FileEditor", {
            metadata : {
              properties : {
                /**
                 * Array containing list of objects which have property name and
                 * link
                 */
                files : {type : "object[]", defaultValue : []},
              ...
              events : {
                manifestChange : {parameters : {value : {type : "string"}}},
                designtimeChange : {parameters : {value : {type : "string"}}},
                fileSwitch : {parameters : {editable : {type : "boolean"}}}
              }
            },
            renderer : {
              apiVersion : 2,
      ...
      FileEditor.prototype._getEditor = function() {
        ...
        if (!oEditor) {
          oEditor = new CodeEditor(
              this.getId() + "-editor",
              {liveChange : this._onFileEdit.bind(this), syntaxHints : false});

          oEditor._oEditor.session.on("changeAnnotation",
                                      this._onSyntaxError.bind(this));
          oEditor._oEditor.completers =
              []; // This will prevent all auto complete and suggestions of the
                  // editor
      ...
      FileEditor.prototype._getImage = function() {
        ...
        if (!oImage) {
          oImage = new Image(this.getId() + "-image", {width : "100%"});
      ...
      FileEditor.prototype._update = function() {
        var sSelectedFileKey = this._getHeader().getSelectedKey(),
            ...
            iSelectedFileIndex = this._aFiles.findIndex(function(
                oEl) { return oEl.key === sSelectedFileKey; }),
            oSelectedFile = this._aFiles[iSelectedFileIndex],
            bEditable = this._isFileEditable(oSelectedFile);

        sFileExtension =
            sFileExtension === 'js' ? 'javascript' : sFileExtension;
      ...
      FileEditor.prototype._fetchContents = function() {
        var aFetchPromises = this._aFiles.map(function(oFile) {
          ...
          oFile.promise = FileUtils.fetch(
              sap.ui.require.toUrl("sap/ui/demo/cardExplorer" + oFile.url));
        ...
        Promise.all(aFetchPromises)
            .then(function(aData) {
              aData.map(function(sData, i) {
                this._aFiles[i]._url = URL.createObjectURL(
                    new Blob([ sData ], {type : this._aFiles[i]._type}));
              ...
              this.fireManifestChange(
                  {value : this.getManifestFile().content, reRender : true});
      ...
      FileEditor.prototype._onFileSwitch = function(oEvent) { this._update(); };
      ...
      FileEditor.prototype._onFileEdit = function(oEvent) {
        ...
        if (this._getHeader().getSelectedKey() === this.getManifestFile().key) {
          ...
          this.fireManifestChange({value : oEvent.getParameter("value")});
        } else if (this._getHeader().getSelectedKey() === "designtime.js") {
          this.getDesigntimeFile().content = oEvent.getParameter("value");
          this.fireDesigntimeChange({value : oEvent.getParameter("value")});
        } else {
          return;
        }
      };
      ...
      /**
       * There should be only 1 card manifest in sample, with name
       * "manifest.json" or "cardManifest.json"
       *
      ...
      FileEditor.prototype._isFileEditable = function(oFile) {
        return !oFile.isApplicationManifest &&
               (oFile.name.endsWith("manifest.json") ||
                oFile.name.endsWith("cardManifest.json") ||
                oFile.key === "designtime.js");
      };

      FileEditor.prototype._findIndex = function(sName) {
        return this._aFiles.findIndex(function(
            oFile) { return oFile.key === sName; });
      };
      ...
      FileEditor.prototype._onSyntaxError = function() {
        ...
        if (aErrorAnnotations && aErrorAnnotations.length) {
          aErrorAnnotations.forEach(function(oError) {
            sMessage +=
                "Line " + String(oError.row) + ": " + oError.text + '\n';
          });
      ...
      FileEditor.prototype._createInternalFiles = function(aFiles) {
        this._aFiles = aFiles.map(function(oFile) {
          return extend({}, oFile, {content : "", promise : null});
        });
      };
        ...
        this.getManifestContent()
            .then(function(sManifest) {
              return SchemaValidator.validate(
                  JSON.parse(sManifest)["sap.card"]);
            })
            .then(function() {
              this._getSchemaErrorsStrip().setText("").setVisible(false);
            }.bind(this))
      ...
      FileEditor.prototype.hideSchemaErrors =
          function() { this._getSchemaErrorsStrip().setVisible(false); };
      ...
      FileEditor.prototype.getManifestContent = function() {
        ...
        // always try to return the content first, in case it is already loaded
        // and edited
      ...
      FileEditor.prototype.getFilesWithContent = function() {
        var aFetchPromises =
            this._aFiles.map(function(oFile) { return oFile.promise; });

        return Promise.all(aFetchPromises)
            .then(function() { return this._aFiles; }.bind(this));
      };

      FileEditor.prototype.showError = function(
          sError) { this._getErrorsStrip().setVisible(true).setText(sError); };

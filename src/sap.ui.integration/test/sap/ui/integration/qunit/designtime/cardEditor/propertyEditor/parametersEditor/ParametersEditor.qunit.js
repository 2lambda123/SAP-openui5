    ...
    function(BaseEditor, QUnitUtils, _debounce, EditorQunitUtils) {
      ...
      QUnit.module(
              ...
              return this.oBaseEditor.getPropertyEditorsByName("sampleParams")
                  .then(
                      function(aPropertyEditor) {
                        ...
                        var oEditorContent = getParameterEditorContent(
                            this.oEditor,
                        );
          ...
          function() {
            if (!isIE) {
              QUnit.test(
                  "When a ParametersEditor is created",
                  function(assert) {
                    assert.ok(
                        this.oEditor.getDomRef() instanceof HTMLElement,
                        "Then it is rendered correctly (1/3)",
                    );
                    assert.ok(
                        this.oEditor.getDomRef() &&
              ...
              QUnit.test("When no label is provided", function(assert) {
                ...
                var fnOnValueChangeDebounced = _debounce(
                    function(vValue) {
                      assert.deepEqual(
                          vValue,
                ...
                this.oEditor.attachValueChange(function(oEvent) {
                  var vReceivedValue = Object.assign(
                      {},
              ...
              QUnit.test(
                  ...
                  function(assert) {
                    ...
                    // MapEditor dataflow problem
                    this.oEditor.attachValueChange(
                        _debounce(
                            function() {
                              assert.strictEqual(
                                  this.aItems[0]
              ...
              QUnit.test(
                  "When an element key is changed to an unique value",
                  function(assert) {
                    var fnDone = assert.async();
              ...
              QUnit.test(
                  "When the label is changed in the editor",
                  function(assert) {
                    var fnDone = assert.async();

                    this.oEditor.attachDesigntimeMetadataChange(
                        function(oEvent) {
                          assert.strictEqual(
                    ...
                    EditorQunitUtils.setInputValue(
                        this.aItems[0].label.getContent(),
                        "Changed Label",
                    );
                  },
              );
      ...
      QUnit.module(
          ...
          function() {
            ...
            QUnit.test(
                "When type change and key change are forbidden",
                function(assert) {
                  this.oBaseEditor.setConfig({
                    properties : {
                      sampleParameters : {
      ...
      QUnit.module(
          "Given the editor is used in the configuration scenario",
          {
            beforeEach : function() {
              this.oBaseEditor = new BaseEditor({
                config : {

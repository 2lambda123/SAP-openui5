...
sap.ui.define(
    [
      ...
      "sap/ui/qunit/QUnitUtils", "sap/base/util/restricted/_debounce",
      "qunit/designtime/EditorQunitUtils"
    ],
    function(BaseEditor, QUnitUtils, _debounce, EditorQunitUtils) {
      "use strict";
      var isIE = false;
      ...
      function getParameterEditorContent(oEditor) {
        return {
          ...
          items : oEditor.getContent().getItems()[0].getItems().map(function(
              oItem) {
      ...
      QUnit.module(
          "Parameters Editor: Given an editor config", {
            beforeEach : function(assert) {
              var mConfig = {
                "properties" : {
                  "sampleParams" :
                      {"path" : "/sampleParams", "type" : "parameters"}
                },
                "propertyEditors" : {
                  "parameters" :
              ...
              var mJson = {
                sampleParams : {
                  foo :
                      {value : "bar", label : "Test Parameter", type : "string"}
                }
              };
              ...
              return this.oBaseEditor.getPropertyEditorsByName("sampleParams")
                  .then(function(aPropertyEditor) {
                    this.oEditor = aPropertyEditor[0];
                    sap.ui.getCore().applyChanges();
                    var oEditorContent =
                        getParameterEditorContent(this.oEditor);
          ...
          function() {
            if (!isIE) {
              QUnit.test(
                  "When a ParametersEditor is created", function(assert) {
                    assert.ok(this.oEditor.getDomRef() instanceof HTMLElement,
                              "Then it is rendered correctly (1/3)");
                    assert.ok(this.oEditor.getDomRef() &&
                                  this.oEditor.getDomRef().offsetHeight > 0,
                              "Then it is rendered correctly (2/3)");
                    assert.ok(this.oEditor.getDomRef() &&
                                  this.oEditor.getDomRef().offsetWidth > 0,
              ...
              QUnit.test("When a value is set", function(assert) {
                ...
                assert.strictEqual(
                    this.aItems[0].value.getValue(), "bar",
              ...
              QUnit.test("When no label is provided", function(assert) {
                ...
                var fnOnValueChangeDebounced = _debounce(function(vValue) {
                  assert.deepEqual(
                      vValue, {"foo" : {"value" : "baz", "type" : "string"}},
                      "Then the fallback label is not written to the manifest");
                  assert.strictEqual(
                      this.aItems[0].label.getContent().getPlaceholder(), "foo",
                      "Then the key is used as a default label");
                  fnDone();
                }.bind(this), 0);
                this.oEditor.attachValueChange(function(oEvent) {
                  var vReceivedValue =
                      Object.assign({}, oEvent.getParameter("value"));
                  fnOnValueChangeDebounced(vReceivedValue);
                });

                this.oEditor.setValue({foo : {value : "baz", type : "string"}});
              });

              QUnit.test(
                  "When the key for an item with fallback label is changed",
                  function(assert) {
                    var fnDone = assert.async();
                    ...
                    this.oEditor.setValue(
                        {foo : {value : "baz", type : "string"}});
                    EditorQunitUtils.setInputValue(
                        this.aItems[0].key.getContent(), "foo2");
                  });

              QUnit.test("When an element key is changed to an unique value",
                         function(assert) {
                           var fnDone = assert.async();
              ...
              QUnit.test(
                  "When the label is changed in the editor", function(assert) {
                    var fnDone = assert.async();

                    this.oEditor.attachDesigntimeMetadataChange(function(
                        oEvent) {
                      assert.strictEqual(
                          oEvent.getParameter("value").foo.__value.label,
                          "Changed Label", "Then the label is updated");
                      fnDone();
                    });
                    EditorQunitUtils.setInputValue(
                        this.aItems[0].label.getContent(), "Changed Label");
                  });
            }
          });
      ...
      QUnit.module(
          ...
          function() {
            if (!isIE) {
              QUnit.test(
                  "When invalid entries should be filtered and a non-string value without a type is set",
                  function(assert) {
                    ...
                    this.oBaseEditor.setJson({
                      sampleParameters : {
                        "invalidProperty" : {value : 123},
                        "validProperty" : {value : "Valid"}
                      }
                    });

                    return this.oBaseEditor
                        .getPropertyEditorsByName("sampleParameters")
                        .then(function(aPropertyEditor) {
                          var oParametersEditor = aPropertyEditor[0];
                          var oParametersEditorContent =
                              getParameterEditorContent(oParametersEditor);
                          var aItems = oParametersEditorContent.items;

                          assert.strictEqual(
                              aItems.length, 1,
                              "Then the invalid value is not included");
                        });
                  });
            }

            QUnit.test("When type change and key change are forbidden", function(
                                                                            assert) {
              this.oBaseEditor.setConfig({
                "properties" : {
                  "sampleParameters" : {
              ...
              return this.oBaseEditor
                  ...
                  .then(function(aPropertyEditor) {
                    var oParametersEditor = aPropertyEditor[0];
                    assert.strictEqual(
                        oParametersEditor.getAggregation('propertyEditor')
                            .getFragment(),
      ...
      QUnit.module("Given the editor is used in the configuration scenario", {
        beforeEach : function() {
          this.oBaseEditor = new BaseEditor({
            ...
            json : {
              sampleParameters :
                  {foo : {value : "foo value"}, bar : {value : "bar value"}}
            }
          });
          this.oBaseEditor.placeAt("qunit-fixture");

          return this.oBaseEditor.getPropertyEditorsByName("sampleParameters")
              .then(function(aPropertyEditor) {
        ...
        afterEach : function() { this.oBaseEditor.destroy(); }
                   ...
                   function() {
                     QUnit.test("When a value is changed", function(assert) {
                       ...
                       this.oParametersEditor.attachEventOnce(
                       ...
                       var oInput = this.oParametersEditor.getContent()
                                        ._getPropertyEditors()[1]
                                        .getContent();

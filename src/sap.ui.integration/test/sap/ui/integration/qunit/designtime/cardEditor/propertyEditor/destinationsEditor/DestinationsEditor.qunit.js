...
sap.ui.define(
    [
      ...
      "qunit/designtime/EditorQunitUtils", "sap/ui/thirdparty/sinon-4"
    ],
    function(BaseEditor, EditorQunitUtils, sinon) {
      "use strict";
      var isIE = false;
      ...
      QUnit.module(
          "Developer scenario", {
            before : function() {
              this.oDestinations = {
                destinations : {
                  sampleDestination : {name : "MySampleDestination"},
                  anotherDestination :
                      {name : "AnotherDestination", label : "Hello World"}
                }
              };
            },
            beforeEach : function() {
              ...
              this.oBaseEditor =
                  new BaseEditor({config : mConfig, json : this.oDestinations});
              this.oBaseEditor.placeAt("qunit-fixture");

              return this.oBaseEditor
                  .getPropertyEditorsByName("sampleDestination")
                  .then(function(aPropertyEditor) {
                    this.oDestinationsEditor = aPropertyEditor[0];
                    this.oNestedArrayEditor =
                        this.oDestinationsEditor.getContent();
                  }.bind(this));
            },
            afterEach : function() {
          ...
          function() {
            QUnit.test("When an editor is created", function(assert) {
              var oDestinationsEditorDomRef =
                  this.oDestinationsEditor.getDomRef();
              assert.ok(oDestinationsEditorDomRef instanceof HTMLElement,
                        "Then it is rendered correctly (1/3)");
              assert.ok(oDestinationsEditorDomRef.offsetHeight > 0,
                        "Then it is rendered correctly (2/3)");
              assert.ok(oDestinationsEditorDomRef.offsetWidth > 0,
            ...
            QUnit.test("When config and json data were set", function(assert) {
              assert.deepEqual(
                  ...
                  [
                    {name : "MySampleDestination", key : "sampleDestination"}, {
            ...
            QUnit.test("When the label is set in the editor", function(assert) {
              ...
              var oLabelEditor =
                  _getComplexMapEditors(this.oNestedArrayEditor)[0].label;

              this.oDestinationsEditor.attachValueChange(function(oEvent) {
                assert.deepEqual(oEvent.getParameter("value"), {
                  "anotherDestination" :
                      {"label" : "Hello World", "name" : "AnotherDestination"},
                  "sampleDestination" :
                      {"label" : "Label", "name" : "MySampleDestination"}
                },
                                 "Then the label is updated");
                fnDone();
              });
              EditorQunitUtils.setInputValue(oLabelEditor.getContent(),
                                             "Label");
            });
          });
      ...
      QUnit.module(
          "Configuration options", {
            beforeEach : function() {
              var mJson = {destinations : {sampleDestination : {name : "abc"}}};
          ...
          function() {
            if (!isIE) {
              QUnit.test("When allowed values are set", function(assert) {
                var mConfig = _createBaseEditorConfig(
                    {allowKeyChange : false, allowedValues : [ "abc", "def" ]});
                ...
                return this.oBaseEditor
                    ...
                    .then(function(aPropertyEditor) {
                      ...
                      return oNestedArrayEditor.ready().then(function() {
                        ...
                        assert.deepEqual(
                            Object.keys(oComplexEditors), [ "name" ],
                            "Then only the name field is editable");

                        assert.deepEqual(
                            oComplexEditors.name.getConfig().items,
                            mConfig.properties.sampleDestination.allowedValues
                                .map(function(sKey) { return {key : sKey}; }),

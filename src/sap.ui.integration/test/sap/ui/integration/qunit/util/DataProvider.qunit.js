...
sap.ui.define(
    ...
    function(DataProviderFactory, ServiceDataProvider, DataProvider,
             RequestDataProvider, Card) {
      "use strict";
      ...
      function testSetSettings(DataProviderClass) {
        ...
        QUnit.test("setSettings - regression test for " +
                       oDataProvider.getMetadata().getName(),
                   function(assert) {
                     // Arrange
                     var oSettings = {"key" : "value"};
                     ...
                     // Assert
                     assert.deepEqual(
                         oDataProvider.getSettings(), oSettings,
                         "Getters and setters should work as expected.");
      ...
      QUnit.module("DataProviderFactory", {
        beforeEach : function() {
          this.oCard = new Card();
          this.oDataProviderFactory =
              new DataProviderFactory(null, null, this.oCard);
      ...
      QUnit.test("create with null or undefined", function(assert) {
        assert.notOk(this.oDataProviderFactory.create(null),
                     "Should have NOT created a DataProvider instance.");
        assert.notOk(this.oDataProviderFactory.create(),
                     "Should have NOT created a DataProvider instance.");
        assert.notOk(DataProvider.prototype.setSettings.calledOnce,
                     "Should have NOT called setSettings.");
      });

      QUnit.test("create with unknown settings", function(assert) {
        // Arrange
        var oDataSettings = {destination : {url : "/some/relative/url"}};

        // Act
        var oDataProvider = this.oDataProviderFactory.create(oDataSettings);

        // Assert
        assert.notOk(oDataProvider,
                     "Should have NOT created a DataProvider instance.");
        assert.notOk(DataProvider.prototype.setSettings.calledOnce,
                     "Should have NOT called setSettings.");
      });
      ...
      QUnit.test("create with request settings", function(assert) {
        // Arrange
        var oDataSettings = {request : {url : "/some/relative/url"}};

        // Act
        var oDataProvider = this.oDataProviderFactory.create(oDataSettings);

        // Assert
        assert.ok(oDataProvider,
                  "Should have created a DataProvider instance.");
        assert.ok(
            oDataProvider.isA("sap.ui.integration.util.RequestDataProvider"),
            "Should have created a DataProvider instance of type RequestDataProvider.");
        assert.ok(DataProvider.prototype.setSettings.calledOnce,
                  "Should have called setSettings.");
        assert.ok(DataProvider.prototype.setSettings.getCall(0).calledWith(
                      oDataSettings),
                  "Should have called setSettings with correct arguments.");
      });

      QUnit.test("create with service settings", function(assert) {
        // Arrange
        var oDataSettings = {service : {name : "UserRecent"}};
        var oServiceManager = {};
        sinon.stub(ServiceDataProvider.prototype, "createServiceInstances");

        // Act
        var oDataProvider =
            this.oDataProviderFactory.create(oDataSettings, oServiceManager);

        // Assert
        assert.ok(oDataProvider,
                  "Should have created a DataProvider instance.");
        assert.ok(
            oDataProvider.isA("sap.ui.integration.util.ServiceDataProvider"),
            "Should have created a DataProvider instance of type ServiceDataProvider.");
        assert.ok(DataProvider.prototype.setSettings.calledOnce,
                  "Should have called setSettings.");
        assert.ok(DataProvider.prototype.setSettings.getCall(0).calledWith(
                      oDataSettings),
                  "Should have called setSettings with correct arguments.");
        assert.ok(
            ServiceDataProvider.prototype.createServiceInstances.calledOnce,
            "Should have called createServiceInstances.");
        assert.ok(
            ServiceDataProvider.prototype.createServiceInstances.getCall(0)
                .calledWith(oServiceManager),
            "Should have called createServiceInstances with ServiceManager reference.");
      ...
      QUnit.test("create with static JSON settings", function(assert) {
        // Arrange
        var oDataSettings = {json : {"key" : "value"}, updateInterval : 1};
        ...
        assert.ok(oDataProvider,
                  "Should have created a DataProvider instance.");
        assert.ok(oDataProvider.isA("sap.ui.integration.util.DataProvider"),
                  "Should have created a DataProvider instance.");
      });

      QUnit.test("destroy", function(assert) {
        // Arrange
        var oSettings = {json : {key : "value"}};
        ...
        // Assert
        assert.ok(
            oDestroySpy.calledOnce,
            "Should destroy all created instances when destroying the factory.");
      });

      QUnit.module("DataProvider", {
        beforeEach : function() { this.oDataProvider = new DataProvider(); },
        afterEach : function() {
      ...
      QUnit.test("updateInterval", function(assert) {
        ...
        // Arrange
        var oSettings = {json : {"key" : "value"}, updateInterval : 1};
        ...
        setTimeout(function() {
          assert.ok(fnSpy.callCount > 1,
                    "onDataRequestComplete is called more than once");
          done();
        }, 2000);
      ...
      QUnit.test("triggerDataUpdate - with JSON", function(assert) {
        ...
        this.oDataProvider.attachDataChanged(function(oEvent) {
          ...
          assert.ok(fnErrorSpy.notCalled,
                    "Should NOT call error event handler.");
      ...
      QUnit.test("triggerDataUpdate - missing JSON", function(assert) {
        ...
        this.oDataProvider.attachError(function(oEvent) {
          ...
          assert.ok(fnDataChangedSpy.notCalled,
                    "Should NOT call data changed event handler.");
      ...
      QUnit.test("method _isValidRequest", function(assert) {
        assert.notOk(this.oDataProvider._isValidRequest({}),
                     "Should have an invalid request.");
        assert.notOk(this.oDataProvider._isValidRequest({url : 5}),
                     "Should have an invalid request.");
        assert.notOk(this.oDataProvider._isValidRequest(),
                     "Should have an invalid request.");
        ...
        assert.notOk(this.oDataProvider._isValidRequest(oInvalidRequest),
                     "Should have an invalid request.");
        ...
        assert.ok(this.oDataProvider._isValidRequest(oValidRequest),
      ...
      QUnit.test("triggerDataUpdate - request successful", function(assert) {
        // Arrange
        var done = assert.async();
        var oSettings = {request : {"url" : "some/relative/url"}};
        var fnErrorSpy = sinon.spy();
        this.oDataProvider.setSettings(oSettings);
        this.oDataProvider.attachError(fnErrorSpy);
        this.oDataProvider.attachDataChanged(function(oEvent) {
        ...
        // Act
        this.oDataProvider.triggerDataUpdate();
        this.deferred.resolve({mockData : [ 1, 2, 3 ]});
      });
      ...
      QUnit.module("RequestDataProvider - Content encoding", {
        beforeEach : function() {
          this.oDataProvider = new RequestDataProvider();
          this.oServer = sinon.createFakeServer({autoRespond : true});
        },
        afterEach : function() { this.oServer.restore(); }
      });
      ...
      QUnit.test("Content-Type: application/json", function(assert) {
        ...
        this.oDataProvider.setSettings({
          request : {
            ...
            parameters : {someKey : "someValue", someKey2 : "someValue2"}
          }
        });

        this.oServer.respondWith(
            "POST", "/data/provider/test/url", function(oXhr) {
              // Assert
              try {
                ...
                assert.ok(false,
                          "Request body is NOT correctly encoded as JSON");
              }
      ...
      QUnit.test(
          ...
          function(assert) {
            ...
            this.oServer.respondWith(
                "POST", "/data/provider/test/url", function(oXhr) {
                  // Assert
                  assert.strictEqual(oXhr.requestBody,
      ...
      QUnit.module("ServiceDataProvider", {
        beforeEach : function() {
          ...
          // Mocks an instance of a service implementing
          // sap.ui.integration.services.Data interface.
          this.oServiceInstanceMock = {
            _getDataShouldFail : false,
            update : function() { this._fnCbk(that.oData); },
            attachDataChanged : function(fnCbk) { this._fnCbk = fnCbk; },
            getData : function() {
      ...
      QUnit.test("triggerDataUpdate - data request successful", function(
                                                                    assert) {
        // Arrange
        var done = assert.async();
        var oDataSettings = {service : "UserRecent"};
        var fnErrorSpy = sinon.spy();
        ...
        this.oDataProvider.attachDataChanged(function(oEvent) {
          ...
          assert.deepEqual(oEvent.getParameter("data"), this.oData,
      ...
      QUnit.test("triggerDataUpdate - data request failed", function(assert) {
        ...
        this.oDataProvider.attachError(function(oEvent) {
          // Assert
          assert.ok(fnDataChangedSpy.notCalled,
                    "Should NOT call data changed handler.");
          assert.ok(oEvent.getParameter("message"),
      ...
      QUnit.test("Service triggers data changed", function(assert) {
        ...
        this.oDataProvider._oDataServicePromise.then(function() {
          ...
          // Assert
          assert.ok(fnErrorSpy.notCalled, "Should NOT call the error handler.");
          assert.equal(fnDataChangedSpy.callCount, 3,
                       "Should call data changed handler three times.");

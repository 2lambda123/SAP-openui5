...
sap.ui.define(
    ...
    function(
      ...
      function testSetSettings(DataProviderClass) {
        ...
        QUnit.test(
            "setSettings - regression test for " +
      ...
      QUnit.test("create with null or undefined", function(assert) {
        assert.notOk(
            this.oDataProviderFactory.create(null),
        ...
        assert.notOk(
            DataProvider.prototype.setSettings.calledOnce,
      ...
      QUnit.test("create with request settings", function(assert) {
        ...
        assert.ok(
            DataProvider.prototype.setSettings.getCall(0).calledWith(
                oDataSettings),
      ...
      QUnit.test("create with static JSON settings", function(assert) {
        ...
        assert.ok(
            oDataProvider.isA("sap.ui.integration.util.DataProvider"),
            "Should have created a DataProvider instance.",
        );
      });
      ...
      QUnit.module("RequestDataProvider", {
        beforeEach : function() {
          ...
          sinon.stub(jQuery, "ajax")
              .callsFake(
                  function() { return this.deferred.promise(); }.bind(this),
              );
        },
        afterEach : function() {
          this.oDataProvider.destroy();
      ...
      QUnit.test("method _isValidRequest", function(assert) {
        assert.notOk(
            this.oDataProvider._isValidRequest({}),
            "Should have an invalid request.",
        );
        assert.notOk(
            this.oDataProvider._isValidRequest({url : 5}),
            "Should have an invalid request.",
        );
        assert.notOk(
            this.oDataProvider._isValidRequest(),
        ...
        assert.notOk(
            this.oDataProvider._isValidRequest(oInvalidRequest),
            "Should have an invalid request.",
        );
        ...
        assert.ok(
            this.oDataProvider._isValidRequest(oValidRequest),
        ...
        this.oServer.respondWith(
            ...
            function(oXhr) {
              ...
              try {
                ...
                assert.ok(
                    true,
                    "Request body is correctly encoded as JSON",
                );
              } catch (e) {
                assert.ok(
                    false,
      ...
      QUnit.test(
          "triggerDataUpdate - data request successful",
          function(assert) {
            // Arrange
            var done = assert.async();
            ...
            this.oDataProvider.attachError(fnErrorSpy);
            this.oDataProvider.attachDataChanged(
                function(oEvent) {
                  // Assert
                  assert.ok(
                      fnErrorSpy.notCalled,
            ...
            this.oDataProvider.createServiceInstances(
                this.oServiceManagerMock,
            );
      ...
      QUnit.test(
          "triggerDataUpdate - data request failed",
          function(assert) {
            // Arrange
            var done = assert.async();
            var oDataSettings = {service : "UserRecent"};
      ...
      QUnit.test(
          "triggerDataUpdate - No service instance",
          function(assert) {
            // Arrange
            var done = assert.async();
            var oDataSettings = {service : "UserRecent"};
      ...
      QUnit.test("Service triggers data changed", function(assert) {
        ...
        this.oDataProvider._oDataServicePromise.then(
            function() {
              // Act
      ...
      QUnit.test(
          "createServiceInstances after setSettings",
          function(assert) {
            // Arrange
            var done = assert.async();
            var oDataSettings = {service : {name : "UserRecent"}};
